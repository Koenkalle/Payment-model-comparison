...
DEFAULTS = dict(time_feat_dim=32, channel_embedding_dim=32, patch_size=2,
...
class Model:
    ...
    def _initialize(self, dataset, parameters):
        ...
        if unknown:
            raise ValueError('Unknown DyGFormer + TAMI parameters: ' + ', '.join(sorted(unknown)))
        self.parameters = {**DEFAULTS, **parameters}
        p = self.parameters
        for key in ('time_feat_dim', 'channel_embedding_dim', 'patch_size', 'num_layers',
                    'num_heads', 'max_input_sequence_length', 'epochs', 'batch_size', 'patience', 'num_threads'):
            if type(p[key]) is not int or p[key] < 1:
                raise ValueError(key + ' must be a positive integer.')
        if p['max_input_sequence_length'] < 2 or p['max_input_sequence_length'] % p['patch_size']:
            raise ValueError('max_input_sequence_length must be >= 2 and divisible by patch_size.')
        ...
        if not np.isfinite(p['learning_rate']) or p['learning_rate'] <= 0 or not np.isfinite(p['weight_decay']) or p['weight_decay'] < 0:
            raise ValueError('learning_rate must be positive and weight_decay nonnegative.')
        torch.manual_seed(p['seed'])
        torch.set_num_threads(p['num_threads'])
        backbone = DyGFormer(
            dataset.node_features.copy(), dataset.edge_features.copy(),
            NeighborSampler(dataset, history_limit=p['max_input_sequence_length'] - 1),
            **{key: p[key] for key in ('time_feat_dim', 'channel_embedding_dim', 'patch_size',
                                      'num_layers', 'num_heads', 'dropout', 'max_input_sequence_length', 'device')})
        dim = dataset.node_features.shape[1]
        self.network = nn.Sequential(backbone, HistoricalDecoder(dim, dim, dim, device=p['device'], gamma=p['gamma'])).to(p['device'])
        self.feature_schema = {'node': list(dataset.node_feature_names), 'edge': list(dataset.edge_feature_names)}
    ...
    def _bind(self, dataset, indices=None):
        if self.feature_schema != {'node': list(dataset.node_feature_names), 'edge': list(dataset.edge_feature_names)}:
            raise ValueError('Graph feature names or order differ from the fitted model.')
        backbone = self.network[0]
        backbone.node_raw_features = torch.from_numpy(dataset.node_features.copy()).to(self.parameters['device'])
        backbone.edge_raw_features = torch.from_numpy(dataset.edge_features.copy()).to(self.parameters['device'])
        backbone.neighbor_sampler = NeighborSampler(dataset, indices, self.parameters['max_input_sequence_length'] - 1)
        self.node_ids = dataset.node_ids
    ...
    def score_group(self, dataset, group, negative_destinations=None):
        ...
        u, v, t = dataset.sources[group], dataset.destinations[group], dataset.times[group]
        source, destination = self.network[0].compute_src_dst_node_temporal_embeddings(u, v, t)
        ...
        if negative_destinations is not None:
            ...
            if valid.any():
                ...
                negative = self.network[1](u[valid], negative_destinations[valid], source, destination).flatten()
        return positive.flatten(), negative, proposed

    def _commit(self, dataset, group, proposed):
        self.memory.update_memories(zip(dataset.sources[group], dataset.destinations[group]), proposed)

    def fit_graph(self, dataset, training, validation, parameters):
        self._initialize(dataset, parameters)
        p = self.parameters
        if not len(training) or not len(validation) or dataset.times[training].max() >= dataset.times[validation].min():
            raise ValueError('Graph fitting needs strictly separated train and validation intervals.')
        optimizer = torch.optim.Adam(self.network.parameters(), lr=p['learning_rate'], weight_decay=p['weight_decay'])
        ...
        for epoch in range(p['epochs']):
            ...
            for group in timestamp_groups(dataset, training):
                ...
                positive, negative, proposed = self.score_group(dataset, group, negatives)
                if negative is not None:
                    logits = torch.cat([positive[negatives >= 0], negative])
                    labels = torch.cat([torch.ones_like(negative), torch.zeros_like(negative)])
            ...
            if pending:
                step()
            if not count:
                raise ValueError('Training has no valid negative links; need alternative observed destinations.')
    ...
    def predict_graph(self, dataset, indices, seed=10042):
        ...
        if not len(indices) or np.any(np.diff(indices) <= 0):
            raise ValueError('Evaluation indices must be nonempty, unique and chronological.')
        self._bind(dataset)
        self.network.eval()
        ...
        with torch.no_grad():
            for group in timestamp_groups(dataset, prefix):
                ...
                self._commit(dataset, group, proposed)
        return {'positive_logits': positive, 'negative_logits': negative, 'negative_destinations': destinations}
    ...
    def save(self, path):
        ...
        weights['memory_values'] = np.stack([memory[key].cpu().numpy() for key in keys]) if keys else np.empty((0, dim), dtype=np.float32)
        weights['metadata'] = np.asarray(json.dumps({'version': 1, 'parameters': self.parameters,
            'features': self.feature_schema, 'node_ids': self.node_ids, 'history': self.training_history,
    ...
    def load_graph(self, path, dataset):
        with np.load(path, allow_pickle=False) as saved:
            ...
            schema = {'node': list(dataset.node_feature_names), 'edge': list(dataset.edge_feature_names)}
            if schema != metadata['features']:
                raise ValueError('Graph feature names or order differ from the fitted model.')
            parameters = {**metadata['parameters'], 'device': 'cpu'}
            self._initialize(dataset, parameters)
            weights = {key: torch.from_numpy(saved[key].copy()) for key in self.network.state_dict()}
            ...
            if list(dataset.node_ids) == metadata['node_ids']:
                self.memory.update_memories(saved['memory_keys'], torch.from_numpy(saved['memory_values'].copy()))
