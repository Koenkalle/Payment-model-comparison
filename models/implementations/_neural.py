...
def ce(logits, targets, mask):
    a = logits.d
    prob = np.exp(a - a.max(-1, keepdims=True))
    prob /= prob.sum(-1, keepdims=True)
    ix = np.arange(len(targets))
    m = mask / max(1, mask.sum())
    out = T(float((-np.log(np.maximum(prob[ix, targets], 1e-30)) * m).sum()), (logits,))

    def back():
        g = prob.copy()
        g[ix, targets] -= 1
        logits.acc(out.g * g * m[:, None])
...
class NeuralModel:
    def __init__(self, seed=512, design="gru_attention"):
        self.design = self.architecture
        self.hidden = self.design["hidden"]
        self.layers = self.design["layers"]
        H = self.hidden
        rng = np.random.default_rng(seed)
        self.p = {}

        def weight(name, ins, outs):
            self.p[name] = T(rng.normal(0, 0.7 / math.sqrt(ins), (ins, outs)))

        def bias(name, n):
            self.p[name] = T(np.zeros(n))

        if self.design["memory"] == "gru":
            for gate in ["z", "r", "n"]:
                weight("W" + gate, 2 * H + 6, H)
                bias("b" + gate, H)
        ...
        if H:
            ...
            weight("rk", H, H)
        weight("pair", 3, 1)
        for name, size in [("amount", len(AMOUNT) + 1), ("gap", len(GAP) + 2)]:
            weight(name, 2 * H + 5, size)
            bias(name + "b", size)

    def update(self, h, event):
        if self.design["memory"] == "none":
            return h
        mask, ids, f = event
        p = self.p
        cp = gather(h, ids) * (1 - f[..., 2:3])
        x = cat(cp, T(f))
        joined = cat(x, h)
        z = (joined @ p["Wz"] + p["bz"]).sigmoid()
        r = (joined @ p["Wr"] + p["br"]).sigmoid()
        nxt = (cat(x, r * h) @ p["Wn"] + p["bn"]).tanh()
        return ((1 + z * -1) * h + z * nxt) * mask + h * (1 - mask)

    def readout(self, h, graph):
        ids, attrs, valid = graph
        p = self.p
        H = self.hidden
        for l in range(self.layers):
            nbr = gather(h, ids)
            edge = T(attrs) @ p[f"edge{l}"]
            val = nbr @ p[f"v{l}"] + edge
            if self.design["readout"] == "attention":
                key = nbr @ p[f"k{l}"] + edge
                q = (h @ p[f"q{l}"]).expand(2)
                w = ((q * key).sum(-1) * H**-0.5 + (1 - valid) * -1e9).softmax()
            else:
                w = T(valid / valid.sum(-1, keepdims=True))
            h = (h @ p[f"self{l}"] + (w.expand(-1) * val).sum(2) + p[f"out{l}"]).tanh()
        return h

    def predict(self, z, query):
        u, v, pairs, context = query
        p = self.p
        sender = gather(z, u)
        receiver = gather(z, v)
        r = (
            ((sender @ p["rq"]).expand(1) * (z @ p["rk"])).sum(-1) * self.hidden**-0.5
            if self.hidden
            else T(np.zeros(z.d.shape[:2]))
        )
...
def prepare(episodes):
    ...
    ov = oc.copy()
    iv = oc.copy()
    pairs = np.zeros((B, N, N))
    lists = [[[] for _ in range(N)] for _ in range(B)]
    out = []
    for step in range(len(episodes[0]["events"])):
        ids = np.tile(np.arange(N)[None, :, None], (B, 1, K))
        attrs = np.zeros((B, N, K, 4))
        valid = np.zeros((B, N, K))
        valid[:, :, 0] = 1
        mask = np.zeros((B, N, 1))
        cp = np.zeros((B, N), int)
        f = np.zeros((B, N, 6))
        ...
        for b, ep in enumerate(episodes):
            ...
            for n in range(N):
                for k, (other, when, amt, role) in enumerate(
                    lists[b][n][-K + 1 :][::-1], 1
                ):
                    ids[b, n, k] = other
                    attrs[b, n, k] = [
                        math.log1p(amt) / 8,
                        math.log1p(max(0, now[b] - when)) / 6,
                        role,
                        1,
                    ]
                    valid[b, n, k] = 1
            if u >= 0:
                lossmask[b] = 1
                pf[b, :, 0] = np.log1p(pairs[b, u]) / 3
                pf[b, :, 1] = np.log1p(ic[b]) / 6
                pf[b, :, 2] = pairs[b, u] + pairs[b, :, u] > 0
                ctx[b] = [
                    math.log1p(ov[b, u] / max(1, oc[b, u])) / 8,
                    math.log1p(iv[b, v] / max(1, ic[b, v])) / 8,
                    math.log1p(seen[b, u]) / 6,
                    math.log1p(seen[b, v]) / 6,
                    math.log1p(pairs[b, u, v]) / 3,
                ]
                targets[:, b] = [
                    v,
                    np.searchsorted(AMOUNT, e["amount"], side="right"),
                    np.searchsorted(GAP, max(0, t - last[b, u]), side="right")
                    if seen[b, u]
                    else len(GAP) + 1,
                ]
            ...
            for n, other, role in actors:
                ...
                cp[b, n] = max(0, other)
                f[b, n, role] = 1
                f[b, n, 3] = float(e.get("settled", True) is False)
                f[b, n, 4:] = [
                    math.log1p(e["amount"]) / 8,
                    math.log1p(max(0, t - last[b, n])) / 6,
                ]
                last[b, n] = t
                seen[b, n] += 1
            if u >= 0 and e.get("settled", True):
                lists[b][u].append((v, t, e["amount"], -1))
                lists[b][v].append((u, t, e["amount"], 1))
                oc[b, u] += 1
                ic[b, v] += 1
                ov[b, u] += e["amount"]
                iv[b, v] += e["amount"]
            ...
            now[b] = t
        out.append(
            ((mask, cp, f), (ids, attrs, valid), (us, vs, pf, ctx), targets, lossmask)
        )
    return out


def run(model, episodes, train=False, trace=False):
    h = T(np.zeros((len(episodes), episodes[0]["accounts"], model.hidden)))
    losses = []
    records = []
    for event, graph, q, targets, mask in prepare(episodes):
        z = model.readout(h, graph)
        pred = model.predict(z, q)
        if train and mask.sum():
            losses.append(sum([ce(a, b, mask) for a, b in zip(pred, targets)], T(0)))
        if trace:
            probs = [x.softmax().d for x in pred]
            ix = np.arange(len(episodes))
            scores = sum(
                -np.log(np.maximum(p[ix, target], 1e-30))
            ...
            records.append(
                {
                    ...
                    "memory_before": h.d.tolist(),
                    "embedding": z.d.tolist(),
                    "queries": [
                        {
                            "u": int(q[0][b]),
                            "v": int(q[1][b]),
                            "pair_features": q[2][b].tolist(),
                            "context": q[3][b].tolist(),
                        }
        ...
        if trace:
            records[-1]["memory_after"] = h.d.tolist()
    return sum(losses, T(0)) * (1 / max(1, len(losses))), records


def train_design(a, data, design, labeled=None):
    model = Model(seed=a.model_seed, design=design)
    rng = np.random.default_rng(7131)
    moments = {n: (np.zeros_like(p.d), np.zeros_like(p.d)) for n, p in model.p.items()}
    start = time.time()
    losslog = []
    for iteration in range(1, a.steps + 1):
        ...
        for p in model.p.values():
            p.g.fill(0)
        loss, _ = run(model, eps, True)
        loss.backward()
        losslog.append(float(loss.d))
        norm = math.sqrt(sum(float((p.g * p.g).sum()) for p in model.p.values()))
        for name, p in model.p.items():
            g = p.g * min(1, 3 / max(norm, 1e-12))
            m, v = moments[name]
            m[:] = 0.9 * m + 0.1 * g
            v[:] = 0.999 * v + 0.001 * g * g
            p.d -= (
                0.009
                * (m / (1 - 0.9**iteration))
                / (np.sqrt(v / (1 - 0.999**iteration)) + 1e-8)
            )
        if iteration % 20 == 0:
            print(
                f"{design} {iteration}/{a.steps} loss {float(loss.d):.4f}; {time.time()-start:.0f}s",
                flush=True,
            )
    val, _ = run(model, data[-24:], True)
    summary = {
        "method": "Self-supervised recipient + amount-bin + gap-bin likelihood",
        "fraud_labels_used": 0,
        ...
        "training_data_sha256": hashlib.sha256(
            (ROOT / "training-data.json").read_bytes()
        ).hexdigest(),
        "training_pool_episodes": len(data) - 48,
        "steps": a.steps,
        "batch": a.batch,
        "events_per_episode": 48,
        "accounts_per_episode": 24,
        "learning_rate": 0.009,
        "last_20_steps_mean_nll": float(np.mean(losslog[-20:])),
        "heldout_nll_nats_per_request": float(val.d),
        "notice": "Synthetic predictive loss; one model initialization; no fraud labels or reports enter training.",
    }
    weights = {n: p.d.tolist() for n, p in model.p.items()}
    supervised = fit_supervised_head(model, labeled) if labeled is not None else None
    payload = {
        "version": 3,
        "id": design,
        "adapter": "categorical_graph",
        ...
        "parameters": sum(p.d.size for p in model.p.values()),
    ...
    if supervised is not None:
        ...
        (ROOT / "parity-supervised" / f"{design}.json").write_text(
            json.dumps(
                {
                    "episode": episode,
                    "expected": supervised_trace(model, supervised, episode),
                },
                separators=(",", ":"),
            )
        )
    (ROOT / "parity" / f"{design}.json").write_text(
        json.dumps({"episode": episode, "expected": trace}, separators=(",", ":"))
    )
    if design == CATALOG["default"]:
        (ROOT / "model.json").write_text(json.dumps(payload, separators=(",", ":")))
        (ROOT / "training-summary.json").write_text(json.dumps(summary, indent=2))
        (ROOT / "parity-input.json").write_text(json.dumps(episode))
        (ROOT / "parity-expected.json").write_text(json.dumps(trace))
    print(
        design,
...
def supervised_row(record):
    ...
    z = record["embedding"][0]
    return np.asarray(
        z[query["u"]] + z[query["v"]] + query["context"] + [record["scores"][0]],
        dtype=float,
    )
    ...
    for episode_index, episode in enumerate(dataset["episodes"]):
        ...
        for index, e in enumerate(episode["events"]):
            if e["kind"] == "payment" and int(e.get("label", -1)) >= 0:
                rows.append(supervised_row(trace[index]))
...
def fit_supervised_head(model, dataset, steps=220, learning_rate=0.08):
    X, y, episode_ids = supervised_rows(model, dataset)
...
def supervised_probability(head, row):
    x = (np.asarray(row, dtype=float) - np.asarray(head["mean"])) / np.asarray(
        head["scale"]
    )
    return float(_sigmoid(float(np.dot(x, np.asarray(head["weights"])) + head["bias"])))
...
def supervised_trace(model, head, episode):
    ...
    for event, record in zip(episode["events"], trace):
        if event["kind"] == "payment":
            p = supervised_probability(head, supervised_row(record))
...
def Model(seed=512, design="gru_attention"):
    ...
    return load_module(model_entry(design)["python_module"]).Model(
