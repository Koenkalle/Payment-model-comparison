...
class TimeEncoder(nn.Module):
    ...
    def __init__(self, time_dim, parameter_requires_grad=True):
        ...
        self.w.weight = nn.Parameter(
            ...
            requires_grad=parameter_requires_grad,
        )
        self.w.bias = nn.Parameter(
            torch.zeros(time_dim), requires_grad=parameter_requires_grad
        )
...
class TRCMemory(nn.Module):
    ...
    def __init__(self, dim, device="cpu"):
        ...
        self.register_buffer(
            "PAD_ZERO", torch.zeros(dim, device=device), persistent=False
        )
        self.most_recent_hist_emb = {}

    def get_memories(self, keys):
        return [
            self.most_recent_hist_emb.get((int(u), int(v)), self.PAD_ZERO).to(
                self.PAD_ZERO.device
            )
            for u, v in keys
        ]
...
class HistoricalDecoder(nn.Module):
    ...
    def __init__(
        self, input_dim1, input_dim2, hidden_dim, output_dim=1, device="cpu", gamma=0.9
    ):
        super().__init__()
    ...
    def score(self, src_ids, dst_ids, src_emb, dst_emb):
        ...
        previous = torch.stack(
            self.historical_interaction_memory.get_memories(zip(src_ids, dst_ids))
        )
    ...
    def forward(self, src_ids, dst_ids, src_emb, dst_emb, update_memories=False):
        ...
        if update_memories:
            self.historical_interaction_memory.update_memories(
                zip(src_ids, dst_ids), proposed
            )
        return logits
