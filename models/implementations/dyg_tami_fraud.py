...
FRAUD_DEFAULTS = dict(
    ...
    class_weight=None,
    training_label_cutoff=None,
    validation_label_cutoff=None,
)
ARCHITECTURE = (
    "time_feat_dim",
...
def _known(dataset, indices, cutoff):
    ...
    if np.isinf(availability).any():
        raise ValueError(
            "Label availability must be finite or NaN for retrospective outcomes."
        )
    return (labels[indices] >= 0) & (
        np.isnan(availability[indices]) | (availability[indices] <= cutoff)
    )
...
class Model(LinkModel):
    ...
    def _initialize_fraud(
        self, graph, parameters, pretrained_model=None, *, stored_features=None
    ):
        unknown = set(parameters) - set(DEFAULTS) - set(FRAUD_DEFAULTS)
        if unknown:
            raise ValueError(
                "Unknown fraud-training parameters: " + ", ".join(sorted(unknown))
            )
        settings = {
            **FRAUD_DEFAULTS,
            **{
                key: value for key, value in parameters.items() if key in FRAUD_DEFAULTS
            },
        }
        ...
        if (
            ...
            or set(head) - {"id", "parameters"}
            or "id" not in head
        ):
            raise ValueError(
                "prediction_head must specify a registered id and optional parameters."
            )
        ...
        if pretrained_model is not None:
            ...
            for key in ARCHITECTURE:
                if key in base and base[key] != pretrained_model.parameters[key]:
                    raise ValueError(
                        "Pretrained encoder architecture differs for " + key + "."
                    )
            ...
            base = {
                **{key: pretrained_model.parameters[key] for key in ARCHITECTURE},
                **base,
            }
            if pretrained_model.feature_schema != {
                "node": list(graph.node_feature_names),
                "edge": list(graph.edge_feature_names),
            }:
                raise ValueError("Pretrained encoder feature schema is incompatible.")
        super()._initialize(graph, base)
        if pretrained_model is not None:
            self.network.load_state_dict(
                pretrained_model.network.state_dict(), strict=True
            )
        ...
        self.uses_dataset_features = (
            bool(graph.provenance.get("features_stored"))
            if stored_features is None
            else stored_features
        )
        candidate_count = (
            len(graph.edge_feature_names) if self.uses_dataset_features else 1
        )
        self.network.append(
            create_learned_head(
                self.head_id, 2 * dim + candidate_count, head.get("parameters")
            ).to(self.parameters["device"])
        )
        representation_features = [
            "current_interaction_" + str(i) for i in range(dim)
        ...
        if self.uses_dataset_features:
            ...
            self.head_input_schema = {
                ...
                + ["normalized_candidate_" + name for name in graph.edge_feature_names],
            ...
            self.head_input_schema = {
                ...
                "amount_unit": "EUR",
                "amount_feature": "log1p_amount",
            }
        self.initialization = {
            "method": "pretrained-link-model"
            if pretrained_model is not None
            else "random"
        }
    ...
    def _amount_column(self, graph):
        if "log1p_amount" not in graph.edge_feature_names:
            raise ValueError(
                "Fraud head requires the canonical log1p_amount payment feature."
            )
    ...
    def transaction_representation(self, dataset, group):
        ...
        if self.uses_dataset_features:
            ...
            normalized = (values - np.asarray(normalization["mean"])) / np.asarray(
                normalization["scale"]
            )
            candidates = torch.as_tensor(
                normalized, dtype=current.dtype, device=current.device
            )
        else:
            amounts = graph.edge_features[
                np.asarray(group) + 1, self._amount_column(graph)
            ]
            normalized = (
                amounts - self.amount_normalization["mean"]
            ) / self.amount_normalization["scale"]
            candidates = torch.as_tensor(
                normalized, dtype=current.dtype, device=current.device
            ).reshape(-1, 1)
        ...
        return TransactionRepresentation(
            torch.cat([current, previous, candidates], dim=1),
    ...
    def score_group(self, dataset, group, negative_destinations=None):
        if negative_destinations is not None:
            raise ValueError(
                "Fraud classification does not use sampled negative links."
            )
    ...
    def fit_fraud(
        self, dataset, training, validation, parameters, *, pretrained_model=None
    ):
        graph = dataset.graph
        ...
        if graph.times[training].max() >= graph.times[validation].min():
            raise ValueError(
                "Fraud fitting needs strictly separated train and validation intervals."
            )
        ...
        for name, indices in (("training", training), ("validation", validation)):
            ...
            counts = {
                str(value): int(np.count_nonzero(known_labels == value))
                for value in (0, 1)
            }
            if not counts["0"] or not counts["1"]:
                raise ValueError(
                    f"{name.capitalize()} requires both confirmed classes after label availability cutoff; "
        ...
        if self.uses_dataset_features:
            ...
            self.feature_normalization = {
                "features": list(graph.edge_feature_names),
            ...
            amounts = np.asarray(
                graph.edge_features[training + 1, self._amount_column(graph)],
                dtype=np.float64,
            )
            if not np.isfinite(amounts).all():
                raise ValueError("Training payment amounts must be finite.")
            self.amount_normalization = {
                "feature": "log1p_amount",
                "mean": float(amounts.mean()),
                "scale": max(float(amounts.std()), 1e-6),
                "fitted_rows": len(training),
            }
        ...
        if weight == "balanced":
            weight = float(
                np.count_nonzero(known_labels == 0)
                / np.count_nonzero(known_labels == 1)
            )
        elif weight is None:
            weight = 1.0
        if (
            ...
            raise ValueError(
                "class_weight must be null, balanced or a positive fraud weight."
            )
        self.positive_class_weight = float(weight)
        self.label_counts = {
            "legitimate": int(np.count_nonzero(known_labels == 0)),
            "fraud": int(np.count_nonzero(known_labels == 1)),
            "unknown_or_unavailable": int(len(training) - len(known_labels)),
        }
        trainable = [
            value for value in self.network.parameters() if value.requires_grad
        ]
        optimizer = torch.optim.Adam(
            trainable, lr=p["learning_rate"], weight_decay=p["weight_decay"]
        )
        loss_function = nn.BCEWithLogitsLoss(
            reduction="sum", pos_weight=torch.tensor(weight, device=p["device"])
        )
        training_targets = {
            int(index): float(dataset.labels[index])
            for index in training[masks["training"]]
        }
        ...
        for epoch in range(p["epochs"]):
            ...
            for group in timestamp_groups(graph, prefix):
                offsets = [
                    ...
                    if int(index) in training_targets
                ]
                with torch.set_grad_enabled(bool(offsets)):
                    ...
                    if offsets:
                        targets = torch.tensor(
                            [
                                training_targets[int(group[offset])]
                        ...
                        if not torch.isfinite(loss):
                            raise ValueError(
                                "Fraud training produced a nonfinite loss."
                            )
                        loss.backward()
            ...
            if pending:
                step()
            prediction = self.predict_fraud(graph, validation)["fraud_logits"][
                masks["validation"]
            ]
            ...
            if improved:
                ...
                stale += 1
            LOGGER.info(
                "Fraud epoch %d/%d: train_loss=%.6f validation_loss=%.6f%s epoch=%.1fs total=%.1fs",
                epoch + 1,
                p["epochs"],
                total / count,
                val_loss,
            ...
            if stale >= p["patience"]:
                LOGGER.info(
                    "Fraud training stopped early after epoch %d; best epoch was %d.",
                    epoch + 1,
                    self.best_epoch,
                )
                break
        ...
        LOGGER.info(
            "Fraud training complete: best_epoch=%d best_validation_loss=%.6f elapsed=%.1fs",
            self.best_epoch,
            best_loss,
            time.perf_counter() - started,
        )
    ...
    def load_fraud(self, path, dataset):
        ...
        with np.load(path, allow_pickle=False) as saved:
            ...
            normalization_key = (
                "feature_normalization" if stored_features else "amount_normalization"
            )
            required = {
                "version",
                "task",
            ...
            if (
                ...
                or metadata["task"] != "temporal-fraud-classification"
            ):
            ...
            if schema != metadata["features"]:
                raise ValueError(
                    "Graph feature names or order differ from the fitted fraud model."
                )
            self._initialize_fraud(
                graph,
                {**metadata["parameters"], "device": "cpu"},
            ...
            try:
                ...
                raise ValueError(
                    "Fraud experiment metadata is not finite JSON."
                ) from error
            if (
                self.head.descriptor() != metadata["head"]
                or self.head_input_schema != metadata["head_input_schema"]
            ):
                raise ValueError(
                    "Fraud checkpoint has an incompatible head input contract."
                )
            expected = set(self.network.state_dict())
            if set(saved.files) != expected | {"metadata"}:
                raise ValueError(
                    "Fraud checkpoint has missing or unexpected tensor weights."
                )
            ...
            try:
                ...
                raise ValueError(
                    "Fraud checkpoint tensor dimensions are incompatible."
                ) from error
            ...
            if stored_features:
                ...
                if (
                    ...
                    or set(normalization) != {"feature", "mean", "scale", "fitted_rows"}
                    or normalization["feature"] != "log1p_amount"
                    or any(
                        type(normalization[key]) not in (int, float)
                        or not np.isfinite(normalization[key])
                        for key in ("mean", "scale")
                    )
                    or normalization["scale"] <= 0
                    or type(normalization["fitted_rows"]) is not int
                    or normalization["fitted_rows"] < 1
                ):
                    raise ValueError(
                        "Fraud checkpoint amount normalization is invalid."
                    )
    ...
    def _validate_feature_normalization(normalization, graph):
        ...
        for key in ("mean", "scale"):
            ...
            if (
                ...
                or len(values) != len(graph.edge_feature_names)
                or any(
