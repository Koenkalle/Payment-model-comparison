...
SEQ_VARIANTS = {
    ...
    "tami": {
        "use_pair": True,
        "use_gnn": False,
        "recipient_pair_weight": 1.2,
        "recipient_activity_weight": 0.2,
        "recipient_temporal_weight": 0.25,
        "recipient_pair_state_weight": 1.0,
        "recipient_graph_weight": 0.0,
    },
    "dyg_tami": {
        "use_pair": True,
        "use_gnn": False,
        "recipient_pair_weight": 0.8,
        "recipient_activity_weight": 0.2,
        "recipient_temporal_weight": 0.8,
        "recipient_pair_state_weight": 1.0,
        "recipient_graph_weight": 0.0,
    },
    "dyg_tami_gnn": {
...
def ensure_labeled_data(flags, seed):
    path = ROOT / "xgb-training.json"
    expected = {"version": 3, "flags_requested": int(flags), "seed": int(seed)}
    if path.exists():
        try:
            ...
            pass
    script = (
        "const fs=require('fs'),s=require('./shared/runtime/scenarios');fs.writeFileSync('xgb-training.json',JSON.stringify(s.trainingLabeled(%d,%d)));"
        % (int(flags), int(seed))
    )
