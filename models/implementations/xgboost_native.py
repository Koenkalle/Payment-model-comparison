...
class Model:
    def __init__(self):
        try:
            ...
            raise RuntimeError(
                "Install the native model dependencies: pip install -r requirements-models.txt"
            ) from error
        self.library = xgboost
        self.library_version = xgboost.__version__
        self.booster = None
    ...
    def fit(self, x, y, feature_names, parameters):
        ...
        if (
            ...
            or params.get("booster", "gbtree") != "gbtree"
        ):
            raise ValueError("This plugin supports binary logistic tree boosting.")
    ...
    def predict(self, x, feature_names, explain=False):
        ...
        margins = self.booster.predict(
            matrix, output_margin=True, validate_features=True
        )
        contributions = (
        ...
        if contributions is not None and not np.allclose(
            contributions.sum(axis=1), margins, rtol=1e-5, atol=1e-5
        ):
            raise ValueError("Native feature contributions do not sum to the margin.")
    ...
    def load(self, path, feature_names):
        self.booster = self.library.Booster()
        self.booster.load_model(path)
        if self.booster.feature_names != list(feature_names):
            raise ValueError(
                "Native checkpoint feature order differs from artifact metadata."
            )
