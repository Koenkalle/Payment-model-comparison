...
def _handbook(row, generated_id):
    ...
    entities = (EntityRef('customer', _identity(row['CUSTOMER_ID'], 'CUSTOMER_ID'), 'source'),
                EntityRef('terminal', _identity(row['TERMINAL_ID'], 'TERMINAL_ID'), 'destination'))
    event = TransactionEvent(identifier, time, 'payment', entities,
                             {'amount': _number(row['TX_AMOUNT'], 'TX_AMOUNT', nonnegative=True)})
    used = {'TRANSACTION_ID', 'TX_TIME_SECONDS', 'CUSTOMER_ID', 'TERMINAL_ID', 'TX_AMOUNT', 'TX_FRAUD'}
    diagnostics = {key: value for key, value in row.items() if key not in used}
    return event, Outcome(identifier, _label(row.get('TX_FRAUD')), diagnostics=diagnostics)
...
def _ulb(row, generated_id):
    ...
    features.update({f'V{index}': _number(row[f'V{index}'], f'V{index}') for index in range(1, 29)})
    used = {'Time', 'Amount', 'Class', *[f'V{index}' for index in range(1, 29)]}
    return (TransactionEvent(generated_id, time, 'payment', (), features),
            Outcome(generated_id, _label(row.get('Class')),
                    diagnostics={key: value for key, value in row.items() if key not in used}))
...
def _paysim(row, generated_id):
    ...
    features = {'amount': _number(row['amount'], 'amount', nonnegative=True),
                'oldbalanceOrg': _number(row['oldbalanceOrg'], 'oldbalanceOrg', nonnegative=True),
                'oldbalanceDest': _number(row['oldbalanceDest'], 'oldbalanceDest', nonnegative=True)}
    entities = (EntityRef('account', _identity(row['nameOrig'], 'nameOrig'), 'source'),
                EntityRef('account', _identity(row['nameDest'], 'nameDest'), 'destination'))
    used = {'step', 'type', 'amount', 'nameOrig', 'nameDest', 'oldbalanceOrg', 'oldbalanceDest', 'isFraud'}
    diagnostics = {key: value for key, value in row.items() if key not in used}
...
ADAPTERS = {
    'handbook': {
        'parse': _handbook,
        'required': ('TRANSACTION_ID', 'TX_TIME_SECONDS', 'CUSTOMER_ID', 'TERMINAL_ID', 'TX_AMOUNT'),
        'feature_names': ['amount'], 'entity_types': ['customer', 'terminal'],
        'source_unit': 'seconds', 'source_refs': [HANDBOOK_SOURCE],
    ...
    'ulb': {
        ...
        'entity_types': [], 'source_unit': 'seconds', 'source_refs': [ULB_SOURCE],
        'event_identity': 'source-file-position-and-row',
    },
    'paysim': {
        'parse': _paysim,
        'required': ('step', 'type', 'amount', 'nameOrig', 'nameDest', 'oldbalanceOrg', 'oldbalanceDest'),
        'feature_names': ['amount', 'oldbalanceOrg', 'oldbalanceDest'],
        'entity_types': ['account'], 'source_unit': 'hours', 'source_refs': [PAYSIM_SOURCE],
        'event_identity': 'source-file-position-and-row',
    },
}
...
def source_catalog():
    ...
    return [{'dataset_id': name, 'source_refs': list(adapter['source_refs']),
             'entity_types': list(adapter['entity_types']),
             'feature_names': list(adapter['feature_names'])} for name, adapter in ADAPTERS.items()]
...
def _paths(config, base):
    ...
    if not paths:
        raise ValueError('Source directory contains no CSV files; export Handbook pickle partitions to CSV first.')
    if len(set(paths)) != len(paths):
        raise ValueError('Duplicate source file paths.')
    for path in paths:
        if path.suffix.lower() != '.csv':
            raise ValueError('Source adapters accept CSV files; export pickle partitions to CSV first.')
    return paths
...
def _rows(path, required):
    with path.open(newline='', encoding='utf-8-sig') as source:
        ...
        if not fields or any(not name for name in fields) or len(set(fields)) != len(fields):
            raise ValueError(str(path) + ': CSV requires nonempty, unique column names.')
        missing = set(required) - set(fields)
        if missing:
            raise ValueError(str(path) + ': Missing CSV columns: ' + ', '.join(sorted(missing)))
        for index, row in enumerate(reader, 1):
            if None in row or any(value is None for value in row.values()):
                raise ValueError(f'{path}: row {index} has a different width from the header.')
            yield index, row
...
def _validate_record(event, outcome, descriptor):
    ...
    if isinstance(event.event_time, bool) or not isinstance(event.event_time, (int, float)):
    ...
    if outcome.available_at is not None:
        ...
        if isinstance(outcome.available_at, bool) or not isinstance(outcome.available_at, (int, float)):
    ...
    for name, value in event.features.items():
        if not isinstance(name, str) or not name or isinstance(value, bool) or not isinstance(value, (int, float)):
            raise ValueError('Declared features must have nonempty names and finite numeric values.')
        _number(value, name)
    roles = []
    for entity in event.entities:
        if not isinstance(entity, EntityRef) or entity.kind not in descriptor['entity_types']:
            raise ValueError('Event entities must use declared entity types.')
        if not isinstance(entity.id, str) or not entity.id.strip() or not isinstance(entity.role, str) or not entity.role:
            raise ValueError('Entity identities and roles must be nonempty strings.')
        roles.append(entity.role)
    graph = descriptor['graph']
    if graph and (roles.count(graph['source_role']) != 1 or roles.count(graph['destination_role']) != 1):
        raise ValueError('Graph events require exactly one source and one destination role.')
    if not graph and roles:
        raise ValueError('An adapter without graph entities cannot emit entity references.')
...
def _prepare(config, base, output, *, temporary):
    ...
    if dataset_id not in ADAPTERS:
        raise ValueError('Unknown source dataset: ' + str(dataset_id) + '; choose ' + ', '.join(ADAPTERS))
    ...
    if currency is not None and (not isinstance(currency, str) or not currency.strip()):
        raise ValueError('currency must be a nonempty explicit unit or null for unknown.')
    hashes = [_digest(path) for path in paths]
    source_hash = hashes[0] if len(hashes) == 1 else hashlib.sha256(json.dumps(hashes).encode()).hexdigest()
    configuration = json.loads(json.dumps(config))
    descriptor = {
        'dataset_id': dataset_id, 'version': str(config.get('release', 'unspecified')),
        'name': config.get('name', dataset_id), 'adapter_version': ADAPTER_VERSION,
        'clock': {'kind': 'relative', 'unit': 'seconds', 'source_unit': adapter['source_unit'], 'timezone': None},
        'entity_types': list(adapter['entity_types']), 'feature_names': list(adapter['feature_names']),
        'graph': {'source_role': 'source', 'destination_role': 'destination'} if adapter['entity_types'] else None,
        'currency': currency, 'label_availability': 'retrospective',
        'label_semantics': {'fraud': 1, 'legitimate': 0, 'unknown': -1},
        'event_identity': adapter['event_identity'],
        'ordering': {'primary': 'event_time', 'ties': 'source-file-order-then-row', 'source_order_is_causal': False},
        'source_refs': list(adapter['source_refs']),
    }
    provenance = {
        'loader': config.get('loader', 'transaction_source'), 'dataset_id': dataset_id,
        'source_name': paths[0].name if len(paths) == 1 else f'{dataset_id} ({len(paths)} CSV files)',
        'source_sha256': source_hash, 'configuration': configuration,
        'configuration_sha256': hashlib.sha256(json.dumps(configuration, sort_keys=True).encode()).hexdigest(),
        'adapter_version': ADAPTER_VERSION, 'source_refs': list(adapter['source_refs']),
        'source_files': [{'name': path.name, 'sha256': digest} for path, digest in zip(paths, hashes)],
        'origin': 'user-supplied', 'dataset_descriptor': descriptor,
    }
    ...
    try:
        ...
        raise ValueError('Prepared dataset output already exists: ' + str(output)) from None
    database = None
    try:
            ...
            for row_number, row in _rows(path, adapter['required']):
                ...
                try:
                    ...
                    database.execute('INSERT INTO events VALUES (?,?,?,?,?,?)', (
                        event.event_id, event.event_time, event.event_type,
                        json.dumps([asdict(entity) for entity in event.entities]),
                        json.dumps(_plain(event.features), allow_nan=False), sequence))
                    database.execute('INSERT INTO outcomes VALUES (?,?,?,?)', (
                        outcome.event_id, outcome.label, outcome.available_at,
                        json.dumps(_plain(outcome.diagnostics), allow_nan=False)))
                except sqlite3.IntegrityError as error:
                    raise ValueError(f'{path}: row {row_number}: duplicate event ID {event.event_id!r}.') from error
                except ValueError as error:
                    raise ValueError(f'{path}: row {row_number}: {error}') from error
                sequence += 1
                known_labels += int(outcome.label >= 0)
                recorded_labels += int(outcome.label >= 0 and outcome.available_at is not None)
        ...
        descriptor['label_availability'] = ('as-of' if known_labels and recorded_labels == known_labels
                                            else 'mixed' if recorded_labels else 'retrospective')
        database.execute('CREATE INDEX chronological_events ON events(event_time,arrival_seq)')
        database.executemany('INSERT INTO metadata VALUES (?,?)', [
            ('schema', json.dumps(SCHEMA)), ('descriptor', json.dumps(descriptor)),
            ('provenance', json.dumps(provenance))])
        database.commit()
        database.close()
        database = None
        return TransactionStream(output, temporary=True) if temporary else open_prepared(output)
