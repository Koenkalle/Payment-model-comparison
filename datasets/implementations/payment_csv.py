...
def load(config, base):
    ...
    if any(key not in columns for key in required):
        raise ValueError("columns must map id, time, sender, recipient and amount.")
    if config.get("currency", "EUR") != "EUR":
        raise ValueError(
            "Explicit currency conversion is required before loading non-EUR payments."
        )
    data = rows(path, list(columns.values()))
    accounts = []
    mapping = {}
    events = []
    truth = {}
    ...
    def account(value):
        ...
        if value not in mapping:
            mapping[value] = len(accounts)
            accounts.append({"id": mapping[value], "external_id": value, "name": value})
    ...
    for row, time in zip(data, times):
        ...
        event = {
            "id": row[columns["id"]].strip(),
            "kind": kind,
            "t": (time - origin) / 60,
            "u": account(row[columns["sender"]]) if kind == "payment" else -1,
            "v": account(row[columns["recipient"]]),
            "amount": number(row[columns["amount"]], "amount"),
        }
        if "settled" in columns:
            value = row[columns["settled"]].strip().lower()
            if value not in ("true", "false", "0", "1"):
                raise ValueError("settled CSV values must be true/false or 1/0.")
            event["settled"] = value in ("true", "1")
        if "reference" in columns:
            event["reference"] = row[columns["reference"]]
        if "label" in columns and kind == "payment":
            outcome = label(row[columns["label"]], config.get("label_values"))
            if outcome >= 0:
                truth[event["id"]] = bool(outcome)
                if (
                    "label_available_at" in columns
                    and row[columns["label_available_at"]].strip()
                ):
                    availability[event["id"]] = (
                        timestamp(
                            row[columns["label_available_at"]],
                            config.get("time_unit", "seconds"),
                        )
                        - origin
                    ) / 60
        events.append(event)
    ...
    metadata["time_origin_unix_or_numeric_seconds"] = origin
    raw = {
    ...
    if availability:
        raw["label_available_at"] = availability
