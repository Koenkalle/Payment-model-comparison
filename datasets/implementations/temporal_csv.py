...
def graph_dataset(ids, times, sources, destinations, features, feature_names, metadata,
                  node_dimension=32, raw_nodes=None, raw_edges=None, edge_indices=None):
    ...
    identities = tuple(dict.fromkeys([str(v) for pair in zip(sources, destinations) for v in pair]))
    ...
    times = times - origin
    metadata = {**metadata, 'time_origin_seconds': origin, 'task': 'dynamic-link-prediction'}
    order = np.argsort(times, kind='stable')
    source_ids = np.asarray([mapping[str(v)] for v in sources], dtype=np.int64)[order]
    destination_ids = np.asarray([mapping[str(v)] for v in destinations], dtype=np.int64)[order]
    if raw_nodes is None:
        ...
        try:
            ...
            raise ValueError('NumPy node features require integer source IDs indexing their rows.') from None
        if min(native_ids) < 0 or max(native_ids) >= len(raw_nodes):
            raise ValueError('Node feature array does not cover the CSV node IDs.')
        node_features = np.vstack([np.zeros((1, raw_nodes.shape[1])), raw_nodes[native_ids]]).astype(np.float32)
    ...
    edge_features = np.vstack([np.zeros((1, values.shape[1])), values]).astype(np.float32)
    sorted_times = times[order]
    for array in (sorted_times, source_ids, destination_ids, node_features, edge_features):
        if not np.isfinite(array).all():
            raise ValueError('Graph features and times must be finite.')
        array.setflags(write=False)
    return TemporalGraphDataset(tuple(ids[i] for i in order), sorted_times, source_ids, destination_ids,
                                node_ids, node_features, edge_features, node_names, tuple(feature_names), metadata)


def load(config, base):
    path = source(config, base)
    columns = config.get('columns', {'id': 'idx', 'source': 'u', 'destination': 'i', 'time': 'ts'})
    ...
    if len(set(features)) != len(features) or set(features) & set(columns.values()):
        raise ValueError('Feature columns must be distinct from mapped identities, times and labels.')
    ...
    for key in ('path', 'node_features_path', 'edge_features_path'):
        if key in canonical:
            ...
            canonical[key] = str((filename if filename.is_absolute() else base / filename).resolve())
    ...
    def matrix(key):
        ...
        if values.ndim != 2 or min(values.shape) < 1 or not np.issubdtype(values.dtype, np.number) or not np.isfinite(values).all():
    ...
    try:
        edge_indices = [int(row[columns.get('edge_index', columns['id'])]) for row in data] if raw_edges is not None else None
    except ValueError:
        raise ValueError('NumPy edge features require integer edge indices.') from None
    return graph_dataset(
        [row[columns['id']].strip() for row in data],
        [timestamp(row[columns['time']], config.get('time_unit', 'seconds')) for row in data],
        [row[columns['source']].strip() for row in data],
        [row[columns['destination']].strip() for row in data],
        [[number(row[f], f) for f in features] for row in data], features, metadata,
        config.get('node_feature_dim', 32), raw_nodes, raw_edges, edge_indices)
