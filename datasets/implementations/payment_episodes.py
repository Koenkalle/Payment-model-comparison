...
def load(config, base):
    ...
    if not isinstance(episodes, list) or not episodes:
        raise ValueError(
            "payment_episodes requires a nonempty list of event-provider configurations."
        )
    ...
    offset, sources, boundaries = 0.0, [], []
    for index, provider in enumerate(episodes):
        if not isinstance(provider, dict):
            raise ValueError(
                "Each episode requires a payment-event provider configuration."
            )
        ...
        if not isinstance(dataset, EventDataset):
            if hasattr(dataset, "close"):
                dataset.close()
            raise ValueError(
                'Each episode requires a payment-events/v1 provider; choose view="payments" for fraud datasets.'
            )
        ...
        document["truth"].update(
            {prefix + key: value for key, value in episode["truth"].items()}
        )
        document["label_available_at"].update(
            {
                prefix + key: value + shift
                for key, value in episode.get("label_available_at", {}).items()
            }
        )
        boundaries.append(
            {
    ...
    metadata = {
        ...
        "origin": "synthetic" if synthetic else "composed",
        "configuration": copy.deepcopy(config),
        "episodes": sources,
        "episode_bounds": boundaries,
    }
