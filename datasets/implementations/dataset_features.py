...
def load(config, base):
    ...
    numeric = read_numeric(
        path if path.is_absolute() else base / path, config.get("payload_sha256")
    )
    if list(numeric.feature_names) != config.get("feature_names"):
        raise ValueError(
            "Stored feature columns do not match the dataset configuration."
        )
    metadata = {
        **numeric.provenance,
        "feature_recipe_version": config["recipe_version"],
        "dataset_feature_fingerprint": config["fingerprint"],
    ...
    order = np.asarray(
        [mapping[identifier] for identifier in graph.ids], dtype=np.int64
    )
    origin = graph.provenance.get("time_origin_seconds", 0.0)
    ...
    with np.errstate(over="ignore"):
        edges = np.vstack(
            [np.zeros((1, len(numeric.feature_names))), numeric.features[order]]
        ).astype(graph.edge_features.dtype)
    if not np.isfinite(edges).all():
        raise ValueError(
            "Selected feature values exceed the numeric range supported by graph models."
        )
    edges.setflags(write=False)
    graph = replace(
        graph,
        edge_features=edges,
        edge_feature_names=numeric.feature_names,
