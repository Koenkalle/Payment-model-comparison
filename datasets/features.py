...
class AccountHistory:
    ...
    recent_out_value: float = 0.0
    recent_in_value: float = 0.0
...
class PaymentHistory:
    ...
    def advance(self, time):
        ...
        while self.recent and self.recent[0].time < lower:
            ...
            source, destination = (
                self.accounts[event.source],
                self.accounts[event.destination],
            )
            ...
            if not source.recent_out_count:
                source.recent_out_value = 0.0
            if not destination.recent_in_count:
                destination.recent_in_value = 0.0
...
def _context(event, history, graph_history=None):
    ...
    if event.source is not None and event.destination is not None:
        source, destination = (
            history.accounts[event.source],
            history.accounts[event.destination],
        )
        for prefix, account in (("sender", source), ("recipient", destination)):
            ...
            raw[prefix + "_gap"] = (
                max(0.0, event.time - account.last) / 60 if account.seen else 0.0
            )
        ...
        raw.update(
            pair_out_count=directed,
            pair_total_count=total,
            prior_contact=float(total > 0),
            amount_vs_sender_out_mean=event.amount
            / max(1.0, source.out_value / max(1, source.out_count)),
            amount_vs_recipient_in_mean=event.amount
            / max(1.0, destination.in_value / max(1, destination.in_count)),
        )
    graph_values = (
        graph_history.values(event.source, event.destination)
...
def _definitions(names, currency, capabilities):
    ...
    for name in names:
        ...
        if name == "log1p_amount":
            definitions[-1].update(
                description="Natural logarithm of one plus the source amount.",
    ...
    for recipe in FEATURE_RECIPES:
        ...
        if identifier in identifiers:
            raise ValueError(
                "Feature recipes must have unique IDs distinct from source columns: "
                + identifier
            )
        ...
        definition.update(
            ...
            unavailable_reason=(
                "Requires "
                + ", ".join(sorted(missing))
                + "; these are not present in this dataset."
            )
            if missing
            else None,
        )
        ...
        if identifier == "amount_bin":
            ...
            definition["description"] += (
                " Fixed edges use "
                + unit
                + "; no currency conversion or fitting is applied."
            )
...
def _materialize(numeric, events, currency, capabilities):
    ...
    available = {
        definition["id"] for definition in definitions if definition["available"]
    }
    recipes = [
        recipe for recipe in FEATURE_RECIPES if recipe.definition["id"] in available
    ]
    ...
    graph_history = (
        GraphFeatureState()
        if any(recipe.definition.get("group") == "graph" for recipe in recipes)
        else None
    )
    produced = 0
        ...
        NumericDataset(
            ...
            (*numeric.feature_names, *(recipe.definition["id"] for recipe in recipes)),
            {
                **numeric.provenance,
                ...
                "feature_history": HISTORY_POLICY,
                ...
                **(
                ...
                "feature_amount_currency": currency,
                "outcomes_used": False,
            },
        ),
        definitions,
...
def prepare_features(config, base=None):
    ...
    if loader == "prepared_fraud":
        ...
        with open_prepared(config["path"]) as stream:
            ...
            added_amount_transform = (
                "amount" in source_config["features"]
                and "log1p_amount" not in source_config["features"]
            )
            ...
            result, definitions = _materialize(
                numeric,
                _prepared_events(stream, source_config),
                stream.descriptor.get("currency"),
                capabilities,
            )
            if added_amount_transform:
                next(
                    definition
                    for definition in definitions
                    if definition["id"] == "log1p_amount"
                ).update(kind="derived", group="payment", requirements=["amount"])
    ...
    if loader == "payment_json":
        ...
        values = np.column_stack(
            (numeric.features, [event["amount"] for event in payments])
        )
        ...
        events = (
            FeatureEvent(
                ...
                event["t"] * 60,
                event["amount"],
                event["u"] if event["u"] >= 0 else None,
                event["v"],
                event["kind"],
                {"amount": event["amount"]},
            )
        ...
        result, definitions = _materialize(
            numeric, events, "EUR", {"amount", "identities", "timestamps"}
        )
        source_info = {
            "log1p_amount": (
                "Natural logarithm of one plus the requested EUR amount.",
                "log1p(amount)",
                "current payment",
            ),
            "prior_source_count": (
                "Strictly earlier observed outgoing payment attempts.",
                "identity",
                "all preceding history",
            ),
            "prior_destination_count": (
                "Strictly earlier observed incoming payment attempts.",
                "identity",
                "all preceding history",
            ),
        }
        ...
        return result, definitions
    raise ValueError(
        "Feature preparation requires a prepared_fraud or payment_json source dataset."
    )
