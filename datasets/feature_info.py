...
_PLACEHOLDERS = {
    'numeric model input', 'numeric model feature',
    ...
    'a stored numeric model input', 'a stored input column in this saved dataset',
}
...
_PAYMENT_DESCRIPTIONS = {feature['id']: feature['description'] for feature in FEATURE_SPECS}
...
_SOURCE_NOTES = {
    ...
    'log1p_amount': ('Log payment amount',
    ...
    'prior_source_count': ('Prior sender payments',
    ...
    'prior_destination_count': ('Prior recipient payments',
    ...
    'oldbalanceOrg': ('Sender balance before payment',
    ...
    'oldbalanceDest': ('Recipient balance before payment',
...
def source_definition(name, currency=None):
    ...
    definition = {'id': name, 'label': label, 'description': description, 'kind': 'source',
                  'group': 'source', 'unit': 'source units', 'transform': 'identity',
                  'window': 'source row', 'requirements': [], 'version': '1', 'available': True}
...
def _source_note(name):
    ...
    return (name.replace('_', ' '), f'Source column “{name}”, preserved exactly as supplied. Its meaning and units must be read from the source schema.',
...
def _interpretation(feature):
    ...
    if name in ('log_amount', 'log1p_amount'):
        return ('A larger value means a larger requested amount. The logarithm compresses very large '
                'payments; this is an amount input, not a model probability. Read the calculation for its scale.')
    if name == 'amount_bin':
        return ('Crossing a fixed bin edge increases the bucket. Amounts within a bin have the same '
    ...
    if name.startswith('graph_'):
        if 'error_bound' in name:
            return ('A smaller bound means greater numerical certainty. A large bound calls for caution '
        ...
        if 'edge_lag' in name:
            return ('Zero means the PageRank snapshot includes all historical directed relationships. '
                    'A positive value counts newly observed relationships waiting for the next refresh, not elapsed time.')
        if 'pagerank' in name:
            return ('PageRank measures global structural importance: incoming relationships from important '
        ...
        if 'ppr_' in name:
            return ('Personalized PageRank measures proximity through contact paths from the restart endpoint. '
    ...
    if name.endswith('_gap'):
        return ('A larger value means a longer pause since earlier activity. Zero can mean no prior '
                'activity or a zero gap, so use it with an activity count to distinguish those cases.')
    if name.endswith('_seen'):
        return ('This measures how much observed account activity is available, including payments and '
    ...
    if name.endswith(('_count', '_value')):
        ...
        window = ('the previous 60 minutes, which can highlight a burst of activity' if '_recent_' in name
                  else 'all available preceding history, which reflects accumulated activity')
        return (f'A larger value means a larger {measure} over {window}. Incoming and outgoing refer '
    ...
    return ('Read this value using the supplied calculation, units and history window. Its magnitude is '
...
def describe_feature(definition):
    ...
    if result.get('kind') == 'source' and _placeholder(description):
    ...
    elif (result.get('kind') != 'source' and result['id'] in _PAYMENT_INTRODUCTIONS
        ...
        description = (f'Stored feature “{result["id"]}”. Read its saved calculation and history '
    ...
    if result.get('kind') != 'source':
        notes['sections'].append({'title': 'Using this input', 'text':
            'Enable or disable this column independently when saving a feature dataset. Its stored '
    ...
    if (_placeholder(supplied.get('description'))
            or supplied.get('description') == result.get('description') and description != result.get('description')):
