    ...
    with load_dataset({**source_config, "view": "stream"}, base) as stream:
        ...
        for batch in stream.iter_batches(1024, **_selection(config)):
            ...
            for event in batch:
                ...
                counts["recorded_availability"] += int(
        ...
        if not stream.descriptor.get("graph"):
            views = {
                ...
                if key in ("numeric", "stream")
            }
        models = {}
        for view, schema in views.items():
            models[view] = [
                model["id"]
                for model in manifest("models")["models"]
                if "python" in model["execution"]
                and (
                    schema in model["inputs"]
                    or any(
                        schema in task.get("inputs", [])
        ...
        return {
            ...
            "label_counts": counts,
            "time_range_seconds": [first, last],
            "views": views,
            "models": models,
            "comparison_conversion_required": stream.descriptor.get("currency")
            != "EUR",
        }
