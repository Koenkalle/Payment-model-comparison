...
def _selection(config):
    ...
    if not isinstance(selection, dict) or set(selection) - {"start", "stop"}:
        raise ValueError(
            "selection accepts start and stop in source seconds (half-open)."
        )
    ...
    if (
        "start" in selection
        and "stop" in selection
        and selection["start"] >= selection["stop"]
    ):
...
def _feature_names(stream, config, default=None):
    ...
    if (
        ...
        raise ValueError(
            "features must list distinct observable numeric feature names."
        )
    permitted = set(available) | ({"log1p_amount"} if "amount" in available else set())
    if set(names) - permitted:
        raise ValueError(
            "Features must be observable inputs; available: "
            + ", ".join(sorted(permitted))
        )
    return tuple(names)


def _values(events, names, factor=1.0):
    def value(event, name):
        raw = (
            event.features["amount"] * factor
            if name in ("amount", "log1p_amount")
            else event.features[name]
        )
        return math.log1p(raw) if name == "log1p_amount" else raw

    values = np.asarray(
        [[value(event, name) for name in names] for event in events], dtype=float
    )
...
def numeric_view(stream, config=None):
    ...
    labels = np.asarray(
        [outcomes[event.event_id].label for event in events], dtype=np.int64
    )
    ...
    return NumericDataset(
        ...
        labels,
        names,
        _metadata(stream, config, "numeric"),
    )
...
def _require_graph(stream):
    ...
    if not graph:
        raise ValueError(
            f"{stream.descriptor['dataset_id']} has no stable relational identities; use view='numeric' or 'stream'."
        )
    return graph
...
def _endpoints(stream, events):
    ...
    for event in events:
        ...
        for role, target in (
            (graph["source_role"], sources),
            (graph["destination_role"], destinations),
        ):
            entity = roles.get(role)
            if entity is None:
                raise ValueError(
                    "Missing required graph endpoint for event " + event.event_id
                )
            target.append(
                json.dumps([*namespace, entity.kind, entity.id], separators=(",", ":"))
            )
...
def graph_view(stream, config=None):
    ...
    return graph_dataset(
        ...
        sources,
        destinations,
        _values(events, names),
        names,
        {
            **_metadata(stream, config, "graph"),
            "outcomes_used": False,
...
def payments_view(stream, config=None):
    ...
    if "features" in config:
        raise ValueError(
            "payments and labeled_graph views use the fixed canonical amount feature; select numeric or graph for configurable features."
        )
    _require_graph(stream)
    ...
    if factor is None:
        if stream.descriptor.get("currency") != "EUR":
            raise ValueError(
                "The comparison uses EUR. Supply an explicit positive amount_to_eur conversion factor, or use numeric/graph/stream views in source units."
            )
        factor = 1.0
    ...
    for event, source, destination in zip(events, sources, destinations):
        ...
        for identity, role in (
            (source, graph["source_role"]),
            (destination, graph["destination_role"]),
        ):
    ...
    origin = 0.0
    metadata = {
        **_metadata(stream, config, "payments"),
        "time_origin_unix_or_numeric_seconds": origin,
        "amount_conversion": {
            "source_currency": stream.descriptor.get("currency"),
            "target_currency": "EUR",
            "factor": factor,
        },
    ...
    document = {
        ...
        "units": {"time": "minutes", "currency": "EUR"},
        "accounts": accounts,
        "events": [
            {
                "id": event.event_id,
                ...
                "u": mapping[source],
                "v": mapping[destination],
                "amount": event.features["amount"] * factor,
            }
            for event, source, destination in zip(events, sources, destinations)
        ],
        "truth": {
            event.event_id: bool(outcomes[event.event_id].label)
            for event in events
            if outcomes[event.event_id].label >= 0
        },
        "label_available_at": {
            event.event_id: (outcomes[event.event_id].available_at - origin) / 60
            for event in events
            if outcomes[event.event_id].label >= 0
...
def project(stream, view, config=None):
    ...
    if view == "stream":
        if config.get("selection"):
            raise ValueError(
                "Use iter_events(start=..., stop=...) to select a stream interval; selection is for materialized views."
            )
        return stream
    if view == "numeric":
    ...
    if view == "labeled_graph":
        ...
        return labeled_payments(
            payments_view(stream, config),
