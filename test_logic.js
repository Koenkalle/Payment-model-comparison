...
const core=require('./model'),sc=require('./scenarios');
...
const data=sc.build('relay','small'),runner=new core.Runner(model,data),prefix=runner.seek(data.startIndex),state=copy(prefix.state),event=data.events[data.startIndex],before=copy(state);
...
const full=runner.seek(data.events.length),first=full.state.decisions.slice(0,128),rest=full.state.decisions.slice(128);
...
const rewind=runner.seek(240),fresh=core.replay(model,data,240);assert.deepStrictEqual(rewind.state,fresh.state);
