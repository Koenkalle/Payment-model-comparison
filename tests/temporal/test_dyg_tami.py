...
class TemporalTests(unittest.TestCase):
    ...
    def test_native_encoder_and_decoder_train_and_save_load(self):
        ...
        model.fit_graph(
            self.dataset, self.parts["train"], self.parts["validation"], PARAMETERS
        )
        final = model.network.state_dict()
        for prefix in (
            "0.time_encoder.",
            "0.neighbor_co_occurrence_encoder.",
            "0.projection_layer.",
            "0.transformers.",
            "0.output_layer.",
            "1.fc1.",
            ...
            self.assertTrue(
                any(
                    not torch.equal(initial[key], final[key])
                    for key in final
        ...
        memory = {
            key: value.clone()
            for key, value in model.memory.most_recent_hist_emb.items()
        }
        ...
        for key in memory:
            torch.testing.assert_close(
                memory[key], restored.memory.most_recent_hist_emb[key]
            )
        actual = restored.predict_graph(self.dataset, self.parts["test"])
        np.testing.assert_allclose(
            expected["positive_logits"], actual["positive_logits"], rtol=0, atol=0
        )
        ...
        np.testing.assert_array_equal(
            first["positive_logits"], again["positive_logits"]
        )
    ...
    def test_causality_unknown_nodes_outcome_and_future_isolation(self):
        ...
        changed = dataclasses.replace(
            self.dataset, edge_features=features, sources=changed_sources
        )
        altered = model.predict_graph(changed, past)
        np.testing.assert_array_equal(
            original["positive_logits"], altered["positive_logits"]
        )
        ...
        self.assertTrue(
            np.isfinite(
                model.predict_graph(self.dataset, self.parts["test"])["positive_logits"]
            ).all()
        )
        ...
        np.testing.assert_array_equal(
            original["positive_logits"],
            model.predict_graph(relabeled, past)["positive_logits"],
        )
    ...
    def test_negative_sampling_and_equal_time_pair_memory(self):
        ...
        for group in timestamp_groups(self.dataset, np.arange(len(self.dataset.ids))):
            ...
            positive = set(
                zip(self.dataset.sources[group], self.dataset.destinations[group])
            )
        ...
        self.assertEqual(
            decoder.historical_interaction_memory.get_memories([(2, 1)])[0].abs().sum(),
            0,
        )
        decoder.historical_interaction_memory.reset_memory()
        decoder.historical_interaction_memory.update_memories(
            zip(u[::-1], v[::-1]), proposed.flip(0)
        )
        torch.testing.assert_close(
            stored, decoder.historical_interaction_memory.most_recent_hist_emb[(1, 2)]
        )
    ...
    def test_experiment_artifact_and_schema_validation(self):
        config = dict(
            model="dyg_tami_native", dataset=self.config, parameters=PARAMETERS
        )
        ...
        with self.assertRaisesRegex(ValueError, "feature"):
            evaluate_artifact(
                artifact, {**self.config, "node_feature_dim": 8}, self.base
            )
        with self.assertRaisesRegex(ValueError, "original graph"):
            evaluate_artifact(
                artifact, {**self.config, "time_unit": "hours"}, self.base, "test"
            )
    ...
    def test_dyglib_indexed_features_and_invalid_data(self):
        ...
        config = dict(
            loader="temporal_csv",
            path="native.csv",
            node_features_path="nodes.npy",
            edge_features_path="edges.npy",
        )
    ...
    def test_test_interval_never_changes_fitted_weights(self):
        ...
        second = self.model()
        second.fit_graph(
            altered, self.parts["train"], self.parts["validation"], parameters
        )
        ...
        for name, parameter in original.network.state_dict().items():
            torch.testing.assert_close(
                parameter, second.network.state_dict()[name], rtol=0, atol=0
            )
    ...
    def test_payment_graph_relative_configs_identify_same_dataset(self):
        ...
        document = {
            "accounts": [{"id": 0}, {"id": 1}],
            "events": [
                {"id": "p", "kind": "payment", "u": 0, "v": 1, "t": 0, "amount": 1}
            ],
            "truth": {"p": True},
        }
        ...
        relative = dict(
            ...
            dataset=dict(loader="payment_json", path="payments.json"),
        )
        absolute = dict(
            loader="payment_graph",
            dataset=dict(loader="payment_json", path=str(self.base / "payments.json")),
        )
        self.assertEqual(
            fingerprint(load_dataset(relative, self.base)),
            fingerprint(load_dataset(absolute, ROOT)),
        )
