...
PARAMETERS = dict(epochs=2, patience=2, batch_size=8, learning_rate=.01,
...
class FraudModelTests(unittest.TestCase):
    def setUp(self):
        ...
        self.graph = graph_dataset([str(i) for i in range(48)], [i // 2 for i in range(48)],
        ...
        labels[3] = -1
        self.dataset = SimpleNamespace(graph=self.graph, labels=labels,
                                       label_available_at=np.full(48, np.nan))
        self.training, self.validation, self.test = np.arange(28), np.arange(28, 36), np.arange(36, 48)
    ...
    def fit(self, mode='finetune', dataset=None, pretrained=None, **extra):
        ...
        model = Model()
        model.fit_fraud(dataset or self.dataset, self.training, self.validation,
                        {**PARAMETERS, 'encoder_training': mode, **extra}, pretrained_model=pretrained)
        return model
    ...
    def test_real_gradients_finetune_and_frozen_encoder(self):
        ...
        before = {key: value.detach().clone() for key, value in initial.network.state_dict().items()}
        frozen = self.fit('frozen', pretrained=initial)
        tuned = self.fit('finetune', pretrained=initial)
        for key, value in before.items():
            torch.testing.assert_close(value, frozen.network.state_dict()[key], rtol=0, atol=0)
            torch.testing.assert_close(value, initial.network.state_dict()[key], rtol=0, atol=0)
        for prefix in ('0.time_encoder.', '0.transformers.', '1.fc1.'):
            self.assertTrue(any(not torch.equal(value, tuned.network.state_dict()[key])
                                for key, value in before.items() if key.startswith(prefix)), prefix)
        self.assertFalse(tuned.network[1].fc2.weight.requires_grad)
        self.assertEqual(frozen.label_counts, {'legitimate': 20, 'fraud': 7, 'unknown_or_unavailable': 1})
        self.assertTrue(all(row['labeled_training_rows'] == 27 for row in frozen.training_history))
        self.assertFalse(torch.equal(frozen.head.linear.weight, tuned.head.linear.weight))
        self.assertFalse(frozen.network[0].training)
        from models.implementations.dyg_tami_fraud import Model
        independent_schedule = Model()
        independent_schedule._initialize_fraud(self.graph, {'encoder_training': 'frozen'}, initial)
        self.assertEqual(independent_schedule.parameters['epochs'], 10)
        self.assertEqual(independent_schedule.parameters['learning_rate'], .0001)
        self.assertEqual(Model.pretraining_parameters({'epochs': 9, 'encoder_training': 'frozen'}), {'epochs': 3})
        self.assertEqual(Model.pretraining_parameters({'epochs': 9}, {'epochs': 5}), {'epochs': 5})
    ...
    def test_training_labels_change_weights_queries_and_test_labels_do_not(self):
        ...
        self.assertFalse(torch.equal(first.head.linear.weight, second.head.linear.weight))
        expected = first.predict_fraud(self.dataset, self.validation)['fraud_logits']
        np.testing.assert_array_equal(expected, first.predict_fraud(relabeled, self.validation)['fraud_logits'])
        heldout = copy.copy(self.dataset)
        heldout.labels = self.dataset.labels.copy()
        heldout.labels[self.test] = 1 - heldout.labels[self.test]
        heldout.graph = dataclasses.replace(self.graph, edge_features=self.graph.edge_features.copy())
        ...
        for key, value in first.network.state_dict().items():
            torch.testing.assert_close(value, third.network.state_dict()[key], rtol=0, atol=0)
    ...
    def test_pure_timestamp_scoring_candidate_amount_and_future_isolation(self):
        ...
        np.testing.assert_array_equal(expected, model.predict_fraud(altered, earlier)['fraud_logits'])
        ...
        with torch.no_grad():
            ...
            self.assertTrue(all(not value.requires_grad for value in model.memory.most_recent_hist_emb.values()))
    ...
    def test_availability_and_class_coverage(self):
        ...
        for indices, stage in ((self.training, 'Training'), (self.validation, 'Validation')):
            missing = copy.copy(self.dataset)
            missing.labels = self.dataset.labels.copy()
            missing.labels[indices] = 0
            with self.assertRaisesRegex(ValueError, stage + ' requires both confirmed classes'):
    ...
    def test_safe_checkpoint_roundtrip_and_missing_head_rejection(self):
        ...
        model.experiment_metadata = {'task': 'temporal-fraud-classification',
        ...
        for key, values in expected.items():
            np.testing.assert_array_equal(values, restored.predict_fraud(self.dataset, self.test)[key])
        with np.load(path, allow_pickle=False) as saved:
            tensors = {key: saved[key].copy() for key in saved.files}
        missing = self.base / 'missing.npz'
        np.savez(missing, **{key: value for key, value in tensors.items() if key != '2.linear.weight'})
    ...
    def test_head_contract_and_stable_fraud_score(self):
        ...
        self.assertEqual([entry['id'] for entry in manifest(kind='transaction-representation')['prediction_heads']], ['fraud_linear'])
        ...
        self.assertEqual(result['fraud_probability'][2], .5)
