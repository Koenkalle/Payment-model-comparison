...
from framework.temporal_fraud_data import (
    labeled_payments,
...
class FraudDataTests(unittest.TestCase):
    ...
    def test_fraction_and_explicit_splits_keep_timestamp_groups_together(self):
        ...
        boundaries = [
            data.times[splits[key][0]]
            for key in ("model_validation", "policy_validation", "test")
        ]
...
class FraudExperimentTests(unittest.TestCase):
    ...
    def test_real_training_roundtrip_relocation_and_artifact_checks(self):
        ...
        evaluated = evaluate_artifact(
            self.base / "artifact", self.config["dataset"], self.base, "test"
        )
    ...
    def test_test_label_changes_leave_fitted_tensors_and_selection_unchanged(self):
        ...
        with np.load(
            self.base / "artifact" / "model.npz", allow_pickle=False
        ) as original, np.load(
            self.base / "changed-labels" / "model.npz", allow_pickle=False
        ) as changed:
        ...
        self.assertEqual(
            [r["score"] for r in first["rows"]], [r["score"] for r in second["rows"]]
        )
        self.assertNotEqual(first["metrics"], second["metrics"])

    def test_missing_class_fails_early_and_missing_policy_classes_has_fixed_default(
        self,
    ):
    ...
    def test_initialization_rejects_mismatched_graph_conversion(self):
        ...
        link_config = {
            ...
            "split": {"train": 0.4, "validation": 0.1},
        }
