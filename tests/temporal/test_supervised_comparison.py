...
class SupervisedComparisonTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.document = {
            "accounts": [
                {"id": i, "external_id": "training-" + str(i)} for i in range(5)
            ],
            "events": [],
            "truth": {},
        }
        ...
        cls.config = {
            ...
            "initialization": {
                "method": "link_pretrain_train_partition",
                "parameters": {"epochs": 1},
            },
    ...
    def test_saved_fraud_logits_and_scores_use_native_model(self):
        ...
        model, _ = create_model(
            "dyg_tami_native",
            "labeled-temporal-graph/v1",
            task="temporal-fraud-classification",
        )
        model.load_graph(self.base / "artifact" / "model.npz", graph)
        expected = model.predict_fraud(graph, np.arange(len(graph.ids)))
        np.testing.assert_array_equal(
            [row["fraud_logit"] for row in actual["predictions"]],
            expected["fraud_logits"],
        )
        np.testing.assert_allclose(
            [row["score"] for row in actual["predictions"]], expected["scores"]
        )
        ...
        for row in actual["predictions"]:
            ...
            self.assertEqual(
                row["decision"] == "BLOCK", row["evidence"]["fraud_probability"] > 0.5
            )
    ...
    def test_identity_describes_loaded_checkpoint_even_if_disk_changes(self):
        ...
        try:
            ...
            self.assertEqual(
                identity["model_sha256"], json.loads(saved)["model_sha256"]
            )
    ...
    def test_target_labels_do_not_change_weights_scores_or_any_policy(self):
        ...
        changed["truth"] = {
            identifier: not label for identifier, label in changed["truth"].items()
        }
    ...
    def test_current_amount_affects_own_score_and_blocks_remove_future_context(self):
        ...
        self.assertNotEqual(
            first["predictions"][0]["logit"], second["predictions"][0]["logit"]
        )
        ...
        allow_all = self.scorer.compare(
            self.target, self.options(mode="enforce", manualTau=100)
        )
        self.assertEqual(
    ...
    def test_original_history_selects_only_test_and_reserved_subsets_are_rejected(self):
        ...
        for policy in ("manual", "tuned", "auto"):
            actual = self.scorer.compare(
                self.document, self.options(decisionPolicy=policy)
            )
