...
class NativeComparisonTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.document = {
            'accounts': [{'id': i, 'external_id': 'historical-' + str(i)} for i in range(5)],
        ...
        graph_config = {'loader': 'payment_graph', 'dataset': {'loader': 'payment_json', 'path': 'payments.json'}, 'node_feature_dim': 4}
        ...
        train_experiment({'model': 'dyg_tami_native', 'dataset': graph_config,
                          'parameters': parameters}, cls.base / 'artifact', cls.base)
        cls.calibration = {'dataset': graph_config, 'max_rows': 3}
        (cls.base / 'calibration.json').write_text(json.dumps(cls.calibration))
        cls.scorer = NativeComparison(cls.base / 'artifact', cls.base / 'calibration.json')
    ...
    def test_true_native_logits_match_model_and_shadow_head_changes_reuse_them(self):
        ...
        scorer = self.scorer
        result = scorer.compare(self.target, {'mode': 'shadow', 'decisionPolicy': 'manual', 'manualTau': 2})
        ...
        expected = native.predict_graph(graph, np.arange(len(graph.ids)))['positive_logits']
        np.testing.assert_array_equal([row['logit'] for row in result['predictions']], expected)
        calls = scorer.inference_calls
        changed = scorer.compare(self.target, {'mode': 'shadow', 'decisionPolicy': 'manual',
                                              'manualTau': .8, 'predictionHead': 'fixed_likelihood'})
        self.assertEqual(scorer.inference_calls, calls)
        self.assertTrue(changed['provenance']['shadow_cache_hit'])
        self.assertEqual([row['logit'] for row in result['predictions']],
                         [row['logit'] for row in changed['predictions']])
        self.assertNotEqual([row['score'] for row in result['predictions']],
    ...
    def test_selected_truth_cannot_change_any_policy_head_or_prediction(self):
        ...
        for mode in ('shadow', 'enforce'):
            for policy in ('shared', 'manual', 'tuned', 'auto'):
                options = {'mode': mode, 'decisionPolicy': policy, 'manualTau': 2, 'warmup': 4}
        ...
        self.assertEqual(self.scorer.describe()['supported_policies'], ['shared', 'manual', 'tuned', 'auto'])
    ...
    def test_enforce_cache_tracks_effective_policy_and_ignores_inactive_controls(self):
        ...
        for policy in ('manual', 'tuned', 'auto'):
            ...
            second = self.scorer.compare(target, {**options, 'alpha': .07, 'warmup': 10, 'eta': .5})
            self.assertEqual(self.scorer.inference_calls, calls)
            self.assertTrue(second['provenance']['enforce_cache_hit'])
            self.assertEqual([row['logit'] for row in first['predictions']], [row['logit'] for row in second['predictions']])
            self.assertEqual([row['decision'] for row in first['predictions']], [row['decision'] for row in second['predictions']])
            self.assertTrue(all(row['evidence']['alpha'] == .07 for row in second['predictions']))
        for options in (
                {'mode': 'enforce', 'decisionPolicy': 'manual', 'manualTau': 1.2},
                {'mode': 'enforce', 'decisionPolicy': 'manual', 'manualTau': 1.2, 'predictionHead': 'fixed_likelihood'},
                {'mode': 'enforce', 'decisionPolicy': 'shared', 'warmup': 4, 'alpha': .1},
                {'mode': 'enforce', 'decisionPolicy': 'shared', 'warmup': 4, 'alpha': .2},
    ...
    def test_enforcement_commits_only_accepted_history(self):
        ...
        import torch
        options = {'mode': 'enforce', 'decisionPolicy': 'shared', 'alpha': .4, 'warmup': 4}
        ...
        with torch.no_grad():
            for group in timestamp_groups(graph, np.arange(len(graph.ids))):
                ...
                kept = [offset for offset, index in enumerate(group) if actual['predictions'][index]['settled']]
                if kept:
        ...
        np.testing.assert_array_equal([row['logit'] for row in actual['predictions']], expected)
        allowed = self.scorer.compare(self.target, {'mode': 'enforce', 'decisionPolicy': 'manual', 'manualTau': 100})
        shadow = self.scorer.compare(self.target, {'mode': 'shadow', 'decisionPolicy': 'manual', 'manualTau': 100})
        self.assertEqual([row['logit'] for row in allowed['predictions']], [row['logit'] for row in shadow['predictions']])
        self.assertNotEqual([row['logit'] for row in actual['predictions']], [row['logit'] for row in shadow['predictions']])
    ...
    def test_blocked_amounts_do_not_enter_future_history_or_same_time_scores(self):
        ...
        self.assertTrue(all(row['decision'] == 'BLOCK' and not row['settled'] for row in first['predictions']))
        ...
        self.assertNotEqual(shadow['predictions'][2:], changed_shadow['predictions'][2:])
    ...
    def test_policies_match_existing_javascript_frontier_and_selection(self):
        ...
        for objective in ('f1', 'f2', 'balanced_accuracy'):
            options = _options({'decisionPolicy': 'tuned', 'falseBlockCost': 3, 'missedFraudCost': 5, 'objective': objective}, self.scorer.heads)
            request = {'rows': [{'score': s, 'label': label} for s, label in zip(scores, labels)], 'options': options}
            raw = subprocess.check_output([node, '-e', script, json.dumps(request)], cwd=ROOT, text=True)
            expected = json.loads(raw)
            self.assertEqual(validation, expected['v'])
            self.assertEqual(select_policy(validation, options), expected['tuned'])
            self.assertEqual(select_policy(validation, {**options, 'decisionPolicy': 'auto'}), expected['auto'])

    def test_relocated_artifact_validates_bytes_graph_and_rejects_historical_reuse(self):
        from framework.native_comparison import NativeComparison
        relocated = self.base / 'relocated'
        relocated.mkdir(exist_ok=True)
        shutil.copytree(self.base / 'artifact', relocated / 'artifact', dirs_exist_ok=True)
        ...
        scorer = NativeComparison(relocated / 'artifact', relocated / 'calibration.json')
        self.assertEqual(scorer.calibration, self.scorer.calibration)
        same = scorer.compare(self.document, {'mode': 'shadow', 'decisionPolicy': 'manual', 'manualTau': 2})
        self.assertTrue(same['provenance']['same_dataset'])
        self.assertEqual(sum(row['decision'] == 'CONTEXT' for row in same['predictions']), 68)
