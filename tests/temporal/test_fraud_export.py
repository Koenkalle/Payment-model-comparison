...
class FraudExportTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.document = {
            "accounts": [
                {"id": i, "external_id": "account-" + str(i)} for i in range(5)
            ],
            "events": [
                {
                    "id": "deposit",
        ...
        cls.document["events"].append(
            {"id": "report", "kind": "report", "u": -1, "v": 2, "t": 40, "amount": 0}
        )
    ...
    def test_registered_benchmark_payment_view_exports(self):
        ...
        self.assertEqual(
            [row["id"] for row in result["predictions"]],
            ["source-a", "source-b", "source-c"],
        )
        self.assertEqual(
            result["dataset"]["truth"], {"source-a": False, "source-b": True}
        )
    ...
    def test_native_scores_heads_and_complete_time_group_calibration(self):
        ...
        self.assertEqual(
            result["calibration"]["ids"], ["payment-" + str(i) for i in range(64, 68)]
        )
        self.assertEqual(
            result["evaluation_ids"], ["payment-" + str(i) for i in range(68, 80)]
        )
        self.assertEqual(
            result["history"],
            {
        ...
        np.testing.assert_array_equal(
            result["heads"]["empirical_tail"]["reference_logits"],
            np.sort(expected[64:68]),
        )
        for identifier, state in result["heads"].items():
            ...
            self.assertEqual(
                result["metrics"][identifier]["known"]
                + result["metrics"][identifier]["unknown"],
                12,
            )
            flags = head.score(actual[68:])["score"] > head.get_threshold(
                result["alpha"]
            )
    ...
    def test_outcomes_do_not_change_model_or_head_and_copies_keep_test_membership(self):
        ...
        relabeled["truth"] = {
            key: not value for key, value in relabeled["truth"].items()
        }
