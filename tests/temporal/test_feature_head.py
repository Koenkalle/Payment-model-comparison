...
class StoredFeatureHeadTests(unittest.TestCase):
    def setUp(self):
        ...
        self.graph = graph_dataset([str(i) for i in range(32)], [i // 2 for i in range(32)],
                                   ['source' + str(i % 2) for i in range(32)],
                                   ['destination' + str(i % 3) for i in range(32)],
                                   [[i % 4, i / 10] for i in range(32)],
                                   ['sender_out_count', 'pair_total_count'], {'features_stored': True}, 4)
        self.training, self.validation, self.test = np.arange(20), np.arange(20, 26), np.arange(26, 32)
        self.dataset = SimpleNamespace(graph=self.graph,
                                       labels=np.asarray([int(i % 4 == 0) for i in range(32)]),
                                       label_available_at=np.full(32, np.nan))
        self.model = Model()
        self.model.fit_fraud(self.dataset, self.training, self.validation,
                             dict(epochs=1, patience=1, batch_size=8, learning_rate=.01,
                                  time_feat_dim=4, channel_embedding_dim=4, num_layers=1,
                                  patch_size=2, max_input_sequence_length=8, dropout=0.,
                                  seed=31, num_threads=1, encoder_training='frozen'))
    ...
    def test_all_candidate_columns_use_training_only_normalization(self):
        ...
        self.assertEqual(self.model.head_input_schema['candidate_features'], list(self.graph.edge_feature_names))
        self.assertEqual(self.model.head.descriptor()['input_contract'], 'tami-payment-representation/v2')
        # Neither padding nor the later validation/test distribution enters fit.
        self.assertNotEqual(normalization['mean'][1], float(self.graph.edge_features[:, 1].mean()))
    ...
    def test_each_selected_column_directly_changes_its_candidate_only(self):
        ...
        with torch.no_grad():
            for column in range(2):
                ...
                self.model.head.linear.weight[0, -2 + column] = 1.
    ...
    def test_v2_roundtrip_uses_saved_contract_and_rejects_corrupt_normalization(self):
        ...
        for name, values in expected.items():
            np.testing.assert_array_equal(values, restored.predict_fraud(self.graph, self.test)[name])
        ...
        self.assertEqual(restored.feature_normalization, metadata['feature_normalization'])
        ...
        with self.assertRaisesRegex(ValueError, 'feature names or order'):
            Model().load_fraud(path, replace(self.graph, edge_feature_names=tuple(reversed(self.graph.edge_feature_names))))
