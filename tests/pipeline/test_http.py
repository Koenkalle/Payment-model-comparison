...
class PipelineHTTPTests(unittest.TestCase):
    ...
    def test_generate_train_compare_and_reopen_over_http(self):
        ...
        status, created = self.request(
            "/datasets/generate",
            {
                ...
                "name": "HTTP benchmark",
                "parameters": {"transactions": 160, "fraud_rate": 0.3, "seed": 42},
            },
        )
        ...
        self.assertEqual(status, 200)
        self.assertTrue(
            next(row for row in models["models"] if row["id"] == "logistic_regression")[
                "available"
            ]
        )
        ...
        status, comparison = self.request(
            "/compare",
            {"dataset_id": identifier, "run_ids": [run["id"]], "partition": "test"},
        )
    ...
    def test_untrusted_or_invalid_requests_do_not_create_datasets(self):
        ...
        status, _ = self.request(
            "/datasets/generate", valid, {"Origin": "http://evil.example"}
        )
    ...
    def test_feature_catalog_and_immutable_selection_over_http(self):
        ...
        status, result = self.request(
            path,
            {"features": ["log_amount", "sender_out_count"], "name": "Shared features"},
        )
        ...
        self.assertEqual(
            detail["sample"]["columns"], ["log_amount", "sender_out_count"]
        )
        ...
        self.assertEqual(
            self.request(path, {"features": ["amount"], "path": "/etc/passwd"})[0], 400
        )
        self.assertEqual(
            self.request(
                path, {"features": ["amount"]}, {"Origin": "http://evil.example"}
            )[0],
            403,
        )
