...
class DatasetStoreTests(unittest.TestCase):
    ...
    def upload(self, **values):
        return self.store.import_source({'source': 'handbook', 'csv': CSV, 'amount_to_eur': 2., **values})
    ...
    def test_fixture_is_stored_and_restart_keeps_ids(self):
        ...
        self.assertEqual(DatasetStore(self.base / 'store').catalog()['datasets'][0]['id'], fixture['id'])

    def test_uploaded_csv_has_durable_numeric_and_payment_views(self):
        metadata = self.upload()
        self.assertEqual((metadata['rows'], metadata['known'], metadata['fraud'], metadata['unknown']), (3, 2, 1, 1))
        ...
        self.assertEqual(numeric.features[:, 0].tolist(), [10., 12., 8.])
        document = self.store.document(metadata['id'])['dataset']
        self.assertEqual([event['amount'] for event in document['events']], [20., 24., 16.])
    ...
    def test_registered_sources_are_snapshots_independent_of_original_files(self):
        ...
        config = {'loader': 'fraud_dataset', 'dataset': 'handbook', 'path': 'source.csv',
                  'name': 'Registered local', 'amount_to_eur': 1., 'selection': {'start': 120}}
        (directory / 'config.json').write_text(json.dumps(config))
        with prepare_source(config, directory, directory / 'source.sqlite'):
            pass
        (directory / 'prepared.json').write_text(json.dumps({'loader': 'prepared_fraud', 'path': 'source.sqlite', 'amount_to_eur': 1.}))
        ...
        metadata = next(item for item in store.catalog()['datasets'] if item['selection'].get('start') == 120)
        self.assertEqual(metadata['rows'], 2)
        (directory / 'source.csv').unlink()
        (directory / 'source.sqlite').unlink()
        store = DatasetStore(self.base / 'configured', paths)
        self.assertEqual(len(store.catalog()['datasets']), 3)
        self.assertEqual(len(load_dataset(store.dataset_config(metadata['id'], 'numeric')).ids), 2)

    def test_registered_payment_selection_and_confirmations_are_snapshotted(self):
        document = {'accounts': [{'id': 0}, {'id': 1}], 'events': [
            {'id': f'p{index}', 'kind': 'payment', 't': index, 'u': 0, 'v': 1, 'amount': index + 2}
            for index in range(4)], 'truth': {f'p{index}': bool(index % 2) for index in range(4)},
            'label_available_at': {'p0': 0, 'p2': 3}}
        (self.base / 'payments.json').write_text(json.dumps(document))
        config = {'loader': 'payment_json', 'path': 'payments.json', 'name': 'Selected payments',
                  'selection': {'start': 60, 'stop': 180}}
        (self.base / 'payments.config.json').write_text(json.dumps(config))
        store = DatasetStore(self.base / 'selected', [self.base / 'payments.config.json'])
        metadata = next(entry for entry in store.catalog()['datasets'] if entry['name'] == 'Selected payments')
        self.assertEqual(metadata['rows'], 2)
        ...
        self.assertEqual(data.times.tolist(), [60., 120.])
        ...
        self.assertEqual(load_dataset(store.dataset_config(metadata['id'], 'numeric')).ids, data.ids)
    ...
    def test_generated_handbook_settings_determinism_and_all_views(self):
        options = {'generator': 'handbook_generator', 'parameters': {'transactions': 120, 'fraud_rate': .2, 'seed': 9}}
        ...
        for view, schema in [('numeric', 'numeric-table/v1'), ('graph', 'temporal-graph/v1'),
                             ('labeled_graph', 'labeled-temporal-graph/v1'), ('payments', 'payment-events/v1')]:
            self.assertEqual(load_dataset(self.store.dataset_config(first['id'], view)).schema, schema)
        changed = self.store.generate({**options, 'parameters': {**options['parameters'], 'seed': 10}})
        self.assertNotEqual(changed['fingerprint'], first['fingerprint'])

    def test_scenario_settings_delays_and_saved_numeric_bridge(self):
        first = self.store.generate({'generator': 'synthetic_payments',
                                     'parameters': {'name': 'relay', 'size': 'small', 'reportDelay': 30, 'forwardDelay': 1}})
        second = self.store.generate({'generator': 'synthetic_payments',
                                      'parameters': {'name': 'relay', 'size': 'small', 'reportDelay': 3000, 'forwardDelay': 10}})
        left = self.store.document(first['id'])['dataset']
        right = self.store.document(second['id'])['dataset']
        self.assertNotEqual([event['t'] for event in left['events'] if event['kind'] == 'report'],
                            [event['t'] for event in right['events'] if event['kind'] == 'report'])
        self.assertTrue(left['label_available_at'])
        self.assertTrue(all(account['external_id'].startswith('synthetic-') for account in left['accounts']))
        self.assertEqual([account['external_id'] for account in left['accounts']],
        ...
        self.assertEqual(data.feature_names, ('log1p_amount', 'prior_source_count', 'prior_destination_count'))
    ...
    def test_fingerprint_ignores_names_and_labels_stay_outside_features(self):
        ...
        self.assertNotEqual(changed['fingerprint'], first['fingerprint'])
        np.testing.assert_array_equal(load_dataset(self.store.dataset_config(first['id'], 'numeric')).features,
    ...
    def test_large_payment_interval_fails_but_numeric_data_and_selected_view_remain(self):
        ...
        self.assertEqual(len(load_dataset(self.store.dataset_config(metadata['id'], 'numeric')).ids), 260)
        with self.assertRaisesRegex(ValueError, '256 accounts'):
            self.store.document(metadata['id'])
        document = self.store.document(metadata['id'], {'start': 100, 'stop': 110})['dataset']
...
class CausalNumericTests(unittest.TestCase):
    def test_ties_and_future_truth_do_not_change_candidate_features(self):
        document = normalize({'accounts': [{'id': 0}, {'id': 1}, {'id': 2}], 'events': [
            {'id': 'a', 'kind': 'payment', 't': 1, 'u': 0, 'v': 1, 'amount': 10},
            {'id': 'b', 'kind': 'payment', 't': 1, 'u': 0, 'v': 1, 'amount': 20},
            {'id': 'deposit', 'kind': 'deposit', 't': 2, 'u': -1, 'v': 0, 'amount': 10},
            {'id': 'c', 'kind': 'payment', 't': 3, 'u': 0, 'v': 1, 'amount': 30},
            {'id': 'future', 'kind': 'payment', 't': 4, 'u': 1, 'v': 2, 'amount': 40},
        ], 'truth': {'a': True, 'b': False}})
        first = numeric_payments(EventDataset(document, {}))
        self.assertEqual(first.ids, ('a', 'b', 'c', 'future'))
        self.assertEqual(first.features[:, 1:].tolist(), [[0, 0], [0, 0], [2, 2], [0, 0]])
        self.assertEqual(first.times.tolist(), [60., 60., 180., 240.])
        ...
        for index, identifier in enumerate(first.ids[:3]):
            np.testing.assert_array_equal(first.features[index], third.features[third.ids.index(identifier)])
