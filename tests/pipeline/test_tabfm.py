...
class TabFMPipelineTests(unittest.TestCase):
    ...
    def test_catalog_explains_optional_setup_and_context_mode(self):
        with patch.object(tabfm, 'availability_error', return_value=tabfm.INSTALL_MESSAGE):
            model = next(row for row in self.service.models('original') if row['id'] == 'tabfm')
            ...
            with self.assertRaisesRegex(ValueError, 'Python 3.11'):
                self.service.start_training({'dataset_id': 'original', 'model_id': 'tabfm'})
            self.assertEqual(self.service.jobs(), [])
        model, descriptor = create_model('tabfm', 'numeric-table/v1', 'fraud-classification')
    ...
    def test_tabfm_parameters_and_feature_limits_are_checked_before_queueing(self):
        with patch.object(tabfm, 'availability_error', return_value=None):
            for parameters in ({'max_context_rows': 1}, {'max_context_rows': 2049},
                               {'inference_batch_size': True}, {'n_estimators': 9},
                               {'seed': -1}, {'checkpoint_path': '/tmp/untrusted'},
                               {'device': 'cuda'}, {'inference_batch_size': float('nan')}):
    ...
    def test_context_excludes_late_and_heldout_labels_then_reloads_for_comparison(self):
        ...
        with patch.object(tabfm, 'availability_error', return_value=None), \
            ...
            job = self.wait(self.service.start_training({
                'dataset_id': 'original', 'model_id': 'tabfm',
                'parameters': {'max_context_rows': 10, 'inference_batch_size': 3}}))
            ...
            self.assertEqual(metadata['model_provenance']['revision'], tabfm.WEIGHTS_REVISION)
            ...
            with self.assertRaisesRegex(ValueError, 'Selected evaluation IDs'):
                evaluate_artifact(folder / 'artifact', config['dataset'], partition='test', evaluation_ids=['p-0'])
            baseline = self.wait(self.service.start_training({
            ...
            comparison = self.wait(self.service.start_comparison({
                'dataset_id': 'original', 'partition': 'test',
            ...
            for row in result['rows']:
                self.assertEqual(row['predictions'][run['id']]['probability'], expected[row['id']])
