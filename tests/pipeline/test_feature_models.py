...
class FeatureModelTests(unittest.TestCase):
    ...
    def variant(self, identifier, features):
        status, result = self.pipeline.post(
            "/api/pipeline/datasets/" + identifier + "/features",
    ...
    def test_saved_columns_reach_real_models_and_tabfm_context(self):
        source = self.pipeline.store.generate(
            {
                "generator": "handbook_generator",
                "parameters": {"transactions": 160, "fraud_rate": 0.3},
            }
        )
        selected = [
            "log_amount",
        ...
        versions = dict.fromkeys(
            ("tabfm", "torch", "numpy", "scikit-learn", "scipy"), "test"
        )
        ...
        class Classifier:
            ...
            def predict_proba(self, x):
                probability = np.where(x[:, 0] > 0.5, 0.8, 0.2)
        ...
        with patch.object(tabfm, "availability_error", return_value=None), patch.object(
            ...
            for model, parameters in models:
                with self.subTest(model=model):
                    result = self.wait(
                        self.pipeline.training.start_training(
                            {
                                "dataset_id": variant["id"],
                                "model_id": model,
                                "parameters": {**parameters, "decision_threshold": 0.5},
                            }
                        )
                    )
                    run = result["run"]
                    ...
                    if model == "tabfm":
                        ...
                        expected = data.features[
                            [data.ids.index(identifier) for identifier in context_ids]
                        ]
                        np.testing.assert_array_equal(state["context_x"], expected)
            self.pipeline.close()
            self.pipeline = PipelineService(self.base / "workspace")
            report = self.wait(
                self.pipeline.training.start_comparison(
                    {
                        "dataset_id": variant["id"],
    ...
    def test_numeric_only_variant_retains_source_identity_and_label_cutoffs(self):
        ...
        csv.write_text(
            ...
            + "\n".join(
                f"p-{i},{i},1,2,{10 + 90 * (i % 2)},{i % 2}" for i in range(100)
            )
            + "\n"
        )
        prepared = self.base / "source.sqlite"
        with prepare_source(
            {"dataset": "handbook", "path": str(csv)}, self.base, prepared
        ):
            pass
        with sqlite3.connect(prepared) as database:
            database.execute(
                "UPDATE outcomes SET available_at=1000 WHERE event_id IN ('p-1','p-61')"
            )
        ...
        source = next(
            row
            for row in self.pipeline.store.catalog()["datasets"]
            if row["name"] == "Numeric history"
        )
        ...
        self.assertEqual(right["label_available_at"], {"p-1": 1000.0, "p-61": 1000.0})
        run = self.wait(
            self.pipeline.training.start_training(
                {
                    "dataset_id": variant["id"],
        ...
        with self.assertRaisesRegex(ValueError, "held-out test"):
            self.pipeline.training._comparison_population(
                {"partition": "all"}, snapshot, [record]
            )
