...
class StoredFeatureTests(unittest.TestCase):
    def setUp(self):
        ...
        self.parent = self.store.import_source(
            {"source": "handbook", "csv": CSV, "amount_to_eur": 2.0}
        )
    ...
    def save(self, features=None, **kwargs):
        return self.store.save_features(
            self.parent["id"],
            {
                "features": features or ["amount", "log_amount", "sender_out_count"],
                **kwargs,
            },
        )
    ...
    def test_catalog_describes_columns_and_exact_population_statistics(self):
        ...
        definitions = {
            definition["id"]: definition for definition in catalog["features"]
        }
        ...
        self.assertIn(
            "strictly earlier", definitions["sender_out_count"]["description"]
        )
    ...
    def test_immutable_variant_stores_ordered_columns_and_keeps_source_unchanged(self):
        ...
        self.assertEqual(
            self.store.sample(variant["id"])["columns"], list(numeric.feature_names)
        )
        np.testing.assert_array_equal(
            load_dataset(self.store.dataset_config(self.parent["id"])).features,
            before.features,
        )
    ...
    def test_graph_and_labeled_graph_reuse_stored_columns_and_keep_clocks(self):
        ...
        with patch.object(
            recipes,
            "prepare_features",
            side_effect=AssertionError("Must use stored columns"),
        ):
            for view in ("graph", "labeled_graph"):
                original = load_dataset(
                    self.store.dataset_config(self.parent["id"], view)
                )
                ...
                np.testing.assert_array_equal(
                    graph.edge_features[1:], numeric.features.astype(np.float32)
                )
                ...
                np.testing.assert_array_equal(
                    graph.destinations, original_graph.destinations
                )
                self.assertEqual(graph.provenance["time_origin_seconds"], 60.0)
                if view == "labeled_graph":
                    np.testing.assert_array_equal(selected.labels, original.labels)
                    np.testing.assert_array_equal(
                        selected.label_available_at, original.label_available_at
                    )
                    compact = load_labeled_dataset(
                        config, graph_config={"node_feature_dim": 8}
                    )
    ...
    def test_cache_restart_and_variants_reenable_columns_without_recomputation(self):
        with patch.object(
            recipes, "prepare_features", wraps=recipes.prepare_features
        ) as calculate:
        ...
        with patch.object(
            ...
            self.assertEqual(
                again.features(variant["id"])["enabled_features"], ["sender_out_count"]
            )
            child = again.save_features(
                variant["id"], {"features": ["amount", "recipient_in_count"]}
            )
        ...
        self.assertEqual(
            load_dataset(again.source_config(child["id"])).feature_names, ("amount",)
        )
    ...
    def test_recipe_updates_do_not_change_saved_catalog_or_values(self):
        ...
        with patch.object(recipes, "RECIPE_VERSION", "future-recipes/v2"), patch.object(
            recipes,
            "prepare_features",
            side_effect=AssertionError("Historical recipe must remain pinned"),
        ):
            self.assertEqual(
                self.store.features(variant["id"]),
                {**catalog, "latest_recipe_version": "future-recipes/v2"},
            )
            child = self.store.save_features(
                variant["id"], {"features": variant["feature_names"]}
            )
            self.assertEqual(child["fingerprint"], variant["fingerprint"])
            self.assertEqual(
                child["feature_recipe_version"], variant["feature_recipe_version"]
            )
            np.testing.assert_array_equal(
                load_dataset(self.store.dataset_config(child["id"])).features,
                before.features,
            )
    ...
    def test_variant_contains_its_source_and_all_original_recipe_columns(self):
        ...
        with patch.object(
            recipes,
            "prepare_features",
            side_effect=AssertionError("Self-contained saved recipe"),
        ):
            self.assertEqual(self.store.features(variant["id"])["row_count"], 3)
            child = self.store.save_features(
                variant["id"], {"features": ["amount", "sender_in_count"]}
            )
    ...
    def test_unavailable_relational_features_are_explained_and_rejected(self):
        ...
        variant = self.store.save_features(
            source["id"], {"features": ["V3", "log_amount"]}
        )
        self.assertEqual(variant["views"], ["numeric"])
        self.assertEqual(
            load_dataset(self.store.dataset_config(variant["id"])).features.shape,
            (1, 2),
        )
    ...
    def test_invalid_selections_and_failed_save_publish_nothing(self):
        ...
        for payload in (
            ...
            {"features": ["amount"], "path": "/etc/passwd"},
            {"features": ["amount"], "name": ""},
        ):
            with self.assertRaises(ValueError):
                self.store.save_features(self.parent["id"], payload)
        self.store.features(self.parent["id"])
        with patch(
            "framework.pipeline_data.write_numeric",
            side_effect=ValueError("Failed write"),
        ):
    ...
    def test_saved_matrix_and_feature_cache_checksums_are_verified(self):
        ...
        (cache / "columns.npz").write_bytes(
            (cache / "columns.npz").read_bytes() + b"changed"
        )
        with self.assertRaisesRegex(ValueError, "feature payload checksum"):
            self.store.features(variant["id"])
    ...
    def test_payment_scenarios_support_same_selected_feature_contract(self):
        ...
        variant = self.store.save_features(
            source["id"], {"features": ["amount", "pair_total_count"]}
        )
        numeric = load_dataset(self.store.dataset_config(variant["id"]))
        labeled = load_dataset(
            self.store.dataset_config(variant["id"], "labeled_graph")
        )
        self.assertEqual(numeric.ids, labeled.ids)
        self.assertEqual(labeled.graph.edge_feature_names, numeric.feature_names)
        np.testing.assert_array_equal(
            labeled.graph.edge_features[1:], numeric.features.astype(np.float32)
        )
