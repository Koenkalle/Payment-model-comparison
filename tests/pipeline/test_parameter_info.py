...
from framework.pipeline_training import (
    MODEL_SETTINGS,
    TrainingService,
    _field,
...
class ParameterInfoTests(unittest.TestCase):
    def catalog(self):
        ...
        with patch(
            "framework.pipeline_training.importlib.import_module",
    ...
    def test_future_fields_must_supply_help_and_catalog_results_are_isolated(self):
        ...
        self.assertNotEqual(
            self.catalog()[0]["parameters"][0], first[0]["parameters"][0]
        )

    def test_tabfm_displayed_limits_and_defaults_match_native_context_adapter(self):
        fields = {
            field["name"]: field for field in MODEL_SETTINGS["tabfm"]["parameters"]
        }
        self.assertEqual(
            set(fields) - {"decision_threshold"}, set(tabfm.DEFAULT_PARAMETERS)
        )
        for name, default in tabfm.DEFAULT_PARAMETERS.items():
            with self.subTest(parameter=name):
                self.assertEqual(fields[name]["default"], default)
                self.assertEqual(
                    (fields[name]["min"], fields[name]["max"]),
                    tabfm.PARAMETER_LIMITS[name],
                )
                for value in tabfm.PARAMETER_LIMITS[name]:
                    self.assertEqual(
                        _validate_parameters("tabfm", {name: value})[name], value
                    )
    ...
    def test_displayed_graph_architectures_satisfy_explained_attention_constraint(self):
        fields = {
            field["name"]: field
            for field in MODEL_SETTINGS["dyg_tami_native"]["parameters"]
        }
