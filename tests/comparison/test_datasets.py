...
class DatasetWebTests(unittest.TestCase):
    ...
    def upload(self, **values):
        return self.service.load({'id': 'csv:handbook', 'csv': CSV, 'amount_to_eur': 2., **values})
    ...
    def test_csv_projection_currency_labels_ties_and_interval(self):
        ...
        self.assertEqual([event['id'] for event in document['events']], ['late', 'unknown'])
        ...
        self.assertTrue(all('TX_FRAUD_SCENARIO' not in event for event in document['events']))
    ...
    def test_paysim_uses_same_adapters_and_excludes_post_event_fields(self):
        csv = ('step,type,amount,nameOrig,nameDest,oldbalanceOrg,oldbalanceDest,newbalanceOrig,newbalanceDest,isFraud,isFlaggedFraud\n'
               '1,TRANSFER,10,C1,C2,100,0,90,10,1,1\n')
        data = self.service.load({'id': 'csv:paysim', 'csv': csv, 'amount_to_eur': 1., 'release': 'local-test'})
    ...
    def test_untrusted_request_options_cannot_select_local_files(self):
        for payload in ({'id': '../README.md'}, {'id': 'handbook-demo', 'path': '/etc/passwd'},
                        {'id': 'csv:handbook', 'python_module': 'os'}, {'id': 'handbook-demo', 'csv': CSV}):
            with self.assertRaises(ValueError):
                self.service.load(payload)
        with self.assertRaisesRegex(ValueError, 'no account or merchant'):
            self.service.load({'id': 'csv:ulb'})
        for selection in ({'start': 3, 'stop': 2}, {'limit': 10}, {'start': float('nan')}):
            with self.assertRaises(ValueError):
    ...
    def test_registered_csv_and_prepared_files_are_available_by_opaque_id(self):
        with tempfile.TemporaryDirectory() as temporary:
            ...
            config = {'loader': 'fraud_dataset', 'dataset': 'handbook', 'path': 'data.csv',
                      'view': 'numeric', 'features': ['amount'], 'amount_to_eur': 1.,
                      'name': 'My local data', 'selection': {'start': 120}}
            (base / 'source.json').write_text(json.dumps(config))
            with prepare_source(config, base, base / 'data.sqlite'):
                pass
            (base / 'prepared.json').write_text(json.dumps({'loader': 'prepared_fraud', 'path': 'data.sqlite',
                                                           'currency': 'EUR', 'amount_to_eur': 1.}))
            ...
            self.assertEqual(service.load({'id': 'local:1', 'selection': {}})['summary']['events'], 3)
