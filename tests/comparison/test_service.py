    ...
    def compare(self, document, options, cancelled=None):
        ...
        try:
            ...
            return {
                ...
                "options": options,
                "predictions": [
                    {"id": document["events"][0]["id"], "logit": float(self.calls)}
                ],
            }
        finally:
            self.active -= 1
...
class ServiceTests(unittest.TestCase):
    def setUp(self):
        ...
        self.service = ComparisonService(
            scorer_factory=lambda *_: self.scorer,
            supervised_artifact=None,
            cache_entries=2,
        )
    ...
    def request(
        self, method="GET", path="/api/models", payload=None, body=None, headers=None
    ):
    ...
    def payload(self, **options):
        return {
            ...
            "dataset": copy.deepcopy(DOCUMENT),
            "options": options,
        }
    ...
    def test_dataset_catalog_loading_and_existing_request_protections(self):
        ...
        self.assertIn(
            "handbook-demo", [entry["id"] for entry in json.loads(body)["datasets"]]
        )
        status, _, body = self.request(
            "POST", "/api/datasets/load", {"id": "handbook-demo"}
        )
        self.assertEqual(status, 200)
        ...
        for payload in (
            ...
            self.assertEqual(
                self.request("POST", "/api/datasets/load", payload)[0], 400
            )
        self.assertEqual(
            self.request(
                "POST",
    ...
    def test_discovery_static_and_same_origin_json(self):
        ...
        self.assertEqual(
            set(model["capabilities"]["prediction_heads"]),
        ...
        status, headers, body = self.request(
            "POST", "/api/compare", self.payload(), headers={"Origin": origin}
        )
        self.assertEqual(status, 200)
    ...
    def test_reject_bad_model_settings_and_documents_without_inference(self):
        variants = [
            ...
            self.payload(trainingMode="unknown"),
            self.payload(trainingMode="supervised", predictionHead="empirical_tail"),
            self.payload(mode="invalid"),
            self.payload(decisionPolicy="unknown"),
            self.payload(predictionHead="unknown"),
            self.payload(alpha=0),
            self.payload(alpha=True),
            self.payload(warmup=1.5),
            self.payload(falseBlockCost=-1),
            self.payload(eta=2),
            self.payload(artifact="/tmp/file"),
        ]
    ...
    def test_cache_includes_stream_truth_and_settings_and_is_bounded(self):
        ...
        self.assertEqual(
            self.service.compare(self.payload(mode="shadow", alpha=0.02))[
                "predictions"
            ][0]["logit"],
            1,
        )
        ...
        self.assertEqual(
            self.service._cache_bytes, sum(map(len, self.service._cache.values()))
        )
        no_cache = ComparisonService(
            scorer_factory=lambda *_: self.scorer, max_cache_bytes=1
        )
    ...
    def test_parallel_requests_serialize_torch_work_and_reuse_duplicates(self):
        self.scorer.delay = 0.03
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as pool:
            results = list(
                pool.map(
                    lambda _: self.request("POST", "/api/compare", self.payload()),
                    range(4),
                )
            )
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as pool:
            results = list(
                pool.map(
                    lambda alpha: self.service.compare(self.payload(alpha=alpha)),
                    [0.03, 0.04, 0.05, 0.06],
                )
            )
    ...
    def test_http_rejects_cross_origin_hidden_paths_large_and_non_json_requests(self):
        for headers in (
            ...
            {"Host": "example.com:" + str(self.server.server_port)},
            {"Sec-Fetch-Site": "cross-site"},
        ):
            self.assertEqual(
                self.request("POST", "/api/compare", self.payload(), headers=headers)[
                    0
                ],
                403,
            )
        ...
        self.assertEqual(
            self.request("POST", "/api/compare", body=b"{", headers=headers)[0], 400
        )
        self.assertEqual(
            self.request(
                "POST", "/api/compare", body=b'{"alpha":NaN}', headers=headers
            )[0],
            400,
        )
        headers["Content-Length"] = str(MAX_BODY_BYTES + 1)
        self.assertEqual(
            self.request("POST", "/api/compare", body=b"{}", headers=headers)[0], 413
        )
    ...
    def test_new_settings_cancel_running_and_queued_work(self):
        ...
        self.assertEqual(
            self.request(
                "POST", "/api/compare", self.payload(alpha=0.1), headers=first
            )[0],
            409,
        )
        self.assertEqual(self.scorer.calls, 2)
        self.assertEqual(
            self.request(
                ...
                "/api/compare",
                self.payload(),
                headers={"X-Comparison-Session": "page-1"},
            )[0],
            400,
        )
        for i in range(40):
    ...
    def test_capabilities_follow_actual_configured_artifact(self):
        ...
        self.assertEqual(
            description["capabilities"]["decision_policies"], ["shared", "manual"]
        )
        self.assertEqual(
            description["capabilities"]["prediction_heads"], ["empirical_tail"]
        )
    ...
    def test_checkpoint_identity_follows_loaded_weights_and_location(self):
        ...
        class IdentifiedScorer(Scorer):
            def __init__(self, path):
                super().__init__()
                self.description = {
                    "checkpoint_id": "loaded-weights",
                    "training_modes": ["unsupervised"],
                    "artifact": {
                        "path": str(path),
                        "model_sha256": "loaded-weights",
                        "best_epoch": 7,
                    },
                }
        ...
        for path, source in [
            ...
            (self.root / "my-trained-model", "custom"),
        ]:
            with self.subTest(source=source):
                scorer = IdentifiedScorer(path)
                service = ComparisonService(
                    path, scorer_factory=lambda *_: scorer, supervised_artifact=None
                )
                catalog = service.models()["models"][0]["capabilities"][
                    "training_mode_capabilities"
                ]["unsupervised"]
                ...
                self.assertEqual(
                    result["model"]["artifact"]["model_sha256"], "loaded-weights"
                )
                ...
                with self.assertRaisesRegex(ValueError, "checkpoint differs"):
                    service.compare(self.payload(alpha=0.1))

    def test_supervised_mode_routes_to_separate_artifact_and_head(self):
        supervised = Scorer()
        supervised.describe = lambda: {
            "checkpoint_id": "fraud-checkpoint",
            "supported_policies": ["shared", "manual", "tuned", "auto"],
        ...
        description = self.service.models()["models"][0]
        self.assertEqual(
            description["capabilities"]["training_modes"],
        ...
        status, _, body = self.request(
            "POST", "/api/compare", self.payload(trainingMode="supervised")
        )
        ...
        self.assertEqual(
            self.request(
                "POST", "/api/compare", self.payload(predictionHead="fraud_linear")
            )[0],
            400,
        )
    ...
    def test_missing_mode_is_isolated_and_never_falls_back(self):
        ...
        self.assertFalse(
            description["capabilities"]["training_mode_capabilities"]["supervised"][
                "available"
            ]
        )
        self.assertEqual(
