...
const imported=context.FraudScenarios.build('takeover','small',17,24*60,2);
let releaseFile,importFinished=false;
const fileText=new Promise(resolve=>releaseFile=resolve);
elements['fd-dataset-file'].files=[{name:'delayed-read.json',text:()=>fileText}];
const importTask=elements['fd-dataset-file'].listeners.change();
const importIdle=demo.whenIdle().then(()=>importFinished=true);
await new Promise(resolve=>setTimeout(resolve,0));
assert.strictEqual(importFinished,false,'whenIdle must include file reading before inference is scheduled.');
releaseFile(JSON.stringify(imported));await Promise.all([importTask,importIdle]);
assert.strictEqual(get().events,imported.events.length);assert.strictEqual(get().accounts,imported.accounts.length);
assert(elements['fd-dataset-status'].textContent.startsWith('Imported'));
let releaseObsolete;
elements['fd-dataset-file'].files=[{name:'obsolete-read.json',text:()=>new Promise(resolve=>releaseObsolete=resolve)}];
const obsolete=elements['fd-dataset-file'].listeners.change();
await demo.setDataset(null);const reset=get();releaseObsolete(JSON.stringify(imported));await obsolete;await demo.whenIdle();
assert.strictEqual(get().events,reset.events,'Reset must invalidate an unfinished file import.');
assert(elements['fd-dataset-status'].textContent.startsWith('Synthetic'));
console.log('PASS UI: file reads participate in idle tracking and reset cancels stale imports.');
