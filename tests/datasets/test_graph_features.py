...
def events(edges, kind='payment'):
    return [SimpleNamespace(source=source, destination=target, kind=kind)
    ...
    for node, neighbors in enumerate(adjacency):
        if neighbors:
            for target in neighbors:
                transition[node, target] = 1. / len(neighbors)
        else:
            transition[node] = 1. / count
    restart = np.full(count, 1. / count) if source is None else np.eye(count)[state.node_lookup[source]]
    return np.linalg.solve(np.eye(count) - state.damping * transition.T,
                           (1 - state.damping) * restart)
...
class GraphFeatureTests(unittest.TestCase):
    def test_cold_queries_and_nonpayment_events_never_admit_nodes(self):
        ...
        state.observe_batch(events([(10, 20)], 'deposit') + events([(30, 40)], 'report') +
    ...
    def test_directed_pagerank_matches_dangling_node_oracle_and_bound(self):
        ...
        actual = np.array([values['graph_sender_pagerank'], values['graph_recipient_pagerank']])
        self.assertLessEqual(np.abs(actual - expected).sum(), values['graph_pagerank_error_bound'])
        self.assertAlmostEqual(values['graph_sender_pagerank'], 1 / (2 + DAMPING), places=7)
        self.assertAlmostEqual(values['graph_recipient_pagerank'], (1 + DAMPING) / (2 + DAMPING), places=7)
    ...
    def test_pagerank_snapshots_show_lag_and_refresh_after_unique_edge_threshold(self):
        ...
        self.assertEqual(stale['graph_sender_pagerank'], previous['graph_sender_pagerank'])
        ...
        actual = np.array([state.values(node, node)['graph_sender_pagerank'] for node in state.nodes])
        self.assertLessEqual(np.abs(actual - stationary(state, directed=True)).sum(),
                             refreshed['graph_pagerank_error_bound'])

    def test_ppr_tiny_graphs_are_lower_bounds_with_valid_error_certificates(self):
        graphs = [[(0, 1)], [(0, 1), (1, 2)], [(0, 1), (1, 2), (2, 0), (2, 3)],
    ...
    def test_two_node_ppr_converges_within_target_mass(self):
        ...
        self.assertAlmostEqual(result.estimate('s'), 1 / (1 + DAMPING), delta=result.error_bound('s'))
        self.assertAlmostEqual(result.estimate('t'), DAMPING / (1 + DAMPING), delta=result.error_bound('t'))
    ...
    def test_high_degree_partial_push_keeps_hub_and_unvisited_neighbor_signal(self):
        ...
        self.assertLessEqual(exact_hub - leaf_result.estimate('hub'), leaf_result.error_bound('hub'))
        ...
        for leaf in range(leaves):
            ...
            self.assertLessEqual(exact_leaf - hub_result.estimate(leaf), hub_result.error_bound(leaf))
    ...
    def test_endpoint_admission_does_not_scan_existing_identity_registry(self):
        class NoRegistryScan(dict):
            def keys(self):
                raise AssertionError('Endpoint admission must not scan all historical identities.')
    ...
    def test_batch_permutation_and_query_order_do_not_change_values(self):
        ...
        pairs = [(source, target) for source in states[0].nodes for target in states[0].nodes]
    ...
    def test_definitions_cover_every_value_and_explain_approximation(self):
        state = GraphFeatureState()
        self.assertEqual({spec['id'] for spec in GRAPH_FEATURE_SPECS}, set(state.values(0, 1)))
        for spec in GRAPH_FEATURE_SPECS:
            for field in ('method', 'parameters', 'approximation', 'orientation', 'readout',
                          'unit', 'transform', 'window', 'requirements', 'version'):
