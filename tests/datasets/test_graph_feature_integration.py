...
class GraphFeatureIntegrationTests(unittest.TestCase):
    ...
    def document(self, events, truth=None):
        return {
            "accounts": [{"id": index} for index in range(8)],
            "events": events,
            "truth": truth or {},
        }
    ...
    def prepare(self, document):
        ...
        return prepare_features(
            {"loader": "payment_json", "path": str(path)}, self.base
        )
    ...
    def test_candidates_ties_future_vertices_and_outcomes_cannot_enter_history(self):
        ...
        changed["events"].insert(
            2,
            {"id": "report", "kind": "report", "t": 0.2, "u": -1, "v": 5, "amount": 0},
        )
        changed["events"].append(payment("future", 10, 6, 7))
        changed["accounts"].extend([{"id": 8}, {"id": 9}])
        updated, _ = self.prepare(changed)
        np.testing.assert_array_equal(
            self.graph_values(baseline), self.graph_values(updated)[: len(baseline.ids)]
        )
        ...
        for index, identifier in enumerate(baseline.ids):
            np.testing.assert_array_equal(
                self.graph_values(baseline)[index],
    ...
    def test_bipartite_ppr_finds_three_hop_relation_before_first_direct_payment(self):
        ...
        with prepare_source(
            {"dataset": "handbook", "path": str(source)}, self.base, prepared
        ):
            pass
        data, definitions = prepare_features(
            {
        ...
        self.assertGreater(
            self.value(data, "query", "graph_ppr_sender_to_recipient"), 0
        )
        self.assertGreater(
            self.value(data, "query", "graph_ppr_recipient_to_sender"), 0
        )
        self.assertEqual(self.value(data, "query", "graph_same_component"), 1)
        self.assertAlmostEqual(
            self.value(data, "query", "graph_sender_component_size"), np.log1p(4)
        )
    ...
    def test_saved_graph_columns_stats_and_model_views_are_reused_without_recalculation(
        self,
    ):
        store = DatasetStore(self.base / "store")
        source = store.generate(
            {
        ...
        for name in names:
            definition = next(
                item for item in catalog["features"] if item["id"] == name
            )
            values = numeric.features[:, numeric.feature_names.index(name)]
            self.assertAlmostEqual(
                definition["statistics"]["mean"], float(values.mean())
            )
        with patch.object(
            features,
            "prepare_features",
            side_effect=AssertionError("Saved feature values only"),
        ):
            reopened = DatasetStore(self.base / "store")
            self.assertEqual(
                reopened.features(selected["id"])["enabled_features"], names
            )
            graph = load_dataset(
                reopened.dataset_config(selected["id"], "labeled_graph")
            ).graph
            ...
            self.assertEqual(
                load_dataset(reopened.dataset_config(child["id"])).feature_names,
                (names[0],),
            )

    def test_numeric_only_sources_skip_graph_computation_and_explain_unavailability(
        self,
    ):
        ...
        with patch.object(
            features,
            "GraphFeatureState",
            side_effect=AssertionError("No graph work without identities"),
        ):
            catalog = store.features(source["id"])
    ...
    def test_existing_v1_variants_stay_pinned_and_original_source_discovers_graph_recipes(
        self,
    ):
        store = DatasetStore(self.base / "store")
        ...
        old_recipes = [
            recipe
            for recipe in features.FEATURE_RECIPES
            if recipe.definition["group"] != "graph"
        ]
        with patch.object(
            features, "RECIPE_VERSION", "payment-features/v1"
        ), patch.object(features, "FEATURE_RECIPES", old_recipes):
            old = store.save_features(
                source["id"], {"features": ["amount", "sender_out_count"]}
            )
        ...
        self.assertTrue(
            any(
                item["id"] == "graph_recipient_pagerank" for item in current["features"]
            )
        )
        np.testing.assert_array_equal(
            load_dataset(store.dataset_config(old["id"])).features, before
        )
