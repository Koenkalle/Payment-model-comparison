...
class FeatureTests(unittest.TestCase):
    ...
    def test_timestamp_ties_are_simultaneous_and_window_boundary_is_inclusive(self):
        config = self.payments(
            [
                payment("first", 0, settled=False),
        ...
        self.assertEqual(
            len(
                [
                    ...
                    if item["kind"] == "derived" and item["group"] != "graph"
                ]
            ),
            27,
        )
        ...
        np.testing.assert_allclose(
            self.column(dataset, "sender_out_count"),
            [0, 0, math.log1p(2) / 6, math.log1p(3) / 6],
        )
        np.testing.assert_allclose(
            self.column(dataset, "sender_recent_out_count"),
            [0, 0, math.log1p(2) / 4, math.log1p(1) / 4],
        )
        np.testing.assert_allclose(
            self.column(dataset, "sender_recent_out_value"),
            [0, 0, math.log1p(300) / 10, math.log1p(100) / 10],
        )
        np.testing.assert_array_equal(
            self.column(dataset, "prior_contact"), [0, 0, 1, 1]
        )
    ...
    def test_outcomes_settlement_and_reports_do_not_change_features(self):
        ...
        original, _ = prepare_features(
            self.payments(events, {"first": False, "candidate": True}), self.base
        )
        changed = [
            {**event, "settled": True} if event["kind"] == "payment" else event
            for event in events
            if event["kind"] != "report"
        ]
        updated, _ = prepare_features(
            self.payments(changed, {"first": True}), self.base
        )
        ...
        self.assertAlmostEqual(
            self.column(original, "sender_seen")[1], math.log1p(2) / 6
        )
        self.assertAlmostEqual(
            self.column(original, "sender_gap")[1], math.log1p(10) / 6
        )
        self.assertAlmostEqual(
            self.column(original, "sender_out_value")[1], math.log1p(100) / 10
        )
    ...
    def test_appending_future_events_cannot_change_existing_rows(self):
        ...
        after, _ = prepare_features(
            self.payments([*events, payment("future", 200, amount=1e8)]), self.base
        )
    ...
    def test_prepared_source_columns_and_units_survive_configured_subset(self):
        ...
        dataset, definitions = prepare_features(config, self.base)
        self.assertEqual(
            dataset.feature_names[:3], ("amount", "oldbalanceOrg", "oldbalanceDest")
        )
        np.testing.assert_array_equal(
            dataset.features[:, :3], [[35, 100, 50], [50, 65, 85]]
        )
        amount_transform = next(
            item for item in definitions if item["id"] == "log1p_amount"
        )
        self.assertEqual(amount_transform["kind"], "derived")
        self.assertNotIn(
            "log1p_amount",
            [item["id"] for item in definitions if item["kind"] == "source"],
        )
        np.testing.assert_allclose(
            self.column(dataset, "log1p_amount"), np.log1p([35, 50])
        )
        self.assertAlmostEqual(
            self.column(dataset, "log_amount")[0], math.log1p(35) / 8
        )
        ...
        self.assertEqual(
            bucket["amount_unit_assumption"], "source currency units (unspecified)"
        )
        self.assertEqual(
            bucket["amount_bins"], [15, 35, 75, 150, 300, 650, 1500, 4000, 10000]
        )
        self.assertAlmostEqual(
            self.column(dataset, "sender_recent_out_count")[1], math.log1p(1) / 4
        )
    ...
    def test_ulb_has_amount_features_and_unavailable_relational_definitions(self):
        ...
        self.assertEqual(
            len([item for item in unavailable if item["group"] != "graph"]), 25
        )
        self.assertTrue(any(item["group"] == "graph" for item in unavailable))
        self.assertTrue(
            all("identities" in item["unavailable_reason"] for item in unavailable)
        )
        self.assertFalse(
            set(item["id"] for item in unavailable) & set(dataset.feature_names)
        )
    ...
    def test_selection_keeps_observed_history_before_its_start(self):
        config = self.prepared(
            ...
            [
                ["before", 10, "same", "same", 100],
                ["selected", 20, "same", "same", 50],
        ...
        self.assertAlmostEqual(
            self.column(dataset, "sender_out_count")[0], math.log1p(1) / 6
        )
    ...
    def test_registry_extension_and_duplicate_ids(self):
        ...
        with patch.object(
            features, "FEATURE_RECIPES", [*features.FEATURE_RECIPES, recipe]
        ):
            dataset, definitions = prepare_features(config, self.base)
        ...
        with patch.object(
            features, "FEATURE_RECIPES", [duplicate]
        ), self.assertRaisesRegex(ValueError, "unique IDs"):
    ...
    def test_history_memory_is_sparse_and_old_event_records_are_released(self):
        ...
        for index in range(10000):
            event = FeatureEvent(
                str(index), index * 60.0, 1.0, ("source", index), ("destination", index)
            )
        ...
        history.advance(10000 * 60.0 + 3600)
    ...
    def test_legacy_checkpoint_outputs_match_saved_parity(self):
        from models.implementations.xgboost_numpy import (
            xgb_trace,
            xgb_unsupervised_trace,
        )

        model = json.loads((ROOT / "models/xgboost.json").read_text())
        for folder, calculate in [
            ("parity", xgb_unsupervised_trace),
