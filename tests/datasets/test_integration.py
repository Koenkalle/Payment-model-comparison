...
class DatasetIntegrationTests(unittest.TestCase):
            ...
            for index in reversed(range(80)):
                writer.writerow(
                    [
                        ...
                        index % 3,
                        index % 3,
                        150 if index % 2 else 10,
                        "" if index == 0 else index % 2,
                        3 if index % 2 else 0,
                    ]
                )
    ...
    def test_views_keep_inputs_units_identity_and_unknown_labels(self):
        ...
        graph = load_dataset(
            {**self.config, "view": "graph", "node_feature_dim": 4}, self.base
        )
        ...
        payment_config = {**self.config, "view": "payments", "amount_to_eur": 2.0}
        payments = load_dataset(payment_config, self.base)
        self.assertEqual(payments.document["events"][0]["amount"], 300.0)
        ...
        self.assertFalse(
            any("TX_FRAUD_SCENARIO" in event for event in payments.document["events"])
        )
        labeled = load_labeled_dataset(
            {**payment_config, "view": "labeled_graph", "node_feature_dim": 4},
            self.base,
        )
        ...
        wrapped = load_dataset(
            {
                ...
                "dataset": payment_config,
                "node_feature_dim": 4,
            },
            self.base,
        )
        np.testing.assert_array_equal(
            wrapped.edge_features, labeled.graph.edge_features
        )
        episodes = load_dataset(
            {
                "loader": "payment_episodes",
                "episodes": [payment_config, payment_config],
            },
            self.base,
        )
        self.assertEqual(len(episodes.document["events"]), 160)
        self.assertEqual(
            len({event["id"] for event in episodes.document["events"]}), 160
        )
        ...
        selected = load_dataset(
            {**self.config, "selection": {"start": 103, "stop": 106}}, self.base
        )
    ...
    def test_ulb_is_explicitly_tabular(self):
        ...
        self.assertEqual(
            load_dataset({**config, "view": "numeric"}, self.base).features.shape,
            (1, 29),
        )
    ...
    def test_prepare_reopen_inspect_and_export_cli(self):
        ...
        def cli(*arguments):
            return subprocess.run(
                [sys.executable, str(ROOT / "experiment.py"), *arguments],
                cwd=self.base,
                text=True,
                capture_output=True,
            )
        ...
        self.assertNotEqual(
            cli(
                "prepare", "--config", str(config_path), "--output", str(store)
            ).returncode,
            0,
        )
        native = load_dataset(self.config, self.base)
        reopened = load_dataset(
            {"loader": "prepared_fraud", "path": str(store), "view": "numeric"},
            self.base,
        )
        self.assertEqual(native.ids, reopened.ids)
        np.testing.assert_array_equal(native.features, reopened.features)
        self.assertEqual(
            native.provenance["source_sha256"], reopened.provenance["source_sha256"]
        )
        ...
        config_path.write_text(
            json.dumps({**self.config, "view": "payments", "amount_to_eur": 1.0})
        )
        ...
        document, _ = validate_request(
            {"model_id": "dyg_tami_native", "dataset": json.loads(export.read_text())}
        )
        self.assertEqual(len(document["events"]), 80)
        self.assertEqual(
            load_dataset({"loader": "payment_json", "path": str(export)}).document[
                "truth"
            ],
            document["truth"],
        )
    ...
    def test_numeric_training_roundtrip_and_partition_guards(self):
        metadata, initial = train_experiment(
            {"model": "logistic_regression", "dataset": self.config},
            self.base / "logistic",
            self.base,
        )
        restored = evaluate_artifact(
            self.base / "logistic", self.config, self.base, "test"
        )
        ...
        self.assertEqual(
            evaluate_artifact(self.base / "logistic", equivalent, other_base, "test")[
                "rows"
            ],
            initial["rows"],
        )
        with self.assertRaisesRegex(ValueError, "original dataset"):
            evaluate_artifact(
                self.base / "logistic",
                {**self.config, "selection": {"start": 101}},
                self.base,
                "test",
            )
    ...
    def test_prepared_artifact_rejects_modified_store(self):
        ...
        with prepare_source(
            {**self.config, "view": "stream"}, self.base, self.base / "data.sqlite"
        ):
            pass
        config = {"loader": "prepared_fraud", "path": "data.sqlite", "view": "numeric"}
        train_experiment(
            {"model": "logistic_regression", "dataset": config},
            self.base / "prepared-model",
            self.base,
        )
    ...
    def test_fraud_training_reload_and_live_comparison(self):
        config = {
            ...
            "dataset": {
                **{key: value for key, value in self.config.items() if key != "path"},
                "paths": ["handbook.csv"],
                "view": "labeled_graph",
                "amount_to_eur": 1.0,
            },
        ...
        result = comparison.compare(
            dataset.document,
            {
                ...
                "predictionHead": "fraud_linear",
                "manualTau": 1.0,
            },
        )
        self.assertEqual(result["schema"], "native-fraud-comparison/v2")
        self.assertEqual(
            comparison.original.ids,
            tuple(event["id"] for event in dataset.document["events"]),
        )
        ...
        np.testing.assert_array_equal(
            changed.graph.edge_features, dataset.graph.edge_features
        )
        selected = load_dataset(
            {
                **self.config,
