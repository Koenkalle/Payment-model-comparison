...
class Stream:
    ...
    def truth_for(self, ids):
        return {
            identifier: self.outcomes[identifier]
...
class Model:
    ...
    def learn(self, feedback, history):
        ...
        self.calls.append(
            (
                "learn",
                tuple(item.event.event_id for item in feedback),
                tuple(event.event_id for event in history.events),
            )
        )
...
class ReplayTests(unittest.TestCase):
    def test_ties_are_atomic_across_batches_and_prefix_is_unscored(self):
        stream = Stream(
            [event("prefix", 0), event("a", 1), event("b", 1), event("c", 2)]
        )
        model = Model()
    ...
    def test_fixed_and_growing_history_with_independent_retention(self):
        ...
        fixed = Model()
        result = replay(
            stream, lambda: fixed, split_time=3, history="fixed", max_events=2
        )
        self.assertEqual(
            [call[2] for call in fixed.calls if call[0] == "predict"],
            [("1", "2"), ("1", "2")],
        )
        ...
        self.assertEqual(
            [call[2] for call in growing.calls if call[0] == "predict"],
            [("1", "2"), ("2", "3")],
        )
    ...
    def test_graph_preserves_typed_identity_and_absent_relations(self):
        ...
        history = HistoryView(
            (event("a", 0, entities=(customer, terminal)), event("b", 1))
        )
    ...
    def test_zero_delay_releases_only_after_entire_timestamp_group(self):
        stream = Stream(
            [event("p", 0), event("a", 1), event("b", 1), event("c", 2)],
    ...
    def test_recorded_feedback_respects_availability_and_retrospective_truth(self):
        stream = Stream(
            [event("p", 0), event("a", 1), event("b", 2), event("c", 3)],
            [
                Outcome("p", 1, 2),
                Outcome("a", 1, diagnostics={"scenario": 99}),
                Outcome("b", -1),
                Outcome("c", 0, 10),
            ],
        )
        ...
        self.assertEqual(
            result["manifest"]["evaluation_maturity"],
    ...
    def test_explicit_delay_overrides_recorded_timing_and_does_not_train_unknown(self):
        ...
        model = Model()
        result = replay(
            stream, lambda: model, split_time=0, learning=True, label_delay=1
        )
    ...
    def test_learning_is_independent_of_fixed_history(self):
        stream = Stream(
            [event("a", 0), event("b", 1)], [Outcome("a", 1), Outcome("b", 0)]
        )
        model = Model()
        replay(
            stream,
    ...
    def test_prediction_alignment_and_validation(self):
        ...
        class Rows:
            def predict(self, events, history):
                return [
                    {"event_id": "b", "score": 0.8},
                    {"event_id": "a", "score": 0.2},
                ]

        self.assertEqual(
            [row["score"] for row in replay(stream, Rows, split_time=0)["rows"]],
            [0.2, 0.8],
        )
        for scores, message in [
            ...
            with self.subTest(scores=scores), self.assertRaisesRegex(
                ValueError, message
            ):
    ...
    def test_protocol_parameter_and_chronology_validation(self):
        ...
        with self.assertRaisesRegex(ValueError, "precede"):
            replay(
                Stream([event("a", 1)], [Outcome("a", 1, 0)]),
                Model,
                split_time=0,
                learning=True,
            )
    ...
    def test_future_suffix_changes_cannot_change_earlier_scores(self):
        class AmountHistoryModel:
            def predict(self, events, history):
                ...
                return {
                    item.event_id: prior_amount + item.features["amount"]
                    for item in events
                }

        shared = [event("prefix", 0), event("a", 1), event("b", 1)]
        first = Stream(shared + [event("future", 2)], [Outcome("a", 1)])
        changed_future = TransactionEvent(
            "future", 2, "payment", (), {"amount": 100000}
        )
        second = Stream(shared + [changed_future], [Outcome("a", 0)])
        before = replay(
            first, AmountHistoryModel, split_time=1, score_kind="risk", batch_size=1
        )
        after = replay(
            second, AmountHistoryModel, split_time=1, score_kind="risk", batch_size=3
        )
        self.assertEqual(
            [row["score"] for row in before["rows"][:2]],
    ...
    def test_final_timestamp_zero_delay_feedback_follows_all_scores(self):
        stream = Stream(
            [event("a", 0), event("b", 0)], [Outcome("a", 1), Outcome("b", 0)]
        )
        model = Model()
        replay(
            stream,
        ...
        self.assertEqual(
            [call[0] for call in model.calls], ["predict", "observe", "learn"]
        )
    ...
    def test_empty_evaluation_is_rejected_but_cold_start_is_allowed(self):
        ...
        self.assertEqual(
            replay(Stream([event("a", 0)]), Model, split_time=0)["manifest"][
                "prefix_count"
            ],
            0,
        )
...
class PredictionJoinTests(unittest.TestCase):
    ...
    def test_explicit_subset_preserves_source_order(self):
        rows = join_predictions(
            self.stream, {"c": -3, "b": 7}, event_ids=["c", "b"], score_kind="risk"
        )
    ...
    def test_duplicate_missing_extra_and_wrong_dataset_are_rejected(self):
        cases = [
            ([{"event_id": "a", "score": 0.2}] * 2, {}, "duplicate"),
            ({"a": 0.1}, {}, "missing"),
            ...
            (
                {"a": 0.1},
                {"event_ids": ["a"], "dataset_identity": {"source_sha256": "wrong"}},
                "identity",
            ),
        ]
        for rows, options, message in cases:
            with self.subTest(options=options, message=message), self.assertRaisesRegex(
                ValueError, message
            ):
                join_predictions(self.stream, rows, **options)
        result = replay(self.stream, Model, split_time=0)
        joined = join_predictions(
            self.stream, result["rows"], dataset_identity=result["dataset"]
        )
