...
from datasets.stream import EntityRef, Outcome, open_prepared, open_source, prepare_source


HANDBOOK_FIELDS = ['TRANSACTION_ID', 'TX_TIME_SECONDS', 'CUSTOMER_ID', 'TERMINAL_ID',
                   'TX_AMOUNT', 'TX_FRAUD', 'TX_FRAUD_SCENARIO', 'TX_DATETIME']
...
class SourceTests(unittest.TestCase):
    ...
    def handbook(self, rows=None, name='handbook.csv'):
        self.csv(name, HANDBOOK_FIELDS, rows or [
    ...
    def test_repeatable_sorted_iteration_and_half_open_batches(self):
        with open_source(self.handbook(), self.base) as stream:
            ...
            self.assertEqual([event.event_id for event in stream.iter_events()], expected)
            first, second = stream.iter_events(), stream.iter_events()
            self.assertEqual(next(first), next(second))
            self.assertEqual(list(first), list(second))
            for size in (1, 2, 4):
                self.assertEqual([event.event_id for batch in stream.iter_batches(size)
                                  for event in batch], expected)
            self.assertEqual([event.event_id for event in stream.iter_events(start=10, stop=30)], expected[:2])
    ...
    def test_distinct_entity_types_preserve_equal_source_identifiers(self):
        with open_source(self.handbook(), self.base) as stream:
            event = next(stream.iter_events())
            self.assertEqual([(entity.kind, entity.id, entity.role) for entity in event.entities],
    ...
    def test_partition_order_and_cross_file_sort(self):
        ...
        self.handbook([['a', 20, 1, 2, 10, 0, 0, ''],
                       ['c', 10, 1, 2, 10, 0, 0, '']], 'parts/01.csv')
        with open_source({'dataset': 'handbook', 'path': 'parts'}, self.base) as stream:
            self.assertEqual([event.event_id for event in stream.iter_events()], ['c', 'a', 'b'])
        with open_source({'dataset': 'handbook', 'paths': ['parts/02.csv', 'parts/01.csv']}, self.base) as stream:
            self.assertEqual([event.event_id for event in stream.iter_events()], ['c', 'b', 'a'])
    ...
    def test_durable_preparation_roundtrip_and_no_overwrite(self):
        ...
        with prepare_source(config, self.base, output) as stream:
            events, descriptor, provenance = list(stream.iter_events()), stream.descriptor, stream.provenance
    ...
    def test_prepared_fingerprint_detects_valid_feature_edits(self):
        ...
        with prepare_source(self.handbook(), self.base, path) as stream:
            ...
            self.assertEqual(prepared_hash, hashlib.sha256(path.read_bytes()).hexdigest())
        with sqlite3.connect(path) as database:
            database.execute('UPDATE events SET features=? WHERE event_id=?',
                             ('{"amount": 999}', 'late'))
    ...
    def test_unknown_labels_and_missing_label_column(self):
        ...
        with open_source({'dataset': 'handbook', 'path': 'unlabeled.csv'}, self.base) as stream:
    ...
    def test_ulb_has_no_fabricated_graph_and_stable_physical_row_ids(self):
        ...
        with open_source({'dataset': 'ulb', 'path': 'ulb.csv'}, self.base) as stream:
            ...
            self.assertEqual([event.event_id for event in events], ['ulb:0:2', 'ulb:0:1'])
    ...
    def test_paysim_hours_pretransaction_balances_and_postevent_diagnostics(self):
        ...
        self.csv('paysim.csv', fields, [[2, 'CASH_OUT', 20, 'C1', 100, 80, 'C2', 30, 50, 1, 0]])
        with open_source({'dataset': 'paysim', 'path': 'paysim.csv'}, self.base) as stream:
            event = next(stream.iter_events())
            self.assertEqual(event.event_time, 7200)
            self.assertEqual(event.event_type, 'CASH_OUT')
            self.assertEqual(dict(event.features), {'amount': 20, 'oldbalanceOrg': 100, 'oldbalanceDest': 30})
            self.assertEqual([(entity.id, entity.role) for entity in event.entities],
                             [('C1', 'source'), ('C2', 'destination')])
            self.assertEqual(dict(stream.truth_for([event.event_id])[event.event_id].diagnostics),
                             {'newbalanceOrig': 80, 'newbalanceDest': 50, 'isFlaggedFraud': 0})

    def test_invalid_source_values_are_rejected_with_row_context(self):
        for column, value in [('TX_TIME_SECONDS', 'nan'), ('TX_AMOUNT', '-1'),
                              ('CUSTOMER_ID', ''), ('TX_FRAUD', '2')]:
    ...
    def test_source_selection_and_iteration_options_validate(self):
        config = self.handbook()
        for change in ({'dataset': 'other'}, {'paths': ['handbook.csv']},
                       {'currency': ''}):
        ...
        with open_source(config, self.base) as stream:
            ...
            for bounds in ({'start': float('nan')}, {'stop': True}, {'start': 20, 'stop': 10}):
                with self.subTest(bounds=bounds), self.assertRaises(ValueError):
                    list(stream.iter_events(**bounds))

    def test_catalog_and_invalid_prepared_store(self):
        self.assertEqual({item['dataset_id'] for item in source_catalog()}, {'handbook', 'ulb', 'paysim'})
    ...
    def test_prepared_outcome_identity_and_availability_corruption(self):
        for mutation in ("UPDATE outcomes SET event_id='extra' WHERE event_id='late'",
    ...
    def test_extension_invariants_are_checked_before_writing(self):
        ...
        mutations = [
            ...
            lambda event, truth: (replace(event, features={'amount': float('nan')}), truth),
            lambda event, truth: (replace(event, features={'wrong-feature': 1}), truth),
            lambda event, truth: (replace(event, entities=(EntityRef('customer', '1', 'bad-role'),)), truth),
        ...
        for mutation in mutations:
            with self.subTest(mutation=mutation):
                ...
                with patch.dict(ADAPTERS['handbook'], {'parse': parse}), self.assertRaises(ValueError):
                    open_source(config, self.base)
    ...
    def test_nested_diagnostics_are_immutable_and_roundtrip(self):
        ...
        def parse(row, generated_id):
            ...
            return event, Outcome(truth.event_id, truth.label,
                                  diagnostics={'nested': {'values': [1, 2]}})
        with patch.dict(ADAPTERS['handbook'], {'parse': parse}), open_source(config, self.base) as stream:
    ...
    def test_extension_recorded_availability_updates_descriptor(self):
        config = self.handbook()
        original = ADAPTERS['handbook']['parse']
        for available_ids, expected in (({'late', 'early-a'}, 'as-of'),
                                        ({'late'}, 'mixed'), (set(), 'retrospective')):
            with self.subTest(expected=expected):
                ...
                with patch.dict(ADAPTERS['handbook'], {'parse': parse}), open_source(config, self.base) as stream:
                    ...
                    self.assertEqual(stream.provenance['dataset_descriptor']['label_availability'], expected)
                    self.assertIsNone(stream.truth_for(['early-b'])['early-b'].available_at)
