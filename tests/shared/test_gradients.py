...
for design in DESIGNS:
    if DESIGNS[design].get("family") in ("xgboost", "temporal_family"):
        continue
    m = Model(design=design)
    loss, _ = run(m, data, True)
    loss.backward()
    errors = {}
    for name in ["Wz", "Wn", "edge0", "q1", "rq", "pair", "amount", "gap"]:
        if name not in m.p:
            continue
        p = m.p[name]
        idx = np.unravel_index(np.argmax(np.abs(p.g)), p.g.shape)
        analytic = p.g[idx]
        old = p.d[idx]
        eps = 1e-5
        p.d[idx] = old + eps
        plus = run(m, data, True)[0].d
        p.d[idx] = old - eps
        minus = run(m, data, True)[0].d
        p.d[idx] = old
        err = abs((plus - minus) / (2 * eps) - analytic)
