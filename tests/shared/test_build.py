    ...
    def test_registered_outputs_are_self_contained(self):
        ...
        for tool in registry["tools"]:
            with self.subTest(tool=tool["id"]):
                ...
                self.assertTrue(
                    all("src" not in script["attrs"] for script in page.scripts)
                )
                payloads = [
                    script
                ...
                wanted = (
                    [design["id"] for design in catalog["designs"]]
                ...
                if tool.get("explanation"):
                    self.assertEqual(
                        payload["explanation"],
                        json.loads((ROOT / tool["explanation"]).read_text()),
                    )
                if tool.get("native_models"):
                    self.assertEqual(
                        [model["id"] for model in payload["native_models"]],
                        tool["native_models"],
                    )
