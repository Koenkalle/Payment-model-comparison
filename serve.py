...
from framework.comparison_service import (
    ComparisonService,
    DEFAULT_ARTIFACT,
    DEFAULT_SUPERVISED_ARTIFACT,
    make_server,
)
...
def main():
    ...
    parser.add_argument(
        "--supervised-artifact",
        type=Path,
        default=DEFAULT_SUPERVISED_ARTIFACT,
    ...
    try:
        ...
        replay_configs = [
            path
            for path in args.dataset_config
        ...
        server = make_server(
            ...
            args.host,
            args.port,
            dataset_service=DatasetService(replay_configs),
    ...
    print(
        f"Model trainer: http://{address}:{server.server_port}/trainer.html", flush=True
    )
    print(
        f"Saved datasets and experiments: {pipeline.root}. Ctrl+C stops the server.",
        flush=True,
    )
    try:
