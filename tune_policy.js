...
'use strict';
const fs=require('fs'),path=require('path'),crypto=require('crypto'),core=require('./model'),policy=require('./policy');
const root=__dirname,read=name=>fs.readFileSync(path.join(root,name),'utf8'),sha=text=>crypto.createHash('sha256').update(text).digest('hex');
function sourceFiles(){return ['tune_policy.js','policy.js','model.js','model-adapters.js','designs.json','xgb-training.json',...JSON.parse(read('designs.json')).designs.map(d=>'models/'+d.id+'.json')];}
