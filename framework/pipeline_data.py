...
_LABELS = {'handbook': 'Fraud Detection Handbook', 'ulb': 'ULB credit-card fraud', 'paysim': 'PaySim'}
_REPLAY_NOTE = ('Selected columns are shared by numeric models and graph edge attributes. '
                'Payment replay retains the original events and amounts; browser policies use their own event inputs.')
_GENERATORS = [
    {'id': 'synthetic_payments', 'label': 'Payment scenarios',
     ...
     'parameters': [
         {'name': 'name', 'label': 'Scenario', 'type': 'select', 'default': 'mixed',
          'options': [{'value': value, 'label': label} for value, label in (
              ('mixed', 'Mixed week with a traffic surge'), ('relay', 'Collection and long relay'),
              ('split', 'Split, merge, and cycle'), ('takeover', 'Account takeover'),
              ('benign', 'Busy merchants and payroll'))]},
         {'name': 'size', 'label': 'Size', 'type': 'select', 'default': 'medium',
          'options': [{'value': value, 'label': value.title()} for value in ('small', 'medium', 'large')]},
         {'name': 'seed', 'label': 'Random seed', 'type': 'integer', 'default': 42, 'min': 0, 'max': 4294967295},
         {'name': 'reportDelay', 'label': 'Report delay (minutes)', 'type': 'number', 'default': 1440, 'min': 0, 'max': 525600},
         {'name': 'forwardDelay', 'label': 'Forwarding delay (minutes)', 'type': 'number', 'default': 2, 'min': 0, 'max': 1440},
     ]},
    {'id': 'handbook_generator', 'label': 'Configurable customer and terminal payments',
     ...
     'parameters': [
         {'name': 'customer_count', 'label': 'Customers', 'type': 'integer', 'default': 24, 'min': 2, 'max': 10000},
         {'name': 'terminal_count', 'label': 'Terminals', 'type': 'integer', 'default': 12, 'min': 1, 'max': 10000},
         {'name': 'transactions', 'label': 'Transactions', 'type': 'integer', 'default': 1000, 'min': 20, 'max': 100000},
         {'name': 'duration_days', 'label': 'Duration (days)', 'type': 'number', 'default': 14, 'min': 0.01, 'max': 3650},
         {'name': 'fraud_rate', 'label': 'Fraud fraction', 'type': 'number', 'default': 0.1, 'min': 0, 'max': 1},
         {'name': 'seed', 'label': 'Random seed', 'type': 'integer', 'default': 42, 'min': 0, 'max': 4294967295},
     ]},
]
...
def _selected_payments(document, bounds):
    ...
    document['truth'] = {key: value for key, value in document['truth'].items() if key in ids}
    document['label_available_at'] = {key: value for key, value in document.get('label_available_at', {}).items() if key in ids}
    return document


def _name(value, field='name'):
    if not isinstance(value, str) or not value.strip() or len(value) > 200:
        raise ValueError(field + ' must be a nonempty string of at most 200 characters.')
    return value.strip()
...
def _public_provenance(value):
    ...
    if isinstance(value, dict):
        return {key: _public_provenance(item) for key, item in value.items()
...
def _statistics(values):
    ...
    count = len(finite)
    result = {'count': count, 'missing': len(values) - count, 'unique': int(len(np.unique(finite))),
              'min': None, 'max': None, 'mean': None, 'std': None,
              'quantiles': {'p25': None, 'p50': None, 'p75': None}, 'histogram': []}
    if not count:
        return result
    ...
    result.update(min=float(finite.min()), max=float(finite.max()),
                  mean=number(precise.mean()), std=number(precise.std()))
    result['quantiles'] = {key: number(value) for key, value in
                           zip(('p25', 'p50', 'p75'), np.quantile(precise, [.25, .5, .75]))}
    lower, upper = precise.min(), precise.max()
    if lower == upper:
        result['histogram'] = [{'min': float(lower), 'max': float(upper), 'count': count}]
    else:
        bins = min(10, result['unique'])
        indices = np.minimum(((precise - lower) / (upper - lower) * bins).astype(np.int64), bins - 1)
        counts = np.bincount(indices, minlength=bins)
        edges = lower + np.linspace(0, 1, bins + 1, dtype=np.longdouble) * (upper - lower)
        result['histogram'] = [{'min': number(edges[index]), 'max': number(edges[index + 1]),
                                'count': int(counts[index])} for index in range(bins)]
    return result
...
class DatasetStore:
    def __init__(self, root, config_paths=()):
        ...
        self._bootstrap('handbook-fixture-v1', ROOT / 'examples/datasets/handbook-payments.config.json')
    ...
    def _directory(self, identifier):
        ...
        if path.is_symlink() or not path.is_dir() or not (path / 'metadata.json').is_file():
            raise ValueError('Unknown dataset ID.')
        return path
    ...
    def catalog(self):
        ...
        with self._lock:
            for path in self.directory.glob('ds-*'):
                if _ID.fullmatch(path.name) and not path.is_symlink() and (path / 'metadata.json').is_file():
                    entries.append(self.describe(self.get(path.name)))
        sources = [
            {'id': source['dataset_id'], 'label': _LABELS[source['dataset_id']] + ' CSV',
             'description': ('Numeric transactions with 28 anonymized features; no payment account identities.'
                             if not source['entity_types'] else 'Store a CSV snapshot. Supply its amount-to-EUR conversion to enable payment replay and fraud graph training.'),
             'requires_conversion': bool(source['entity_types']),
             'views': ['numeric', 'graph', 'labeled_graph', 'payments'] if source['entity_types'] else ['numeric']}
            for source in source_catalog()]
        return {'datasets': sorted(entries, key=lambda entry: entry['created_at'], reverse=True),
                'generators': self.generators(), 'sources': sources,
                'limits': {'max_upload_bytes': MAX_UPLOAD_BYTES, 'max_events': MAX_EVENTS, 'max_accounts': MAX_ACCOUNTS}}
    ...
    def describe(self, metadata):
        ...
        if metadata.get('kind') != 'features':
            definitions = [source_definition(name, metadata.get('currency'))
                           for name in metadata.get('feature_names', [])]
        else:
            catalog = {}
            cache = self._directory(metadata['id']) / ('.features-' + _hash(metadata['feature_recipe_version'])[:16])
            try:
                if cache.is_symlink() or any((cache / name).is_symlink()
            ...
            for name in metadata.get('feature_names', []):
                if name in catalog:
                    ...
                    definitions.append({'id': name, 'label': name, 'kind': 'derived',
                        ...
                        'info': {'sections': [{'title': 'Saved definition unavailable', 'text':
                            'The original feature documentation could not be read. Open this dataset '
    ...
    def _config(self, identifier):
        ...
        if metadata.get('config_sha256') and _file_hash(path / 'config.json') != metadata['config_sha256']:
            raise ValueError('Stored dataset configuration checksum changed. Import a new dataset snapshot.')
        config = json.loads((path / 'config.json').read_text(encoding='utf-8'))
        filename = config['path']
        # Internal snapshots use only flat filenames generated by this store.
        if filename not in ('transactions.sqlite', 'payments.json') or (path / filename).is_symlink():
            raise ValueError('Invalid stored dataset payload.')
        if metadata.get('payload_sha256') and _file_hash(path / filename) != metadata['payload_sha256']:
            raise ValueError('Stored dataset payload checksum changed. Import a new dataset snapshot.')
    ...
    def source_config(self, identifier, view='numeric'):
        ...
        if view not in metadata['views']:
            raise ValueError('Dataset does not support the ' + str(view) + ' view.'
                             + (' Supply an explicit amount-to-EUR conversion when importing this dataset.'
                                if metadata.get('requires_conversion') and view in ('payments', 'labeled_graph') else ''))
    ...
    def dataset_config(self, identifier, view='numeric'):
        ...
        return {'loader': 'dataset_features', 'view': view, 'path': str(path),
                'payload_sha256': metadata['feature_payload_sha256'], 'dataset': source,
    ...
    def _feature_cache(self, identifier):
        ...
        if not cache.exists():
            if metadata.get('kind') == 'features':
                raise ValueError('This saved dataset is missing its original feature recipe cache. Restore the dataset snapshot.')
            temporary = directory / ('.calculating-features-' + uuid.uuid4().hex)
            temporary.mkdir()
            try:
                numeric, definitions = prepare_features(config, directory)
                numeric = NumericDataset(numeric.ids, numeric.times, numeric.features, numeric.labels,
                                         numeric.feature_names, _public_provenance(numeric.provenance))
                write_numeric(temporary / 'columns.npz', numeric)
                columns = {name: index for index, name in enumerate(numeric.feature_names)}
                manifest = {'source_dataset_id': source_id, 'source_fingerprint': source_fingerprint,
                            'recipe_version': recipe_version, 'row_count': len(numeric.ids),
                            'features': [{**definition,
                                          'statistics': _statistics(numeric.features[:, columns[definition['id']]])
                                          if definition['available'] else None}
                                         for definition in definitions]}
                _json(temporary / 'manifest.json', manifest)
                _json(temporary / 'checksums.json', {
        ...
        if (not isinstance(checksums, dict)
                or any(not isinstance(checksums.get(key), str) or not re.fullmatch(r'[0-9a-f]{64}', checksums[key])
        ...
        if _file_hash(cache / 'manifest.json') != checksums.get('manifest_sha256'):
            raise ValueError('Stored feature cache metadata checksum changed. Create a new dataset snapshot.')
        manifest = json.loads((cache / 'manifest.json').read_text(encoding='utf-8'))
        if (manifest.get('recipe_version') != recipe_version
                or manifest.get('source_fingerprint') != source_fingerprint):
            raise ValueError('Stored feature cache does not match this dataset snapshot.')
    ...
    def features(self, identifier):
        ...
        with self._lock:
            metadata = self.get(identifier)
            self.dataset_config(identifier)  # Also verify a saved variant's feature checksum.
            _, manifest = self._feature_cache(identifier)
            enabled = metadata['feature_names']
            return {**manifest, 'dataset_id': identifier, 'fingerprint': metadata['fingerprint'],
                    'latest_recipe_version': RECIPE_VERSION,
                    'enabled_features': list(enabled), 'replay_note': _REPLAY_NOTE,
                    'features': [{**describe_feature(definition), 'enabled': definition['id'] in enabled}
    ...
    def save_features(self, identifier, payload):
        ...
        if not isinstance(payload, dict) or set(payload) - {'features', 'name'}:
            raise ValueError('Feature selection accepts only features and an optional dataset name.')
        names = payload.get('features')
        if (not isinstance(names, list) or not names or not all(isinstance(name, str) for name in names)
                or len(set(names)) != len(names)):
            raise ValueError('features must list nonempty, distinct available feature IDs.')
        with self._lock:
            ...
            definitions = {definition['id']: definition for definition in manifest['features']}
            if any(name not in definitions or not definitions[name]['available'] for name in names):
                raise ValueError('Select available observable feature IDs from the dataset feature catalog.')
            indices = [numeric.feature_names.index(name) for name in names]
            values = numeric.features[:, indices].copy()
            recipe = [{'id': name, 'version': definitions[name]['version']} for name in names]
            fingerprint = _hash({'source_fingerprint': manifest['source_fingerprint'],
                                 'recipe_version': manifest['recipe_version'], 'features': recipe,
                                 'values_sha256': hashlib.sha256(values.tobytes()).hexdigest()})
            selected = NumericDataset(numeric.ids, numeric.times, values, numeric.labels, tuple(names),
                                      {**numeric.provenance, 'feature_recipe_version': manifest['recipe_version'],
                                       'dataset_feature_fingerprint': fingerprint, 'features_stored': True})
            original_directory = self._directory(identifier)
            source_config = json.loads((original_directory / 'config.json').read_text(encoding='utf-8'))
            with self._new() as (new_id, destination):
                shutil.copyfile(original_directory / source_config['path'], destination / source_config['path'])
                cache_name = '.features-' + _hash(manifest['recipe_version'])[:16]
                shutil.copytree(original_directory / cache_name, destination / cache_name)
                _json(destination / 'config.json', source_config)
                write_numeric(destination / 'features.npz', selected)
                metadata = {**copy.deepcopy(parent), 'id': new_id, 'name': name, 'kind': 'features',
                            'created_at': datetime.now(timezone.utc).isoformat(),
                            'parent_dataset_id': identifier, 'source_dataset_id': manifest['source_dataset_id'],
                            'source_fingerprint': manifest['source_fingerprint'], 'fingerprint': fingerprint,
                            'feature_names': list(names), 'feature_recipe': recipe,
                            'feature_recipe_version': manifest['recipe_version'], 'feature_replay_note': _REPLAY_NOTE,
                            ...
                            'provenance': {**parent.get('provenance', {}),
                                           ...
                                           'features_stored': True, 'outcomes_used_for_features': False}}
    ...
    def _bootstrap(self, registration, path):
        with self._lock:
            ...
            if not isinstance(config, dict) or config.get('loader') not in ('fraud_dataset', 'prepared_fraud', 'payment_json'):
                raise ValueError('Registered web datasets require fraud_dataset, prepared_fraud or payment_json configurations.')
            with self._new() as (identifier, destination):
                self._snapshot(identifier, destination, config, path.parent, kind='fixed', parameters={})
                _json(destination / 'registration.json', {'key': registration})

    def _snapshot(self, identifier, destination, config, base, *, kind, parameters, generator=None):
        ...
        if config['loader'] == 'payment_json':
            ...
            if config['loader'] == 'fraud_dataset':
                ...
                preparation = {key: value for key, value in config.items() if key not in ('path', 'paths', 'selection')}
                preparation['paths'] = copied
                with prepare_source(preparation, destination, output):
                    pass
            else:
                source = Path(config['path'])
                shutil.copyfile(source if source.is_absolute() else base / source, output)
            stored = {key: value for key, value in config.items()
        ...
        metadata.update(id=identifier, name=config.get('name', metadata.get('name', 'Stored dataset')),
                        kind=kind, created_at=datetime.now(timezone.utc).isoformat(),
                        generator=generator, parameters=copy.deepcopy(parameters))
        if generator:
    ...
    def _stream_metadata(stream, config):
        digest = hashlib.sha256()
        counts = {'rows': 0, 'payment_rows': 0, 'known': 0, 'fraud': 0, 'legitimate': 0, 'unknown': 0}
        ...
        for batch in stream.iter_batches(512, **_selection(config)):
            ...
            for event in batch:
                ...
                counts['rows'] += 1
                counts['payment_rows'] += 1  # Each source transaction becomes a payment candidate.
                ...
                references = [(entity.kind, entity.id, entity.role) for entity in event.entities]
                entities.update((entity.kind, entity.id) for entity in event.entities)
                digest.update(_hash([event.event_id, event.event_time, event.event_type, references,
                                     dict(event.features), outcome.label, outcome.available_at]).encode())
        ...
        conversion = config.get('amount_to_eur', 1. if descriptor.get('currency') == 'EUR' else None)
        views = ['numeric'] + (['graph'] if has_graph else [])
        if has_graph and conversion is not None:
            views += ['labeled_graph', 'payments']
        fingerprint = _hash({'observations_and_labels': digest.hexdigest(), 'features': descriptor['feature_names'],
                             'currency': descriptor.get('currency'), 'amount_to_eur': conversion})
        return {**counts, 'name': descriptor['name'], 'schema': stream.schema, 'views': views,
                'source': descriptor['dataset_id'], 'source_namespace': [descriptor['dataset_id'], descriptor['version']],
                'accounts': len(entities), 'time_range': {'start': start, 'end': end, 'unit': 'seconds'},
                'feature_names': list(config.get('features', descriptor['feature_names'])),
                'currency': descriptor.get('currency'), 'amount_to_eur': conversion,
                'requires_conversion': has_graph and conversion is None,
                'fingerprint': fingerprint, 'selection': config.get('selection', {}),
    ...
    def _payment_metadata(document):
        ...
        fingerprint = _hash({key: document.get(key) for key in ('accounts', 'events', 'truth', 'label_available_at', 'units')})
        return {'name': document['name'], 'schema': 'payment-events/v1',
                'source': 'payment_json', 'source_namespace': fingerprint,
                'views': ['numeric', 'graph', 'labeled_graph', 'payments'],
                'rows': len(document['events']), 'payment_rows': len(payments), 'known': len(truth),
                'fraud': sum(bool(value) for value in truth.values()),
                'legitimate': sum(not value for value in truth.values()), 'unknown': len(payments) - len(truth),
                'accounts': len(document['accounts']),
                'time_range': {'start': payments[0]['t'] * 60, 'end': payments[-1]['t'] * 60, 'unit': 'seconds'},
                'feature_names': ['log1p_amount', 'prior_source_count', 'prior_destination_count'],
                'currency': 'EUR', 'amount_to_eur': 1., 'requires_conversion': False,
                'fingerprint': fingerprint, 'selection': {},
                'provenance': _public_provenance(document.get('provenance', {}))}

    def generate(self, payload):
        if not isinstance(payload, dict) or set(payload) - {'generator', 'name', 'parameters'}:
            raise ValueError('Choose a generator, optional dataset name and generator parameters.')
        schema = next((entry for entry in _GENERATORS if entry['id'] == payload.get('generator')), None)
        if schema is None:
        ...
        for key, definition in definitions.items():
            ...
            elif (type(value) not in ((int,) if definition['type'] == 'integer' else (int, float))
                  or not math.isfinite(value) or not definition['min'] <= value <= definition['max']):
                raise ValueError(f'{key} must be a {definition["type"]} between {definition["min"]} and {definition["max"]}.')
            parameters[key] = value
        name = _name(payload.get('name', schema['label'] + ' · seed ' + str(parameters['seed'])))
        with self._lock, self._new() as (identifier, destination):
            if schema['id'] == 'synthetic_payments':
                dataset = load_dataset({'loader': 'synthetic_payments', 'options': parameters})
                ...
                namespace = 'synthetic-' + _hash({key: parameters[key] for key in ('seed', 'size')})[:24]
                ...
                for event in document['events']:
                    ...
                    if event['kind'] == 'report' and reference in document['truth']:
                        confirmed[reference] = min(confirmed.get(reference, event['t']), event['t'])
                if confirmed:
                    document['label_available_at'] = confirmed
                _json(destination / 'generated.json', document)
                config = {'loader': 'payment_json', 'path': 'generated.json', 'name': name}
            else:
                self._handbook_csv(destination / 'generated.csv', parameters)
                config = {'loader': 'fraud_dataset', 'dataset': 'handbook', 'path': 'generated.csv',
                          'name': name, 'currency': 'EUR', 'amount_to_eur': 1.,
                          'release': 'invented-' + _hash({key: parameters[key] for key in
                                                       ('seed', 'customer_count', 'terminal_count')})[:24]}
            metadata = self._snapshot(identifier, destination, config, destination,
                                      kind='generator', parameters=parameters, generator=schema['id'])
            (destination / ('generated.json' if schema['id'] == 'synthetic_payments' else 'generated.csv')).unlink()
            return metadata
    ...
    def _handbook_csv(path, parameters):
        ...
        with path.open('w', newline='', encoding='utf-8') as handle:
            ...
            writer.writerow(['TRANSACTION_ID', 'TX_TIME_SECONDS', 'CUSTOMER_ID', 'TERMINAL_ID', 'TX_AMOUNT', 'TX_FRAUD'])
            for index in range(count):
                fraud = int(index in frauds)
                amount = round(math.exp(rng.gauss(4.9 if fraud else 3.3, 0.8)), 2)
                writer.writerow([f'TX{index:07d}', round((index + rng.random()) * duration / count, 6),
                                 rng.randrange(parameters['customer_count']), rng.randrange(parameters['terminal_count']),
                                 amount, fraud])

    def import_source(self, payload):
        allowed = {'source', 'csv', 'name', 'release', 'amount_to_eur'}
        if not isinstance(payload, dict) or set(payload) - allowed:
            raise ValueError('Import accepts a source adapter, CSV contents, name, release and amount-to-EUR conversion.')
        ...
        if not isinstance(text, str) or not text.strip() or len(text.encode('utf-8')) > MAX_UPLOAD_BYTES:
            raise ValueError('Upload a nonempty CSV of at most 6 MiB, or register a larger dataset at server startup.')
        config = {'loader': 'fraud_dataset', 'dataset': source, 'path': 'upload.csv',
        ...
        with self._lock, self._new() as (identifier, destination):
            ...
            metadata = self._snapshot(identifier, destination, config, destination, kind='fixed', parameters={})
            (destination / 'upload.csv').unlink()
            return metadata

    def document(self, identifier, selection=None):
        config = self.dataset_config(identifier, 'payments')
        bounds = _selection({'selection': selection}) if selection is not None else _selection(config)
        if config['loader'] == 'prepared_fraud':
            ...
            with open_prepared(config['path']) as stream:
                for event in stream.iter_events(**bounds):
                    ...
                    if count > MAX_EVENTS or len(entities) > MAX_ACCOUNTS:
                        raise ValueError('This interval exceeds 20,000 events or 256 accounts. Select a smaller time interval; no rows were silently dropped.')
            document = load_dataset(config).document
        else:
            document = _selected_payments(load_dataset(config).document, bounds)
            active = sorted({event[side] for event in document['events'] for side in ('u', 'v') if event[side] >= 0})
            if len(document['events']) > MAX_EVENTS or len(active) > MAX_ACCOUNTS:
                raise ValueError('This interval exceeds 20,000 events or 256 accounts. Select a smaller time interval; no rows were silently dropped.')
            mapping = {old: index for index, old in enumerate(active)}
            document['accounts'] = [{**document['accounts'][old], 'id': mapping[old]} for old in active]
            for event in document['events']:
        ...
        return {'dataset': document, 'summary': {'events': len(document['events']), 'payments': payments,
                'accounts': len(document['accounts']), 'known': len(document['truth']),
                'unknown': payments - len(document['truth']), 'source': self.get(identifier)['source'],
                'dataset_id': identifier, 'time_unit': 'minutes'}}
    ...
    def sample(self, identifier, max_rows=20):
        ...
        if config['loader'] == 'prepared_fraud':
            ...
            with open_prepared(config['path']) as stream:
                ...
                rows = [{'id': event.event_id, 'time': event.event_time,
                         'features': dict(zip(names, values[index].tolist())), 'label': labels[event.event_id].label}
                        for index, event in enumerate(events)]
        else:
            data = load_dataset(config)
            names = data.feature_names
            rows = [{'id': data.ids[index], 'time': float(data.times[index]),
                     'features': dict(zip(names, data.features[index].tolist())), 'label': int(data.labels[index])}
                    for index in range(min(max_rows, len(data.ids)))]
        return {'columns': list(names), 'rows': rows, 'time_unit': 'seconds',
