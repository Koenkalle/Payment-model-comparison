...
def fingerprint(dataset):
    ...
    hasher.update(json.dumps({'ids': dataset.ids, 'nodes': dataset.node_ids,
                             'node_features': dataset.node_feature_names,
                             'edge_features': dataset.edge_feature_names,
                             'provenance': dataset.provenance}, sort_keys=True).encode())
    for array in (dataset.times, dataset.sources, dataset.destinations, dataset.node_features, dataset.edge_features):
...
def report(model, descriptor, dataset, indices, threshold, seed, partition):
    ...
    metrics = {'observed_links': len(indices), 'paired_links': int(valid.sum()),
               'unpaired_links': int((~valid).sum()), 'negative_links': int(valid.sum()),
               'average_precision': float(average_precision_score(labels, scores)) if len(labels) else None,
               'roc_auc': float(roc_auc_score(labels, scores)) if len(labels) else None,
               'accuracy': float(np.mean((scores > threshold) == labels)) if len(labels) else None,
               'log_loss': float(np.mean(np.logaddexp(0, logits) - labels * logits)) if len(labels) else None}
    rows = []
    for offset, i in enumerate(indices):
        negative = int(prediction['negative_destinations'][offset])
        rows.append({'id': dataset.ids[i], 'source': dataset.node_ids[dataset.sources[i]],
                     'destination': dataset.node_ids[dataset.destinations[i]],
                     'elapsed_seconds': float(dataset.times[i]),
                     'link_probability': float(probabilities(prediction['positive_logits'][offset])),
                     'link_logit': float(prediction['positive_logits'][offset]),
                     'negative_destination': dataset.node_ids[negative] if negative >= 0 else None,
                     'negative_probability': float(probabilities(prediction['negative_logits'][offset])) if negative >= 0 else None})
    return {'version': 1, 'schema': 'temporal-link-evaluation/v1', 'task': 'dynamic-link-prediction',
            'model_id': descriptor['id'], 'partition': partition, 'threshold': float(threshold),
            'negative_sampling': {'strategy': 'causal-random-destination', 'seed': seed, 'per_positive': 1,
            ...
            'dataset': dataset.provenance, 'metrics': metrics, 'rows': rows}
...
def train(config, output, dataset):
    ...
    elif not isinstance(threshold, (int, float)) or not np.isfinite(threshold) or not 0 <= threshold <= 1:
        raise ValueError('decision_threshold must be between zero and one.')
    results = report(model, descriptor, dataset, splits['test'], threshold, evaluation_seed, 'test')
    results['training_rows_in_evaluation'] = 0
    metadata = {'version': 1, 'task': 'dynamic-link-prediction', 'input_schema': dataset.schema,
                'model_id': descriptor['id'], 'implementation': descriptor, 'implementation_sha256': source_hashes(descriptor),
                'library_version': model.library_version, 'parameters': model.parameters,
                'features': model.feature_schema, 'dataset': dataset.provenance, 'dataset_fingerprint': fingerprint(dataset),
                'split': {name: [dataset.ids[i] for i in indices] for name, indices in splits.items()},
                'threshold': float(threshold), 'threshold_source': 'validation_f1' if config.get('decision_threshold') is None else 'configured',
                'evaluation_seed': evaluation_seed, 'best_epoch': model.best_epoch, 'training_history': model.training_history,
                'model_file': 'model.npz'}
    output.parent.mkdir(parents=True, exist_ok=True)
    with tempfile.TemporaryDirectory(prefix='.payment-graph-', dir=output.parent) as temporary:
...
def evaluate(artifact, dataset_config, base_dir=None, partition='all'):
    ...
    if partition == 'all':
        ...
        if not same_dataset:
            raise ValueError('Saved split membership requires the original graph dataset and configuration.')
    ...
    result = report(model, descriptor, dataset, indices, metadata['threshold'], metadata['evaluation_seed'], partition)
    trained = set(metadata['split']['train'])
    result['training_rows_in_evaluation'] = sum(dataset.ids[i] in trained for i in indices) if same_dataset else None
    return result
