...
def labeled_payments(events, graph_config=None):
    ...
    if not isinstance(events, EventDataset):
        raise ValueError(
            "Fraud training requires a payment JSON, CSV, or generated event dataset."
        )
    ...
    graph = to_graph(
        EventDataset(document, events.provenance),
        graph_config.get("node_feature_dim", 32),
    )
    truth = document.get("truth", {})
    availability = document.get("label_available_at", {})
    origin = graph.provenance["time_origin_seconds"]
    labels = np.asarray(
        [int(truth[i]) if i in truth else -1 for i in graph.ids], dtype=np.int64
    )
    times = np.asarray(
        [
            availability[i] * 60 - origin if i in availability else np.nan
            for i in graph.ids
        ],
        dtype=np.float64,
    )
    ...
    assumption = (
        "as-of"
        if explicit == known and known
        else ("mixed" if explicit else "retrospective")
    )
    return LabeledTemporalGraphDataset(
        graph,
        labels,
        times,
        {
            **events.provenance,
            ...
            "label_availability": assumption,
            "explicit_label_times": explicit,
...
def load_labeled_dataset(config, base_dir=None, graph_config=None):
    ...
    if config.get("view") == "labeled_graph" and graph_config:
        ...
        if "node_feature_dim" in graph_config:
            if (
                "node_feature_dim" in config
                and config["node_feature_dim"] != graph_config["node_feature_dim"]
            ):
                raise ValueError(
                    "Conflicting dataset and graph node_feature_dim settings."
                )
    ...
    if isinstance(dataset, LabeledTemporalGraphDataset):
        if graph_config:
            if (
                set(graph_config) - {"node_feature_dim"}
                or graph_config.get(
                    "node_feature_dim", dataset.graph.node_features.shape[1]
                )
                != dataset.graph.node_features.shape[1]
            ):
                raise ValueError(
                    "Configure node_feature_dim in the labeled_graph dataset view."
                )
        return dataset
...
def available_labels(dataset, cutoff=None):
    ...
    if cutoff is not None:
        ...
        labels[
            np.isfinite(dataset.label_available_at)
            & (dataset.label_available_at > cutoff)
        ] = -1
    return labels
        ...
        json.dumps(
            {
                ...
                "node_features": graph.node_feature_names,
                "edge_features": graph.edge_feature_names,
                "time_origin_seconds": graph.provenance["time_origin_seconds"],
                "source_time_origin": dataset.provenance.get(
                    "time_origin_unix_or_numeric_seconds"
                ),
            },
            sort_keys=True,
        ).encode()
