...
def chronological_split(dataset, config):
    train_fraction = config.get("train", 0.6)
    validation_fraction = config.get("validation", 0.2)
    if (
    ...
    a = int(len(unique) * train_fraction)
    b = int(len(unique) * (train_fraction + validation_fraction))
    if a < 1 or b <= a or b >= len(unique):
        raise ValueError(
            "Need enough distinct timestamps for nonempty train, validation and test partitions."
        )
    # Keep equal timestamps together; no random split or tie leakage.
    return {
        "train": np.flatnonzero(dataset.times < unique[a]),
        "validation": np.flatnonzero(
            (dataset.times >= unique[a]) & (dataset.times < unique[b])
        ),
        "test": np.flatnonzero(dataset.times >= unique[b]),
    }
...
def metrics(labels, probabilities, threshold):
    known = labels >= 0
    actual = labels[known]
    blocked = probabilities[known] > threshold
    tp = int(np.sum((actual == 1) & blocked))
    fp = int(np.sum((actual == 0) & blocked))
    fn = int(np.sum((actual == 1) & ~blocked))
    tn = int(np.sum((actual == 0) & ~blocked))
    return {
        "rows": len(labels),
        "known": int(known.sum()),
        ...
        "block_rate": float(np.mean(probabilities > threshold))
        if len(labels)
        else None,
    }
...
def choose_threshold(labels, probabilities):
    ...
    if set(y.tolist()) != {0, 1}:
        raise ValueError(
            "Automatic threshold selection needs both known classes in validation; configure decision_threshold for a fixed cutoff."
        )
    ...
    while index < len(y):
        ...
        candidate = (
            f1,
            -index,
            float(p[index]) if index < len(y) else -float(np.finfo(float).eps),
        )
...
def checked_predictions(model, dataset, indices, explain=False):
    ...
    if (
        ...
        raise ValueError(
            "Model must return one finite probability in [0,1] per input row."
        )
    return result
...
def report(model, dataset, indices, threshold, explain=False):
    ...
    for offset, index in enumerate(indices):
        row = {
            ...
            "label": int(dataset.labels[index]) if dataset.labels[index] >= 0 else None,
            "decision": "BLOCK"
            if prediction.probabilities[offset] > threshold
            else "ALLOW",
        }
        if prediction.margins is not None:
            row["margin"] = float(prediction.margins[offset])
        if prediction.contributions is not None:
            row["contributions"] = prediction.contributions[offset, :-1].tolist()
            row["baseline"] = float(prediction.contributions[offset, -1])
        rows.append(row)
    return {
        "version": 1,
        ...
        "metrics": metrics(
            dataset.labels[indices], prediction.probabilities, threshold
        ),
        "rows": rows,
    }
...
def train_experiment(config, output, base_dir=None):
    ...
    if dataset.schema == "transaction-stream/v1":
        dataset.close()
        raise ValueError(
            'Choose dataset view="numeric" for tabular models or view="graph" for link models; use the replay API for streams.'
        )
    if config.get("task") == "dynamic-link-prediction" and not isinstance(
    ...
    if not isinstance(dataset, NumericDataset):
        raise ValueError("The experiment runner requires a numeric feature dataset.")
    ...
    known = training[dataset.labels[training] >= 0]
    if set(dataset.labels[known].tolist()) != {0, 1}:
        raise ValueError("Training requires known examples from both classes.")
    parameters = dict(config.get("parameters", {}))
    model.fit(
        dataset.features[known],
        dataset.labels[known],
    ...
    if threshold is None:
        threshold = choose_threshold(
            dataset.labels[splits["validation"]], validation.probabilities
        )
    elif (
        ...
        raise ValueError("decision_threshold must be a probability between 0 and 1.")
    ...
    metadata = {
        ...
        "library_version": getattr(model, "library_version", None),
        ...
        "validation_metrics": metrics(
            dataset.labels[splits["validation"]], validation.probabilities, threshold
        ),
    }
    ...
    with tempfile.TemporaryDirectory(
        prefix=".payment-experiment-", dir=output.parent
    ) as temporary:
        stage = Path(temporary)
        model.save(stage / "model.json")
        metadata["model_sha256"] = digest(stage / "model.json")
...
def evaluate_artifact(
    artifact, dataset_config, base_dir=None, partition="all", *, evaluation_ids=None
):
    artifact = Path(artifact)
    metadata = json.loads((artifact / "manifest.json").read_text())
    if metadata.get("version") != 1:
        raise ValueError("Unsupported model artifact version.")
    if evaluation_ids is not None and metadata.get("task") in (
        "temporal-fraud-classification",
        "dynamic-link-prediction",
    ):
        raise ValueError(
            "Selected evaluation IDs are supported only for numeric model artifacts."
        )
    ...
    if metadata.get("task") == "dynamic-link-prediction":
        ...
        return evaluate(artifact, dataset_config, base_dir, partition)
    if digest(artifact / "model.json") != metadata["model_sha256"]:
    ...
    if list(dataset.feature_names) != metadata["feature_names"]:
        raise ValueError("Dataset feature names or order differ from the fitted model.")
    implementation = ROOT / (descriptor["python_module"].replace(".", "/") + ".py")
    if digest(implementation) != metadata["implementation_sha256"]:
        raise ValueError(
    ...
    if partition == "all":
        ...
        if (
            "prepared_sha256" in dataset.provenance
            or "prepared_sha256" in metadata["dataset"]
        ):
            identity_keys.append("prepared_sha256")
        if any(
            dataset.provenance.get(key) != metadata["dataset"].get(key)
            for key in identity_keys
        ):
            raise ValueError(
                "Saved split membership applies only to the original dataset."
            )
        wanted = set(metadata["split"][partition])
        indices = np.asarray(
            [i for i, value in enumerate(dataset.ids) if value in wanted]
        )
        if len(indices) != len(wanted):
            raise ValueError("Saved partition IDs are missing from this dataset.")
    else:
        raise ValueError("Unknown evaluation partition.")
    if evaluation_ids is not None:
        if isinstance(evaluation_ids, (str, bytes)):
            raise ValueError("Selected evaluation IDs must be a collection of row IDs.")
        wanted = set(evaluation_ids)
        available = {dataset.ids[i] for i in indices}
        if not wanted or not wanted <= available:
            raise ValueError(
                "Selected evaluation IDs are missing from the chosen dataset partition."
            )
        indices = np.asarray(
            [i for i in indices if dataset.ids[i] in wanted], dtype=int
        )
    ...
    result = report(
        ...
        indices,
        metadata["threshold"],
        bool(descriptor["capabilities"].get("explanations")),
    )
    result["partition"] = partition
    same_source = (
        dataset.provenance["source_sha256"] == metadata["dataset"]["source_sha256"]
    )
    trained = set(metadata["split"]["train"])
    result["training_rows_in_evaluation"] = (
        sum(dataset.ids[i] in trained for i in indices) if same_source else None
    )
    return result
