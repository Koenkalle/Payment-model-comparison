...
class PreparedStreamProvider:
    ...
    def populate(self, config, writer):
        with open_prepared(config["path"]) as stream:
            ...
            if not graph:
                raise ValueError(
                    "This dataset has no entity relationships for a graph view."
                )
            for batch in stream.iter_batches(512, **_selection(config)):
                ...
                for event in batch:
                    ...
                    writer.edge(
                        GraphEdge(
                            event.event_id,
                            node_id(source.kind, source.id),
                            node_id(target.kind, target.id),
                            event.event_type,
                            event.event_time,
                            outcome.label,
                            properties,
...
class PaymentDocumentProvider:
    ...
    def populate(self, config, writer):
        ...
        for account in document["accounts"]:
            ...
            writer.node(
                GraphNode(
                    identifier, str(account.get("name", external_id)), kind, properties
                )
            )
        ...
        for event in document["events"]:
            ...
            properties = {
                ...
                "outcome": {
                    "label": label,
                    "available_at": None if confirmed is None else confirmed * 60,
                },
            }
            writer.edge(
                GraphEdge(
...
PROVIDERS = {
    "prepared_fraud": PreparedStreamProvider,
    "payment_json": PaymentDocumentProvider,
}
...
class _IndexWriter:
    ...
    def node(self, node):
        self.database.execute(
            "INSERT OR IGNORE INTO nodes VALUES (?,?,?,?,?,?,0,0)",
            (
                node.id,
                node.label,
                node.type,
                ...
                node.label.casefold(),
                str(node.properties.get("external_id", node.label)).casefold(),
            ),
        )
    ...
    def edge(self, edge):
        ...
        self.database.execute(
            "UPDATE nodes SET out_degree=out_degree+1 WHERE id=?", (edge.source,)
        )
        self.database.execute(
            "UPDATE nodes SET in_degree=in_degree+1 WHERE id=?", (edge.target,)
        )
        if edge.source == edge.target:
            self.database.execute(
                "INSERT INTO adjacency VALUES (?,?,1,1)", (edge.source, self.sequence)
            )
        else:
            self.database.execute(
                "INSERT INTO adjacency VALUES (?,?,0,1)", (edge.source, self.sequence)
            )
            self.database.execute(
                "INSERT INTO adjacency VALUES (?,?,1,0)", (edge.target, self.sequence)
            )
...
def _query(payload):
    ...
    for field in ("start", "stop"):
        if field in query:
            try:
                finite = type(query[field]) in (float, int) and math.isfinite(
                    query[field]
                )
...
class DatasetGraphService:
    ...
    def _config(self, identifier):
        ...
        signature = tuple(
            (name, self._signature(directory / name))
            for name in (
                "metadata.json",
                "config.json",
                "transactions.sqlite",
    ...
    def _index(self, identifier):
        with self._lock:
            ...
            if provider is None:
                raise ValueError(
                    "This storage format does not yet provide graph inspection."
                )
            key = _json(
                [
                    INDEX_VERSION,
                ...
                try:
                    with closing(self._connect(path)) as database:
                        row = database.execute(
                            'SELECT value FROM metadata WHERE key="cache_key"'
                        ).fetchone()
            ...
            with closing(sqlite3.connect(temporary)) as database:
                ...
                with database:
                    ...
                    interval = database.execute(
                        "SELECT min(time),max(time) FROM edges"
                    ).fetchone()
                    summary = {
                        "supported": True,
                        "schema": SCHEMA,
    ...
    def query(self, identifier, payload):
        ...
        with closing(self._connect(self._index(identifier))) as database:
            ...
            if len(selected) > query["node_limit"]:
                raise ValueError(
                    "The selected node and existing nodes exceed node_limit."
                )
            self._nodes(
                database, selected
            )  # Reject fabricated identities before scanning relationships.
            ...
            rows = database.execute(
                sql, [*parameters, query["edge_limit"] + 1]
            ).fetchall()
            ...
            for row in rows:
                ...
                edges.append(
                    {
                        key: row[key]
                        for key in ("id", "source", "target", "type", "time", "label")
                    }
                )
            ...
            if (
                ...
                selected.update(
                    (row["id"], None)
                    for row in database.execute(
                        "SELECT id FROM nodes ORDER BY id LIMIT ?",
                        (query["node_limit"],),
                    )
                )
            ...
            return {
                ...
                "page": {
                    "offset": query["offset"],
                    "next_offset": cursor
                    if has_more and cursor > query["offset"]
                    else None,
                    "has_more": has_more,
                },
    ...
    def search(self, identifier, query="", limit=20):
        ...
        with closing(self._connect(self._index(identifier))) as database:
            ...
            exact = database.execute(
                "SELECT * FROM nodes WHERE id=?", (query,)
            ).fetchone()
            if exact:
                found[exact["id"]] = self._node(exact)
            nodes = sorted(
                found.values(), key=lambda node: (node["label"].casefold(), node["id"])
            )
