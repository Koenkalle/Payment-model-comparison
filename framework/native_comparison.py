...
from .fraud_export import (_artifact, _calibration_prefix, _evaluation_ids,
                           _partitions, _payment_signature, _same_graph,
                           BROWSER_MAX_ACCOUNTS, BROWSER_MAX_EVENTS)
...
def frontier(scores, labels):
    ...
    return {'requests': len(rows), 'positives': positives, 'negatives': negatives,
            'unknown': len(rows) - positives - negatives, 'candidates': candidates}


def select_policy(validation, options):
    if validation is None:
        raise ValueError('Historical threshold tuning needs both confirmed fraud and legitimate outcomes in the reserved validation period.')
    selected = None
    automatic = options['decisionPolicy'] == 'auto'
    for tau, fp, fn, blocks in validation['candidates']:
        metrics = _metrics(validation['positives'] - fn, fp, fn, validation['negatives'] - fp)
        value = metrics[options['objective']] if automatic else (
            options['falseBlockCost'] * fp + options['missedFraudCost'] * fn)
        key = (-value if automatic else value, blocks)
        if selected is None or key < selected[0]:
            selected = (key, {'tau': tau, 'fp': fp, 'fn': fn, 'blocks': blocks,
                              'metrics': metrics, 'value' if automatic else 'loss': value})
    result = selected[1]
    result.update({key: validation[key] for key in ('requests', 'positives', 'negatives', 'unknown')})
    result['impliedAlpha'] = result['blocks'] / validation['requests']
    if automatic:
        result.update(objective=options['objective'], candidatesTested=len(validation['candidates']))
    else:
        result['costs'] = {'falseBlock': options['falseBlockCost'], 'missedFraud': options['missedFraudCost']}
    return result
...
def _options(raw, heads, training_mode='unsupervised', default_head=None):
    ...
    options = {
        ...
        'alpha': raw.get('alpha', .02), 'eta': raw.get('eta', .025),
        'warmup': raw.get('warmup', 128), 'manualTau': raw.get('manualTau'),
    ...
    if options['trainingMode'] != training_mode:
        raise ValueError('This native artifact supports ' + training_mode + ' scoring only.')
    ...
    if options['eta'] < 0 or options['falseBlockCost'] <= 0 or options['missedFraudCost'] <= 0:
...
class NativeComparison:
    ...
    def __init__(self, artifact, calibration_config=None, base_dir=None):
        ...
        self.training_mode = 'supervised' if task == 'temporal-fraud-classification' else 'unsupervised'
        ...
        if graph_config.get('loader') != 'payment_graph':
            raise ValueError('Native comparison calibration requires the original payment_graph configuration.')
        ...
        if fingerprint(self.original) != self.metadata.get('dataset_fingerprint'):
            ...
            if (not saved.get('source_sha256')
                    or self.original.provenance.get('source_sha256') != saved['source_sha256']
                    or fingerprint(relocated) != self.metadata.get('dataset_fingerprint')):
                raise ValueError('Calibration must use the original graph dataset and configuration.')
        self.original_events = load_dataset(graph_config['dataset'], base).document
        parts = _partitions(self.metadata, self.original)
        self.reference_indices = _calibration_prefix(self.original, parts['test'], config.get('max_rows', 100))
        self.validation_indices = parts['test'][len(self.reference_indices):]
        self.model, self.descriptor = create_model(self.metadata['model_id'], self.original.schema)
        if source_hashes(self.descriptor) != self.metadata.get('implementation_sha256'):
            raise ValueError('Model implementation changed since this artifact was created.')
        ...
        self.heads = {entry['id']: create_head(entry['id']).fit(reference)
                      for entry in head_manifest()['prediction_heads']}
        truth = self.original_events.get('truth', {})
        labels = [int(truth[self.original.ids[index]]) if self.original.ids[index] in truth else -1
                  for index in self.validation_indices]
        self.frontiers = {identifier: frontier(head.score(historical_logits[self.validation_indices])['score'], labels)
                          for identifier, head in self.heads.items()}
        self.calibration = {
            'ids': [self.original.ids[index] for index in self.reference_indices],
            'count': len(reference), 'partition': 'test-prefix',
            'dataset_fingerprint': self.metadata['dataset_fingerprint'],
            'minimum_tail_probability': 1 / (len(reference) + 1),
            'threshold_validation_ids': [self.original.ids[index] for index in self.validation_indices],
    ...
    def describe(self):
        ...
        return {
            ...
            'supported_policies': ['shared', 'manual'] + (['tuned', 'auto'] if all(self.frontiers.values()) else []),
            'prediction_heads': [{'id': identifier, 'label': labels.get(identifier, identifier)}
    ...
    def _initialize_supervised(self):
        ...
        _, self.descriptor = create_model(self.metadata['model_id'], labeled.schema,
        ...
        self.validation_indices = np.asarray([positions[identifier] for identifier in
                                              self.metadata['split']['policy_validation']], dtype=np.int64)
        historical_logits = self._shadow_logits(self.original)
        ...
        self.frontiers = {head.id: frontier(head.score(historical_logits[self.validation_indices])['score'], labels)}
        self.calibration = {
            'ids': [], 'count': 0, 'partition': 'policy_validation',
            'dataset_fingerprint': self.metadata['dataset_fingerprint'],
            'threshold_validation_ids': [self.original.ids[index] for index in self.validation_indices],
            'threshold_validation_count': len(self.validation_indices),
    ...
    def _evaluation(self, graph, document, options):
        if self.training_mode == 'supervised':
            ...
            reserved = {identifier for partition in ('train', 'model_validation', 'policy_validation')
                        for identifier in self.metadata['split'][partition]}
            used = {_payment_signature(self.original_events, event)
                    for event in self.original_events['events']
                    if event['kind'] == 'payment' and event['id'] in reserved}
            if any(_payment_signature(document, event) in used
                   for event in document['events'] if event['kind'] == 'payment'):
                raise ValueError('Target overlaps historical training, model-validation or policy-validation transactions; use the complete original dataset for held-out test selection or a separate dataset.')
            return list(graph.ids), False
        evaluation, same_dataset = _evaluation_ids(
            self.metadata, self.reference_indices, self.original, self.original_events,
            graph, document)
        if same_dataset and options['decisionPolicy'] in ('tuned', 'auto'):
            raise ValueError('The selected dataset supplied historical threshold-validation outcomes. Use a separate comparison dataset for tuned or automatic policies.')
        if options['decisionPolicy'] in ('tuned', 'auto'):
            validation_ids = {self.original.ids[index] for index in self.validation_indices}
            used = {_payment_signature(self.original_events, event)
                    for event in self.original_events['events']
                    if event['kind'] == 'payment' and event['id'] in validation_ids}
            if any(_payment_signature(document, event) in used
                   for event in document['events'] if event['kind'] == 'payment'):
                raise ValueError('The selected dataset overlaps historical threshold-validation transactions. Use a separate dataset for tuned or automatic policies.')
    ...
    def _graph(self, document):
        ...
        return graph_dataset(
            [event['id'] for event in events], [event['t'] * 60 for event in events],
            [names[event['u']] for event in events], [names[event['v']] for event in events],
            [[np.log1p(event['amount'])] for event in events], ['log1p_amount'],
            {'origin': 'comparison-current-dataset'}, len(self.original.node_feature_names))
    ...
    def compare(self, document, options=None, *, cancelled=lambda: False):
        ...
        if len(document['accounts']) > BROWSER_MAX_ACCOUNTS or len(document['events']) > BROWSER_MAX_EVENTS:
            raise ValueError('Comparison supports at most 256 accounts and 20,000 events.')
        ...
        fit = select_policy(validation, options) if options['decisionPolicy'] in ('tuned', 'auto') else None
        tau = options['manualTau'] if options['decisionPolicy'] == 'manual' else fit['tau'] if fit else None
        policy_state = {
            **{key: options[key] for key in ('decisionPolicy', 'alpha', 'warmup', 'eta', 'mode', 'predictionHead', 'trainingMode')},
            'errorCosts': {'falseBlock': options['falseBlockCost'], 'missedFraud': options['missedFraudCost']},
            'policyFit': fit, 'tau': tau,
        }
        warmup = []
        rows = []
        ...
        def decisions(group, logits):
            ...
            for offset, index in enumerate(group):
                ...
                before = tau
                decision = ('CONTEXT' if context else 'LEARNING' if tau is None else
                            'BLOCK' if score > tau else 'ALLOW')
                ...
                if options['decisionPolicy'] == 'shared' and not context:
                    if decision == 'LEARNING':
                        ...
                        if len(warmup) >= options['warmup']:
                            position = max(0, min(len(warmup) - 1, int(np.ceil((1 - options['alpha']) * len(warmup))) - 1))
                            tau = float(sorted(warmup)[position])
                    else:
                        tau += options['eta'] * (int(decision == 'BLOCK') - options['alpha'])
                logit = float(logits[offset])
                evidence = ({'kind': 'native-fraud', 'version': 1, 'fraud_logit': logit,
                             'fraud_probability': float(result['fraud_probability'][offset]),
                             'head': head.id, 'alpha': options['alpha']}
                            if self.training_mode == 'supervised' else
                            {'kind': 'native-likelihood', 'logit': logit,
                             'likelihood_probability': float(np.exp(-np.logaddexp(0, -logit))),
                             'tail_probability': float(result['tail_probability'][offset]), 'head': head.id,
                             'reference_count': head.reference_count, 'alpha': options['alpha']})
                rows.append({
                    'id': identifier, 'logit': logit, 'score': score,
                    **({'fraud_logit': logit} if self.training_mode == 'supervised' else {}),
                    'tauBefore': before, 'tauAfter': tau, 'decision': decision,
                    'settled': settled, 'evaluationEligible': not context,
                    'evidence': evidence,
                })
            return accepted
        ...
        if options['mode'] == 'shadow':
            ...
            policy_fields = {
                'shared': ('alpha', 'warmup', 'eta'), 'manual': ('manualTau',),
                'tuned': ('falseBlockCost', 'missedFraudCost'), 'auto': ('objective',),
            }[options['decisionPolicy']]
            identity = (fingerprint(graph), head.id, options['decisionPolicy'],
            ...
            if identity in self._enforce_cache:
                ...
                self.model._bind(graph, [])
                neighbors = _AcceptedNeighbors(graph, self.model.parameters['max_input_sequence_length'] - 1)
                ...
                with torch.no_grad():
                    for group in groups:
                        ...
                        if accepted:
                            ...
                            self.model._commit(graph, accepted_group, proposed[accepted])
                            neighbors.commit(accepted_group)
                self._enforce_cache[identity] = cached
                while len(self._enforce_cache) > 4:
                    self._enforce_cache.popitem(last=False)
        result = {
            'version': 2, 'schema': 'native-fraud-comparison/v2', 'dataset': document,
            'model': {'id': self.descriptor['id'], 'label': 'DyGFormer + TAMI · native',
                      'checkpoint_id': self.metadata['model_sha256'], 'implementation': self.descriptor,
                      'parameters': sum(parameter.numel() for parameter in self.model.network.parameters()),
                      'policy_validation': {self.training_mode: validation} if validation else {},
                      'training': {'mode': self.training_mode, 'task': self.metadata['task'],
                                   'encoder_training': self.metadata.get('encoder_training'),
                                   'head': head.id, 'label_counts': self.metadata.get('label_counts')}},
            'predictions': rows, 'heads': {identifier: value.to_dict() for identifier, value in self.heads.items()},
            'head': head.id, 'alpha': options['alpha'], 'options': options,
            'policy_state': policy_state, 'policyFit': fit,
            'evaluation_ids': evaluation, 'calibration': copy.deepcopy(self.calibration),
            'provenance': {'artifact_model_sha256': self.metadata['model_sha256'],
                           'task': self.metadata['task'],
                           'artifact_dataset': self.metadata['dataset'],
                           'same_dataset': same_dataset, 'implementation_sha256': self.metadata['implementation_sha256'],
                           'model_parameters': self.metadata.get('parameters', {}),
                           'evaluation_scope': ('saved-test' if self.training_mode == 'supervised' else 'saved-test-after-calibration') if same_dataset else 'separate-target-dataset',
                           'inference': 'native-cpu', 'shadow_cache_hit': cache_hit,
                           'enforce_cache_hit': enforce_cache_hit},
            'history': {'mode': options['mode'],
                        'payments': 'observed-attempts' if options['mode'] == 'shadow' else 'accepted-payments',
                        'timestamps': 'strictly-before', 'deposits': False, 'reports': False},
            'score_meaning': ('Fraud probability estimated from temporal interaction context, pair memory and candidate amount. Score = softplus(fraud logit) / ln(2); a score cutoff of 1 corresponds to estimated fraud probability above 0.5. Probabilities are not calibrated.'
                              if self.training_mode == 'supervised' else
                              'Low observed-link likelihood indicates unusual endpoints and timing. Current payment amount is not a scoring input. Neither link likelihood nor empirical tail rank is fraud probability.'),
        }
