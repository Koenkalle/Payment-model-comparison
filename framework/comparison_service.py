...
MODE_CAPABILITIES = {
    "unsupervised": {
        "prediction_heads": [
            entry["id"] for entry in prediction_head_manifest()["prediction_heads"]
        ],
    ...
    "supervised": {
        "prediction_heads": [
            entry["id"]
            for entry in prediction_head_manifest(kind="transaction-representation")[
                "prediction_heads"
            ]
        ],
...
CAPABILITIES = {
    ...
    "prediction_heads": [
        identifier
        for entry in MODE_CAPABILITIES.values()
        for identifier in entry["prediction_heads"]
    ],
...
OPTION_DEFAULTS = {
    ...
    "manualTau": 10.0,
    "falseBlockCost": 1.0,
    "missedFraudCost": 20.0,
    "objective": "f1",
    "predictionHead": "empirical_tail",
    "eta": 0.025,
}
...
def validate_request(payload):
    ...
    if payload.get("model_id") != MODEL_ID:
        raise ValueError(
            "Unsupported comparison model: " + str(payload.get("model_id"))
        )
    ...
    for key, allowed in choices.items():
        if options[key] not in allowed:
            raise ValueError(
                f"Unsupported {key}: {options[key]}. Supported: " + ", ".join(allowed)
            )
...
class ComparisonService:
    ...
    def __init__(
        ...
        *,
        ...
        scorer_factory=None,
        supervised_scorer_factory=None,
        cache_entries=4,
        max_cache_bytes=32 * 1024 * 1024,
    ):
        self.artifact = Path(artifact).resolve()
        self.calibration_config = Path(
            calibration_config or self.artifact / "calibration.config.json"
        ).resolve()
        self.supervised_artifact = (
            Path(supervised_artifact).resolve()
            if supervised_artifact is not None
            else None
        )
    ...
    def _load_locked(self, training_mode="unsupervised"):
        if training_mode not in self._scorers:
            try:
                ...
                if training_mode not in description.get(
                    "training_modes", [training_mode]
                ):
                    raise ValueError(
                        "The configured artifact does not support "
                        + training_mode
                        + " training."
                    )
                ...
                if description.get("artifact"):
                    ...
                    default = (
                        DEFAULT_ARTIFACT
                        if training_mode == "unsupervised"
                        else DEFAULT_SUPERVISED_ARTIFACT
                    )
                    artifact["source"] = (
                        "default"
                        if Path(artifact["path"]).resolve() == default.resolve()
                        else "custom"
                    )
    ...
    def models(self):
        ...
        with self._lock:
            for mode, defaults in MODE_CAPABILITIES.items():
                ...
                mode_capabilities[mode] = {
                    ...
                    "modes": list(
                        description.get("supported_modes", CAPABILITIES["modes"])
                    ),
                    "prediction_heads": [
                        head["id"] for head in description.get("prediction_heads", [])
                    ]
                    if "prediction_heads" in description
                    else list(defaults["prediction_heads"]),
                    "decision_policies": list(
                        description.get(
                            "supported_policies", CAPABILITIES["decision_policies"]
                        )
                    ),
                    "default_head": description.get(
                        "default_head", defaults["default_head"]
                    ),
                }
        capabilities = {
        ...
        available = {
            mode: entry
            for mode, entry in mode_capabilities.items()
            if entry["available"]
        }
        ...
        for key in ("modes", "prediction_heads", "decision_policies"):
            capabilities[key] = list(
                dict.fromkeys(
                    value for entry in available.values() for value in entry[key]
                )
            )
    ...
    def register_request(self, session=None, revision=None):
        ...
        if (
            ...
            or len(revision) > 16
            or int(revision) > 2**53 - 1
        ):
            raise ValueError(
                "Comparison session and integer revision headers must be supplied together."
            )
    ...
    def compare_bytes(self, payload, cancelled=None):
        ...
        with self._lock:
            ...
            if "predictionHead" not in payload.get("options", {}) and description.get(
                "default_head"
            ):
                options["predictionHead"] = description["default_head"]
            if options["mode"] not in description.get(
                "supported_modes", CAPABILITIES["modes"]
            ):
                raise ValueError(
                    "The selected history mode is unavailable in the configured artifact."
                )
            if options["predictionHead"] not in [
                head["id"]
                for head in description.get(
                    "prediction_heads", [{"id": options["predictionHead"]}]
                )
            ]:
                raise ValueError(
                    "The selected head is unavailable in the configured artifact."
                )
            if options["decisionPolicy"] not in description.get(
                "supported_policies", CAPABILITIES["decision_policies"]
            ):
                raise ValueError(
                    "The selected policy is unavailable for this artifact; historical validation needs both known classes."
                )
            ...
            check_cancelled()
            if description.get("artifact"):
                if result.get("model", {}).get("checkpoint_id") != description.get(
                    "checkpoint_id"
                ):
                    raise ValueError(
                        "Prediction checkpoint differs from the loaded model identity."
                    )
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def _trusted_request(self):
            try:
                ...
                if (
                    not _loopback(authority.hostname)
        ...
        def _get(self, head=False):
            if not self._trusted_request():
                self._error(
                    403, "This service accepts same-origin loopback requests only."
                )
                return
            try:
                ...
                if pathname.startswith("/api/pipeline/"):
                    if pipeline_service is None:
                        self._error(
                            503,
                            "The preparation pipeline is unavailable. Start the app with python serve.py.",
                        )
                        return
                    try:
                        result = pipeline_service.get(
                            pathname, urlsplit(self.path).query
                        )
                        self._send(
                            200,
                        ...
                        self._error(
                            500,
                            "Pipeline lookup failed. Check the server output and retry.",
                        )
                    return
                if pathname == "/api/datasets":
                    self._send(
                        200,
                        json.dumps(dataset_service.catalog(), allow_nan=False).encode(
                            "utf-8"
                        ),
                        head=head,
                    )
                ...
                if pathname == "/api/models":
                    try:
                        self._send(
                            200,
                            json.dumps(service.models(), allow_nan=False).encode(
                                "utf-8"
                            ),
                            head=head,
                        )
                        ...
                        self._error(
                            500,
                            "Model discovery failed. Check the server output and retry.",
                        )
                    return
        ...
        def do_POST(self):
            ...
            graph_query = re.fullmatch(
                r"/api/pipeline/datasets/ds-[0-9a-f]{32}/graph/query", endpoint
            )
            feature_selection = re.fullmatch(
                r"/api/pipeline/datasets/ds-[0-9a-f]{32}/features", endpoint
            )
            if (
                endpoint
                not in (
                    "/api/compare",
                    "/api/datasets/load",
                    "/api/pipeline/datasets/generate",
                    "/api/pipeline/datasets/import",
                    "/api/pipeline/train",
                    "/api/pipeline/compare",
                )
                and not graph_query
                and not feature_selection
            ):
                self._error(404, "Unknown API endpoint.")
                return
            ...
            try:
                ...
                if endpoint.startswith("/api/pipeline/"):
                    ...
                    self._send(
                        status, json.dumps(result, allow_nan=False).encode("utf-8")
                    )
                    return
                if endpoint == "/api/datasets/load":
                    self._send(
                        200,
                        json.dumps(
                            dataset_service.load(payload), allow_nan=False
                        ).encode("utf-8"),
                    )
                    return
                validate_request(payload)
                superseded = service.register_request(
                    self.headers.get("X-Comparison-Session"),
                ...
                self._error(
                    500,
                    "Server operation failed. Check the server output; a new request can be retried.",
                )
