...
from .temporal_fraud_data import (
...
def chronological_splits(dataset, config=None):
    ...
    if "boundaries" in config:
        ...
        if (
            ...
            raise ValueError(
                "Split boundaries must be three increasing finite relative seconds."
            )
    else:
        if set(config) - set(PARTITIONS[:-1]):
            raise ValueError(
                "Fraud splits use train, model_validation and policy_validation fractions."
            )
        fractions = [
            config.get(key, default)
            for key, default in zip(PARTITIONS, (0.6, 0.15, 0.1))
        ]
        if (
            any(
            ...
            raise ValueError(
                "Positive fraud split fractions must leave a nonempty test period."
            )
        positions = np.floor(np.cumsum(fractions) * len(unique)).astype(int)
        if (
            not len(unique)
            or positions[0] < 1
            or positions[-1] >= len(unique)
            or np.any(np.diff(positions) <= 0)
        ):
            raise ValueError(
                "Need enough distinct timestamps for four nonempty fraud partitions."
            )
        boundaries = unique[positions]
    membership = np.searchsorted(boundaries, dataset.times, side="right")
    result = {
        name: np.flatnonzero(membership == i) for i, name in enumerate(PARTITIONS)
    }
...
def label_cutoffs(dataset, splits, configured=None):
    ...
    if set(configured) - set(PARTITIONS[:-1]):
        raise ValueError(
            "Label cutoffs apply to train, model_validation and policy_validation."
        )
    result = {}
    for name, following in zip(PARTITIONS, PARTITIONS[1:]):
        ...
        if type(value) not in (int, float) or not np.isfinite(value) or value > limit:
            raise ValueError(
                f"{name} label cutoff must precede the next partition in relative seconds."
            )
...
def _path_config(config, base, relative_to=None):
    ...
    if "path" in result:
        ...
        result["path"] = (
            os.path.relpath(path, relative_to) if relative_to is not None else str(path)
        )
    if "paths" in result:
        result["paths"] = [
            _path_config({"path": path}, base, relative_to)["path"]
            for path in result["paths"]
        ]
    if "episodes" in result:
        result["episodes"] = [
            _path_config(episode, base, relative_to) for episode in result["episodes"]
        ]
    return result


def _initialize(config, dataset, splits, parameters, base, fraud_model):
    initialization = config.get(
    ...
    if method == "checkpoint":
        ...
        if (
            saved.get("task") != "dynamic-link-prediction"
            or saved.get("model_id") != config["model"]
        ):
            raise ValueError(
                "Initialization requires a link checkpoint for the same model."
            )
        if saved.get("implementation_sha256") != source_hashes(descriptor) or saved.get(
            "model_sha256"
        ) != digest(path / "model.npz"):
            raise ValueError(
                "Initialization checkpoint source or weights checksum does not match."
            )
        checksum = dataset.provenance.get("source_sha256")
        if not checksum or saved.get("dataset", {}).get("source_sha256") != checksum:
            raise ValueError(
                "Checkpoint initialization requires verified source data matching this training dataset."
            )
        ...
        if link_fingerprint(saved_graph) != saved.get("dataset_fingerprint"):
            raise ValueError(
                "Initialization checkpoint graph or conversion differs from the fraud dataset."
            )
        ...
        if not used or not used <= allowed:
            raise ValueError(
                "Initialization checkpoint used transactions outside the fraud training interval."
            )
        model.load_graph(path / "model.npz", dataset.graph)
        if model.feature_schema != saved["features"] or model.parameters != {
            **saved["parameters"],
            "device": model.parameters["device"],
        }:
            raise ValueError(
                "Initialization checkpoint metadata differs from saved features or parameters."
            )
        LOGGER.info("Encoder initialization: loaded verified link checkpoint %s.", path)
        return model, {
            "method": method,
            "model_sha256": saved["model_sha256"],
            "training_ids": saved["split"]["train"],
            "selection_ids": saved["split"]["validation"],
...
def report(model, dataset, indices, threshold, partition, cutoff=None):
    ...
    statistics.update(
        {
            **counts,
            "average_precision": float(
                average_precision_score(labels[known], scores[known])
            )
            if both
            else None,
            "roc_auc": float(roc_auc_score(labels[known], scores[known]))
            if both
            else None,
            "log_loss": float(
                np.mean(np.logaddexp(0, logits[known]) - labels[known] * logits[known])
            )
            if known.any()
            else None,
            "false_block_rate": statistics["fp"] / counts["legitimate"]
    ...
    return {
        "version": 1,
        "schema": "temporal-fraud-evaluation/v1",
        "task": TASK,
        "partition": partition,
        "threshold": float(threshold),
        "threshold_units": "fraud-surprise-bits",
        "history": "observed attempts; strictly earlier context; frozen weights and reset state",
        "label_cutoff": cutoff,
        "dataset": dataset.provenance,
        "metrics": statistics,
        "metric_definitions": {
            "average_precision": "Non-interpolated average precision on known payment outcomes; fraud is positive."
        },
        "rows": [
            {
                "id": dataset.ids[i],
                "timestamp_seconds": float(dataset.times[i]),
                "fraud_logit": float(logits[j]),
                "fraud_probability": float(probabilities[j]),
                "score": float(scores[j]),
                "label": int(labels[j]) if labels[j] >= 0 else None,
                "decision": "BLOCK" if scores[j] > threshold else "ALLOW",
            }
...
def train(config, output, base_dir=None):
    ...
    counts = {
        name: class_counts(available_labels(dataset, cutoffs.get(name))[indices])
        for name, indices in splits.items()
    }
    LOGGER.info(
        "Fraud dataset: %d payments; train=%d (%d fraud), model_validation=%d (%d fraud), "
        "policy_validation=%d (%d fraud), test=%d (%d fraud).",
        len(dataset.ids),
        counts["train"]["rows"],
        counts["train"]["fraud"],
        counts["model_validation"]["rows"],
        counts["model_validation"]["fraud"],
        counts["policy_validation"]["rows"],
        counts["policy_validation"]["fraud"],
        counts["test"]["rows"],
        counts["test"]["fraud"],
    )
    for name in ("train", "model_validation"):
        if not counts[name]["fraud"] or not counts[name]["legitimate"]:
            raise ValueError(
                f"{name} requires both confirmed fraud and legitimate labels available by its cutoff; counts: {counts[name]}. Supply a labeled history or adjust chronological boundaries."
            )
    if "decision_threshold" in config:
        raise ValueError(
            "Fraud experiments use score_threshold in bits, not a link probability decision_threshold."
        )
    threshold = config.get("score_threshold")
    if threshold is not None and (
        type(threshold) not in (int, float) or not np.isfinite(threshold)
    ):
    ...
    parameters = {
        **config.get("parameters", {}),
        ...
        "training_label_cutoff": cutoffs["train"],
        "validation_label_cutoff": cutoffs["model_validation"],
    }
    pretrained, lineage = _initialize(config, dataset, splits, parameters, base, model)
    model.fit_fraud(
        dataset,
        splits["train"],
        splits["model_validation"],
        parameters,
        pretrained_model=pretrained,
    )
    LOGGER.info(
        "Selecting the decision threshold on %d policy-validation payments.",
    ...
    if threshold is not None:
        ...
        counts["policy_validation"]["fraud"]
        and counts["policy_validation"]["legitimate"]
    ):
        ...
        threshold = choose_threshold(
            available_labels(dataset, cutoffs["policy_validation"])[
        ...
        threshold, threshold_source = (
            1.0,
            "default_probability_0.5_missing_policy_classes",
        )
    LOGGER.info(
        "Evaluating the frozen best model on %d test payments.", len(splits["test"])
    )
    ...
    metadata = {
        ...
        "parameters": model.parameters,
        "encoder_training": parameters["encoder_training"],
        "prediction_head": model.parameters["prediction_head"],
        ...
        **(
            ...
            if model.uses_dataset_features
            else {"amount_normalization": model.amount_normalization}
        ),
        "positive_class_weight": model.positive_class_weight,
        "dataset": dataset.provenance,
        "dataset_config": _path_config(config["dataset"], base),
        "dataset_fingerprint": fingerprint(dataset),
        "graph_fingerprint": fingerprint(dataset, include_labels=False),
        "split": {
            name: [dataset.ids[i] for i in indices] for name, indices in splits.items()
        },
        "split_bounds": {
            name: [float(dataset.times[indices[0]]), float(dataset.times[indices[-1]])]
            for name, indices in splits.items()
        },
        "label_cutoffs": cutoffs,
        "label_counts": counts,
        "threshold": float(threshold),
    ...
    with tempfile.TemporaryDirectory(
        prefix=".payment-fraud-", dir=output.parent
    ) as temporary:
        ...
        (stage / "calibration.config.json").write_text(
            json.dumps(
                {
                    "dataset": _path_config(config["dataset"], base, output),
                    "graph": graph_config,
                },
                indent=2,
            )
...
def load_artifact(artifact, dataset_config=None, base_dir=None):
    ...
    model, descriptor = create_model(
        metadata["model_id"], metadata["input_schema"], task=TASK
    )
    if source_hashes(descriptor) != metadata["implementation_sha256"]:
        raise ValueError(
            "Fraud model implementation changed since training; regenerate the artifact."
        )
    if dataset_config is None:
        ...
        config = (
            ...
            if path.exists()
            else {"dataset": metadata["dataset_config"]}
        )
        dataset_config, base_dir = config["dataset"], artifact
        if config.get("graph", metadata["graph"]) != metadata["graph"]:
            raise ValueError(
                "Historical graph configuration differs from the artifact."
            )
    dataset = load_labeled_dataset(dataset_config, base_dir, metadata["graph"])
    if fingerprint(dataset) != metadata["dataset_fingerprint"]:
        raise ValueError(
            "Fraud artifact requires the original historical graph and outcomes."
        )
    if metadata["dataset"].get("source_sha256") != dataset.provenance.get(
        "source_sha256"
    ):
        raise ValueError(
            "Historical dataset source checksum differs from the artifact."
        )
    ...
    if model.experiment_metadata != expected_metadata:
        raise ValueError(
            "Fraud experiment metadata differs from the provenance saved with its weights."
        )
    ...
    if (
        model.parameters.get("prediction_head") != metadata["prediction_head"]
        or model.parameters.get("encoder_training") != metadata["encoder_training"]
    ):
        raise ValueError(
            "Fraud artifact head or training mode differs from saved weights."
        )
    ...
    flattened = [
        identifier for name in PARTITIONS for identifier in metadata["split"][name]
    ]
    if flattened != list(dataset.ids):
        raise ValueError("Fraud artifact partition membership is invalid.")
    splits = {
        name: np.asarray([indices[i] for i in metadata["split"][name]])
        for name in PARTITIONS
    }
    if any(not len(v) for v in splits.values()) or any(
        dataset.times[splits[a][-1]] >= dataset.times[splits[b][0]]
...
def evaluate(artifact, dataset_config, base_dir=None, partition="all"):
    ...
    if partition == "all":
        ...
        if not same:
            raise ValueError(
                "Saved fraud partition membership requires the original dataset and outcomes."
            )
    ...
    result["training_rows_in_evaluation"] = (
        sum(dataset.ids[i] in trained for i in indices) if same else None
    )
