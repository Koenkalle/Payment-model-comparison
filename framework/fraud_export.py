...
def _artifact(path):
    ...
    if (metadata.get('version') != 1 or metadata.get('model_file') != 'model.npz'
        ...
        raise ValueError('export-fraud requires a native DyGFormer + TAMI link-model artifact.')
...
def _partitions(metadata, graph):
    ...
    if any(not isinstance(group, list) or not group for group in groups):
        raise ValueError('Artifact requires nonempty saved train, validation and test splits.')
    flat = [identifier for group in groups for identifier in group]
    if flat != list(graph.ids):
        raise ValueError('Saved splits must cover the original graph once in chronological order.')
    ...
    return {name: np.arange(offsets[i], offsets[i + 1])
...
def _payment_signature(document, event):
    ...
    return (str(accounts[event['u']]['external_id']),
            str(accounts[event['v']]['external_id']),
            float(event['t']), float(event['amount']))


def _evaluation_ids(metadata, calibration_indices, original_graph, original_events,
                    target_graph, target_events):
    same = _same_graph(original_graph, target_graph)
    reserved = (set(metadata['split']['train']) | set(metadata['split']['validation'])
                | {original_graph.ids[i] for i in calibration_indices})
    if same:
        evaluation = [identifier for identifier in metadata['split']['test'] if identifier not in reserved]
        if not evaluation:
            raise ValueError('Calibration consumes the entire test partition; reduce calibration.max_rows or use a separate target dataset.')
    ...
    used = {_payment_signature(original_events, event) for event in original_events['events']
            if event['kind'] == 'payment' and event['id'] in reserved}
    overlaps = [event['id'] for event in target_events['events']
                if event['kind'] == 'payment' and _payment_signature(target_events, event) in used]
    if overlaps:
        raise ValueError('Target overlaps training, validation or calibration transactions; use the original full dataset for automatic held-out selection, or a separate dataset. Example: ' + overlaps[0])
    source_hash = original_graph.provenance.get('source_sha256')
    if source_hash is not None and source_hash == target_graph.provenance.get('source_sha256'):
        raise ValueError('The original source was reinterpreted with a different graph configuration; this cannot be treated as independent evaluation data.')
    return list(target_graph.ids), False


def _positive_logits(model, graph, indices, seed):
    values = np.asarray(model.predict_graph(graph, indices, seed)['positive_logits'], dtype=float)
...
def _metrics(document, ids, logits, head, alpha):
    ...
    actual = np.asarray([int(truth[identifier]) if identifier in truth else -1 for identifier in ids])
    known = actual >= 0
    tp = int(np.sum((actual == 1) & flags))
    fp = int(np.sum((actual == 0) & flags))
    fn = int(np.sum((actual == 1) & ~flags))
    tn = int(np.sum((actual == 0) & ~flags))
    return {'rows': len(ids), 'known': int(known.sum()), 'unknown': int((~known).sum()),
            'flagged': int(flags.sum()), 'flag_rate': float(flags.mean()),
            'tp': tp, 'fp': fp, 'fn': fn, 'tn': tn,
            'precision': tp / (tp + fp) if tp + fp else None,
            'recall': tp / (tp + fn) if tp + fn else None,
            'f1': 2 * tp / (2 * tp + fp + fn) if 2 * tp + fp + fn else (0 if known.any() else None)}
...
def export_fraud(config, base_dir=None):
    ...
    if graph_config.get('loader') != 'payment_graph':
        raise ValueError('Calibration requires the original payment_graph dataset and configuration.')
    original = load_dataset(graph_config, base)
    if not isinstance(original, TemporalGraphDataset) or fingerprint(original) != metadata.get('dataset_fingerprint'):
        raise ValueError('Calibration requires the original graph dataset and configuration used to fit the artifact.')
    ...
    target_graph = load_dataset({'loader': 'payment_graph', 'dataset': target_config,
                                 'node_feature_dim': len(node_names)}, base)
    evaluation, same_dataset = _evaluation_ids(metadata, calibration, original, original_events.document,
                                              target_graph, target.document)
    ...
    heads = {identifier: create_head(identifier).fit(reference) for identifier in head_ids}
    logits = _positive_logits(model, target_graph, np.arange(len(target_graph.ids)), seed)
    ...
    result = {
        'version': 1, 'schema': 'native-fraud-comparison/v1',
        'dataset': target.document,
        'model': {'id': descriptor['id'], 'label': 'DyGFormer + TAMI · native',
                  'checkpoint_id': metadata['model_sha256'], 'implementation': descriptor},
        'predictions': [{'id': identifier, 'logit': float(logit)}
                        for identifier, logit in zip(target_graph.ids, logits)],
        'heads': {identifier: head.to_dict() for identifier, head in heads.items()},
        'head': selected, 'alpha': float(alpha), 'evaluation_ids': evaluation,
        'calibration': {
            'ids': [original.ids[i] for i in calibration], 'count': count,
            'partition': 'test-prefix', 'dataset_fingerprint': metadata['dataset_fingerprint'],
        ...
        'provenance': {
            ...
            'artifact_dataset': metadata['dataset'], 'target_dataset': target.provenance,
            'same_dataset': same_dataset,
            'implementation_sha256': metadata['implementation_sha256'],
            'model_parameters': metadata.get('parameters', {}),
            'evaluation_scope': 'saved-test-after-calibration' if same_dataset else 'separate-target-dataset',
        ...
        'history': {'mode': 'shadow', 'payments': 'observed-attempts',
        ...
        'metrics': {identifier: _metrics(target.document, evaluation, evaluation_logits, head, alpha)
