...
class Head(nn.Module):
    ...
    def __init__(self, input_dim, parameters=None):
        ...
        if parameters:
            raise ValueError('fraud_linear has no configurable architecture parameters.')
        self.input_dim = input_dim
        self.linear = nn.Linear(input_dim, 1)

    def forward(self, representation):
        if representation.ndim != 2 or representation.shape[1] != self.input_dim:
            raise ValueError('Fraud-head transaction representation has incompatible dimensions.')
        return self.linear(representation).flatten()

    def descriptor(self):
        return {'id': self.id, 'input_contract': self.input_contract,
                'output_contract': self.output_contract, 'input_dim': self.input_dim,
                'parameters': {}}
...
class FraudLogitScorer:
    ...
    def __init__(self, head_id='fraud_linear'):
        ...
        ids = {entry['id'] for entry in manifest(kind='transaction-representation')['prediction_heads']}
    ...
    def score(self, logits):
        ...
        return {'fraud_probability': probability, 'score': np.logaddexp(0, values) / np.log(2)}

    def to_dict(self):
        return {'version': 1, 'id': self.id, 'kind': 'learned-fraud',
                'input': 'transaction-representation', 'output': 'fraud-logit',
                'score_input': 'fraud-logit', 'score_output': 'fraud-probability',
