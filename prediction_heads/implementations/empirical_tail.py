...
def _logits(values, *, nonempty=False):
    try:
        ...
        raise ValueError('Link logits must be a finite one-dimensional numeric array.') from exc
...
def _threshold(alpha):
    ...
    try:
        ...
        raise ValueError('Tail cutoff alpha must be a finite number in (0, 1].') from exc
