...
def manifest(kind='link-logits'):
    ...
    if len(set(ids)) != len(ids) or set(ids) != set(implementations):
        raise ValueError('Prediction-head registry does not match the implementation allowlist.')
    ...
    if kind is not None:
        payload['prediction_heads'] = [entry for entry in entries if entry['kind'] == kind]
    return payload
...
def create_learned_head(identifier, input_dim, parameters=None):
    ...
    return importlib.import_module(_LEARNED_IMPLEMENTATIONS[identifier]).Head(input_dim, parameters)
