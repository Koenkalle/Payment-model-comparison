...
def main():
    ...
    inspect=sub.add_parser('inspect-data',help='Inspect source capabilities, ordering, provenance and label coverage.')
    ...
    try:
        if args.command=='list':
            ...
            for source in source_catalog():print('sources',source['dataset_id'],'entities='+(','.join(source['entity_types']) or 'none'),'features='+','.join(source['feature_names']))
            return
        ...
        if args.command=='inspect-data':
            ...
            print(json.dumps(inspect_dataset(config,base),indent=2,allow_nan=False));return
        ...
        if args.command=='export-fraud':
            ...
            if config.get('loader')=='fraud_dataset' and config.get('view','stream')=='stream':
                if config.get('selection'):raise ValueError('Prepare the complete stream, then select an interval in a numeric, graph or payments view.')
                from datasets.stream import prepare_source
                with prepare_source(config,base,args.output):pass
                print(args.output);return
            data=load_dataset(config,base)
            if not isinstance(data,EventDataset):
                if hasattr(data,'close'):data.close()
                raise ValueError('prepare exports view="payments" to JSON or fraud_dataset view="stream" to SQLite; numeric and graph views feed the experiment runner directly.')
