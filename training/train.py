...
def main():
    ...
    if not (ROOT/'training-data.json').exists():subprocess.run(['node','-e',"require('fs').writeFileSync('training-data.json',JSON.stringify(require('./shared/runtime/scenarios.js').trainingEpisodes()))"],cwd=ROOT,check=True)
