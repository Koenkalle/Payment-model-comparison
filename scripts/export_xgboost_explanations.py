...
SOURCE_PATHS = (
    'models/xgboost.json', 'xgb-training.json', 'training/train.py',
    'training/autodiff.py', 'shared/runtime/xgboost.js',
...
def model_fingerprint(model):
    return fingerprint({key: model[key] for key in (
        'features', 'amount_bins', 'base_score', 'learning_rate', 'trees')})
...
def count_tree(tree, rows):
    ...
    for row in rows:
        ...
        while True:
            ...
            if 'leaf' in node:
                break
            index = current['left' if row[node['feature']] <= node['threshold'] else 'right']
...
def create_sidecar(hashes=None):
    ...
    if model['features'] != XGB_FEATURES or model['amount_bins'] != AMOUNT:
        raise ValueError('Checkpoint feature schema differs from the training feature extractor.')
    if (model['training']['fraud_seed'] != history['seed'] or
            model['training']['fraud_flags_requested'] != history['flags_requested']):
        raise ValueError('Historical data does not match the checkpoint training configuration.')
    rows, labels, episode_ids = xgb_rows(history)
    split = max(1, int(len(history['episodes']) * .8))
    fitting = episode_ids < split
    reference_rows = rows[fitting]
    if (len(reference_rows) != model['training']['training_rows'] or
            int(labels[fitting].sum()) != model['training']['fraud_labels_used']):
        raise ValueError('Reconstructed fitting population differs from checkpoint metadata.')
    sidecar = {
        'version': 1,
...
def is_fresh(sidecar, hashes):
    ...
    return (sidecar.get('version') == 1 and
            sidecar.get('feature_schema_version') == 1 and
            sidecar.get('provenance') == hashes and
            sidecar.get('reference_id') == fingerprint(unsigned))


def main():
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument('--check', action='store_true', help='Fail if the sidecar is absent or stale; do not write files.')
    parser.add_argument('--force', action='store_true', help='Reconstruct the fitting rows even if the sidecar is fresh.')
    ...
    if not args.force and is_fresh(existing, hashes):
        print('XGBoost explanation reference is current (' + str(existing['reference']['row_count']) + ' fitting rows).')
        return
    if args.check:
        raise SystemExit('XGBoost explanation reference is stale; run python scripts/export_xgboost_explanations.py.')
    result = create_sidecar(hashes)
    OUTPUT.parent.mkdir(parents=True, exist_ok=True)
    OUTPUT.write_text(json.dumps(result, separators=(',', ':')) + '\n')
    print('Exported XGBoost explanation reference from ' + str(result['reference']['row_count']) + ' fitting rows; model weights unchanged.')
