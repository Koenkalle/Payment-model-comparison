...
def validate_registry(registry):
    ...
    for tool in registry['tools']:
        ...
        if not re.fullmatch(r'[a-z][a-z0-9-]*\.html', tool['output']) or tool['output'] in outputs:
...
def navigation(registry, active):
    ...
    for tool in registry['tools']:
        ...
        links.append(f'<a href="{html.escape(tool["output"], quote=True)}"{current}>{html.escape(tool["label"])}</a>')
    return source('shared/ui/navigation.html').replace('<!-- TOOL_LINKS -->', ''.join(links))
...
def main():
    ...
    subprocess.run(['node', 'scripts/tune_policy.js', '--if-stale'], cwd=ROOT, check=True)
    if any(tool.get('explanation') for tool in selected):
        subprocess.run([sys.executable, 'scripts/export_xgboost_explanations.py'], cwd=ROOT, check=True)
    validation = read_json('policy-validation.json')
    models = [read_json(entry['checkpoint']) for entry in plugin_manifest('models')['models'] if entry.get('browser') and entry.get('checkpoint')]
    ...
    bundle = {'version': 5, 'default': catalog['default'], 'policy': {
        'training_mode': 'unsupervised', 'decision_policy': 'shared',
        'validation': validation['provenance'], 'warmup': catalog.get('default_warmup', 128),
        'fraud_flags': xgb['training'].get('fraud_flags_requested', 96)}, 'models': models}
    (ROOT / 'model-bundle.json').write_text(json.dumps(bundle, separators=(',', ':')))
    ui_scripts = '\n'.join('<script>\n' + source(name) + '\n</script>' for name in registry.get('ui_scripts', []))
    shell = source('shared/ui/standalone-shell.html').replace('<!-- SHARED_UI_SCRIPTS -->', ui_scripts)
    for tool in selected:
        payload = dict(bundle)
        if tool['model_ids'] != 'all':
            payload['models'] = [model for model in models if model['id'] in tool['model_ids']]
        ...
        if '<!-- TOOL_DATA -->' in template:
            ...
            if at < 0:
                raise ValueError(f'Tool template needs a data marker or root: {tool["template"]}')
